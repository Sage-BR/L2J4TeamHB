...
public final class L2ItemInstance extends L2Object
{
	...
	public static final int CHARGED_BLESSED_SOULSHOT = 2; // It's a really
	                                                      // exists? ;-)
	        ...
	        L2Object reference)
	{
		...
		if (creator != null)
		{
			if (creator.isGM())
			{
				...
				GMAudit.auditGMAction(creator.getName(), process + "(id: "
				        + getItemId() + " name: " + getName()
				        + ")", targetName, "L2Object referencing this action is: "
				                + referenceName);
			}
		}
	}
	        ...
	        L2Object reference)
	{
		...
		if (creator != null)
		{
			if (creator.isGM())
			{
				...
				String targetName = (creator.getTarget() != null ? creator.getTarget().getName() : "no-target");
				GMAudit.auditGMAction(creator.getName(), process + "(id: "
				        + getItemId() + " objId: " + getObjectId() + " name: "
				        + getName() + " count: " + count
				        + ")", targetName, "L2Object referencing this action is: "
				                + referenceName);
			}
	...
	public void changeCountWithoutTrace(int count, L2PcInstance creator,
	        L2Object reference)
	{
	...
	public boolean isHeroItem()
	{
		return ((_itemId >= 6611 && _itemId <= 6621)
		        || (_itemId >= 9388 && _itemId <= 9390) || _itemId == 6842);
	}

	public boolean isCommonItem()
	{
		return ((_itemId >= 12006 && _itemId <= 12361)
		        || (_itemId >= 11605 && _itemId <= 12308));
	}
	...
	public void onAction(L2PcInstance player)
	{
		...
		{
			if (player.isInParty())
			{ // do not allow owner who is in party to
				// pick tickets up
	...
	public void updateDatabase(boolean force)
	{
		...
		if (_existsInDb)
		{
			if (_ownerId == 0 || _loc == ItemLocation.VOID
		...
		else
		{
			...
			if (_loc == ItemLocation.VOID || _loc == ItemLocation.NPC
			        || _ownerId == 0)
			{
				return;
			}
	...
	public static L2ItemInstance restoreFromDb(int ownerId, ResultSet rs)
	{
		...
		int objectId, item_id, count, loc_data, enchant_level, custom_type1,
		        custom_type2, manaLeft;
		ItemLocation loc;
		try
		{
		...
		{
			_log.log(Level.SEVERE, "Could not restore an item owned by "
			        + ownerId + " from DB:", e);
			return null;
		}
		...
		if (item == null)
		{
			_log.severe("Item item_id=" + item_id + " not known, object_id="
			        + objectId);
			return null;
		}
		...
		if (inst.isEquipable())
		{
			...
			{
				_log.log(Level.SEVERE, "Could not restore augmentation for item "
				        + objectId + " from DB: " + e.getMessage(), e);
			}
			finally
			{
				try
	...
	public class doItemDropTask implements Runnable
	{
		...
		public doItemDropTask(L2ItemInstance item, L2Character dropper, int x,
		        int y, int z)
		{
			_x = x;
			_y = y;
	...
	private void updateInDb()
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("UPDATE items SET owner_id=?,count=?,loc=?,loc_data=?,enchant_level=?,custom_type1=?,custom_type2=?,mana_left=? "
			        + "WHERE object_id = ?");
	...
	private void insertIntoDb()
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("INSERT INTO items (owner_id,item_id,count,loc,loc_data,enchant_level,object_id,custom_type1,custom_type2,mana_left) "
			        + "VALUES (?,?,?,?,?,?,?,?,?,?)");
			statement.setInt(1, _ownerId);
	...
	private void removeFromDb()
	{
		...
		{
			_log.log(Level.SEVERE, "Could not delete item " + getObjectId()
			        + " in DB: " + e.getMessage(), e);
		}
		finally
		{
			try
