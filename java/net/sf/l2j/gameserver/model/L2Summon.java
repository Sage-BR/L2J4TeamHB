...
public abstract class L2Summon extends L2PlayableInstance
{
	...
	public L2Summon(int objectId, L2NpcTemplate template, L2PcInstance owner)
	{
		...
		setXYZInvisible(owner.getX() + 50, owner.getY() + 100, owner.getZ()
		        + 100);
	}
	...
	public void onSpawn()
	{
		...
		this.setShowSummonAnimation(false); // addVisibleObject created the info
		                                    // packets with summon animation
		                                    // if someone comes into range now,
		                                    // the animation shouldnt show any
		                                    // more
	...
	public void onAction(L2PcInstance player)
	{
		...
		else if (player.getTarget() == this)
		{
			...
			{
				// This Action Failed packet avoids player getting stuck when
				// clicking three or more times
				player.sendPacket(ActionFailed.STATIC_PACKET);
				if (Config.GEODATA > 0)
				{
					if (GeoData.getInstance().canSeeTarget(player, this))
	...
	public boolean isInvul()
	{
		return _isInvul || _isTeleporting
		        || (getOwner() != null && getOwner().isInvul());
	}
