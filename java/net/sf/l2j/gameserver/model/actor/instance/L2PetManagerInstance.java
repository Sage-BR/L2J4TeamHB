...
public class L2PetManagerInstance extends L2MerchantInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (command.startsWith("evolve"))
		{
			...
			switch (val)
			{
				// Info evolve(player, "curent pet summon item", "new pet summon
				// item", "lvl required to evolve")
				// To ignore evolve just put value 0 where do you like example:
				// evolve(player, 0, 9882, 55);
				case 1:
	...
	public final void exchange(L2PcInstance player, int itemIdtake,
	        int itemIdgive)
	{
	...
	public final void evolve(L2PcInstance player, int itemIdtake,
	        int itemIdgive, int petminlvl)
	{
		NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		L2Summon curpet = player.getPet();

		if ((curpet == null) || (itemIdtake == 0) || (itemIdgive == 0)
		        || (petminlvl == 0))
		{
		...
		petSummon.spawnMe(player.getX() + 50, player.getY()
		        + 100, player.getZ());
