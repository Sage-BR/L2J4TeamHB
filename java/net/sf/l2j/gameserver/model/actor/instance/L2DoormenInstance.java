...
public class L2DoormenInstance extends L2FolkInstance
{
		...
		else if (condition == COND_CASTLE_OWNER || condition == COND_HALL_OWNER)
		{
			...
			else if (command.startsWith("open_doors"))
			{
				if (condition == COND_HALL_OWNER)
				{
					...
					player.sendPacket(new NpcHtmlMessage(getObjectId(), "<html><body>You have <font color=\"FF9955\">opened</font> the clan hall door.<br>Outsiders may enter the clan hall while the door is open. Please close it when you've finished your business.<br><center><button value=\"Close\" action=\"bypass -h npc_"
					        + getObjectId()
			...
			else if (command.startsWith("close_doors"))
			{
				if (condition == COND_HALL_OWNER)
				{
					...
					player.sendPacket(new NpcHtmlMessage(getObjectId(), "<html><body>You have <font color=\"FF9955\">closed</font> the clan hall door.<br>Good day!<br><center><button value=\"To Beginning\" action=\"bypass -h npc_"
					        + getObjectId()
	...
	public void showMessageWindow(L2PcInstance player)
	{
		...
		if (getClanHall() != null)
		{
			if (condition == COND_HALL_OWNER)
			{
				str = "<html><body>Greetings!<br><br><font color=\"00FFFF\">"
				        + ClanTable.getInstance().getClan(getClanHall().getOwnerId()).getName()
			...
			else
			{
				...
				if (owner != null && owner.getLeader() != null)
				{
					...
					str += "I am sorry, but only the clan members who belong to the "
					        + owner.getName()
					        + " clan can enter the clan hall.</body></html>";
				}
				else
				{
