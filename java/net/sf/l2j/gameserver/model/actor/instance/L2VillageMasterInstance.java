...
public final class L2VillageMasterInstance extends L2FolkInstance
{
		...
		else if (command.startsWith("Subclass"))
		{
			...
			switch (cmdChoice)
			{
				case 1: // Add Subclass - Initial
					// Avoid giving player an option to add a new sub class, if
				...
				case 4: // Add Subclass - Action (Subclass 4 x[x])
					...
					{
						_villageMasterLog.warning("Player " + player.getName()
						        + " has performed a subclass change too fast");
						return;
					}
					 ...
					 * completed the Mimir's Elixir (Path to Subclass) and
					 * Fate's Whisper (A Grade Weapon) quests by checking for
					 * instances of their unique reward items.
					 *
					...
					if (allowAddition)
					{
						...
						if (!player.addSubClass(paramOne, player.getTotalSubClasses()
						        + 1))
						{
						...
						player.sendPacket(new SystemMessage(SystemMessageId.CLASS_TRANSFER)); // Transfer
						                                                                      // to
						                                                                      // new
						                                                                      // class.
					}
				...
				case 7: // Change Subclass - Action
					/*
					 * Warning: the information about this subclass will be
					 * removed from the subclass list even if false!
					 */
					...
					{
						player.stopAllEffects(); // all effects from old
						                         // subclass stopped!
						player.setActiveClass(paramOne);
						...
						player.sendPacket(new SystemMessage(SystemMessageId.ADD_NEW_SUBCLASS)); // Subclass
						                                                                        // added.
					}
					else
					{
						/*
						 * This isn't good! modifySubClass() removed subclass
						 * from memory we must update _classIndex! Else
						 * IndexOutOfBoundsException can turn up some place down
						 * the line along with other seemingly unrelated
						 * problems.
						 */
						player.setActiveClass(0); // Also updates _classIndex
						                          // plus switching _classid to
						                          // baseclass.
	...
	public void dissolveClan(L2PcInstance player, int clanId)
	{
		...
		clan.setDissolvingExpiryTime(System.currentTimeMillis()
		        + Config.ALT_CLAN_DISSOLVE_DAYS * 86400000L); // 24*60*60*1000 =
		                                                      // 86400000
		clan.updateClanInDB();
	...
	private final Set<PlayerClass> getAvailableSubClasses(L2PcInstance player)
	{
		...
		if (((charClassId >= 88 && charClassId <= 118)
		        || (charClassId >= 131 && charClassId <= 134)
		        || charClassId == 136)
		 ...
		 * each class as a subclass to the other class.
		 *
		 * If the race of your main class is Kamael, you may not subclass any
		 * other race If the race of your main class is NOT Kamael, you may not
		 * subclass any Kamael class
		 *
		 ...
		 * Spellhowler
		 *
		 * Also, Kamael have a special, hidden 4 subclass, the inspector, which
		 * can only be taken if you have already completed the other two Kamael
		 * subclasses
		 *
		 */
	...
	public void showPledgeSkillList(L2PcInstance player)
	{
		...
		if ((player.getClan() == null) || player.isTransformed())
		{
			return;
		}
