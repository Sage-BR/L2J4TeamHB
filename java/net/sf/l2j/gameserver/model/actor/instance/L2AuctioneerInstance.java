...
public final class L2AuctioneerInstance extends L2FolkInstance
{
	...
	private Map<Integer, Auction> _pendingAuctions = new ConcurrentHashMap<>();
		...
		else if (condition == COND_REGULAR)
		{
			...
			if (actualCommand.equalsIgnoreCase("auction"))
			{
				...
				try
				{
					...
					try
					{
						...
						Auction a = new Auction(player.getClan().getHasHideout(), player.getClan(), days
						        * 86400000L, bid, ClanHallManager.getInstance().getClanHallByOwner(player.getClan()).getName());
			...
			else if (actualCommand.equalsIgnoreCase("bidding"))
			{
				...
				try
				{
					...
					if (a != null)
					{
						...
						html.replace("%AGIT_AUCTION_END%", String.valueOf(format.format(a.getEndDate())));
						html.replace("%AGIT_AUCTION_REMAIN%", String.valueOf((a.getEndDate()
						        - System.currentTimeMillis()) / 3600000)
						        + " hours "
						        + String.valueOf((((a.getEndDate()
					...
					{
						_log.warning("Auctioneer Auction null for AuctionId : "
						        + auctionId);
					}
					player.sendPacket(html);
				}
			...
			else if (actualCommand.equalsIgnoreCase("bid1"))
			{
				...
				try
				{
					...
					html.replace("npc_%objectId%_bid", "npc_" + getObjectId()
					        + "_bid " + val);
					player.sendPacket(html);
					return;
				}
			...
			else if (actualCommand.equalsIgnoreCase("list"))
			{
				...
				{
					items += "<td><center><a action=\"bypass -h npc_"
					        + getObjectId() + "_list " + j + "\"> Page " + j
					        + " </a></center></td>";
				}
				...
				for (Auction a : auctions)
				{
					...
					items += "<tr>" + "<td>"
					        ...
					        + format.format(a.getEndDate()) + "</td>" + "<td>"
			...
			else if (actualCommand.equalsIgnoreCase("selectedItems"))
			{
				        ...
				        && player.getClan().getAuctionBiddedAt() > 0)
				{
					...
					{
						_log.warning("Auctioneer Auction null for AuctionBiddedAt : "
				        ...
				        && AuctionManager.getInstance().getAuction(player.getClan().getHasHideout()) != null)
				{
					...
					{
						_log.warning("Auctioneer Auction null for getHasHideout : "
				        ...
				        && player.getClan().getHasHideout() != 0)
				{
					...
					{
						_log.warning("Clan Hall ID NULL : " + ItemId
			...
			else if (actualCommand.equalsIgnoreCase("cancelBid"))
			{
				...
				html.replace("%AGIT_BID_REMAIN%", String.valueOf((int) (bid
				        * 0.9)));
				html.replace("%AGIT_LINK_BACK%", "bypass -h npc_"
				        + getObjectId() + "_selectedItems");
				html.replace("%objectId%", String.valueOf(getObjectId()));
			...
			else if (actualCommand.equalsIgnoreCase("rebid"))
			{
				...
				try
				{
					...
					if (a != null)
					{
						...
						html.replace("%AGIT_LINK_BACK%", "bypass -h npc_"
						        + getObjectId() + "_selectedItems");
						html.replace("npc_%objectId%_bid1", "npc_"
						        + getObjectId() + "_bid1 " + a.getId());
					}
					else
					{
