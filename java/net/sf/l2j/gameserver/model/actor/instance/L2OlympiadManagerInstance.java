...
public class L2OlympiadManagerInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
			...
			switch (val)
			{
				...
				case 3:
					...
					if (points >= 0)
					{
						reply = new NpcHtmlMessage(getObjectId());
						replyMSG = new StringBuilder("<html><body>");
						replyMSG.append("There are " + points
						        + " Grand Olympiad "
				...
				case 6:
					...
					else
					{
						...
						// TODO Send HTML packet "Saying not enough olympiad
						// points.
					}
					break;
				case 7:
					...
					break;
				default:
					_logOlymp.warning("Olympiad System: Couldnt send packet for request "
					        + val);
					break;
		...
		else if (command.startsWith("OlyBuff"))
		{
			...
			if (params[1] == null)
			{
				_log.warning("Olympiad Buffer Warning: npcId = " + npcId
			...
			{
				_log.warning("Olympiad Buffer Warning: npcId = " + npcId
				        + " Location: " + getX() + ", " + getY() + ", " + getZ()
				        + " Player: " + player.getName()
				        + " has tried to use skill group (" + buffGroup
			...
			if (player.olyBuff > 0)
			{
				html.setFile(Olympiad.OLYMPIAD_HTML_FILE
				        + "olympiad_buffs.htm");
			...
			switch (val)
			{
				case 1:
					...
					else
					{
						...
						{
							showbattle = Integer.parseInt(element.substring(1, 2));
							stad = Integer.parseInt(element.substring(4, 5));
							if (showbattle == 1)
							{
								replyMSG.append("<br><a action=\"bypass -h npc_"
								        + getObjectId() + "_Olympiad 3_" + stad
								        + "\">" + element + "</a>");
							}
				...
				case 2:
					        ...
					        || classId == 136)
					{
						...
						if (names.size() != 0)
						{
							...
							{
								replyMSG.append("<tr>");
								replyMSG.append("<td align=\"left\">" + index
								        + "</td>");
								replyMSG.append("<td align=\"right\">" + name
								        + "</td>");
								replyMSG.append("</tr>");
								index++;
							}
