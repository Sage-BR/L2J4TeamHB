...
public class L2SummonInstance extends L2Summon
{
	...
	public int lastShowntimeRemaining; // Following FbiAgent's example to avoid
	        ...
	        L2PcInstance owner, L2Skill skill)
	{
		...
		{
			_nextItemConsumeTime = _totalLifeTime
			        - _totalLifeTime / (_itemConsumeSteps + 1);
		}
	...
	static class SummonLifetime implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				{
					_summon.decNextItemConsumeTime(maxTime
					        / (_summon.getItemConsumeSteps() + 1));
				...
				// prevent useless packet-sending when the difference isn't
				// visible.
				if ((_summon.lastShowntimeRemaining
				        - newTimeRemaining) > maxTime / 352)
				{
			...
			catch (Throwable e)
			{
				if (Config.DEBUG)
				{
					log.warning("Summon of player [#" + _activeChar.getName()
					        + "] has encountered item consumption errors: "
					        + e);
				}
			}
		}
