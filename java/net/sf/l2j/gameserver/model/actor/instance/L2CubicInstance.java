...
public class L2CubicInstance
{
	...
	public L2CubicInstance(L2PcInstance owner, int id, int level, int mAtk,
	        int activationtime, int activationchance, boolean givenByOther)
	{
		_owner = owner;
		_id = id;
		_matk = mAtk;
	...
	public void getCubicTarget()
	{
		try
		{
			...
			if (TvTEvent.isStarted()
			        && TvTEvent.isPlayerParticipant(_owner.getObjectId()))
			{
				TvTEventTeam enemyTeam = TvTEvent.getParticipantEnemyTeam(_owner.getObjectId());

				if (_owner.getTarget() instanceof L2PcInstance)
				{
					if (enemyTeam.containsPlayer(_owner.getTargetId())
					        && !((L2PcInstance) _owner.getTarget()).isDead())
					{
				...
				for (L2PcInstance enemy : enemyTeam.getParticipatedPlayers().values())
				{
					...
					{
						if (isInCubicRange(_owner, enemy.getPet())
						        && !enemy.getPet().isDead())
						{
					...
					{
						if (isInCubicRange(_owner, enemy.getTrap())
						        && !enemy.getTrap().isDead())
						{
			...
			if (_owner.getTarget() != null)
			{
				if (_owner.getTarget() instanceof L2Character
				        && _owner.getTarget() != _owner.getPet()
				        && _owner.getTarget() != _owner)
				{
					// target mob which has aggro on you or your summon
					if (_owner.getTarget() instanceof L2Attackable)
					{
						if (((L2Attackable) _owner.getTarget()).getAggroListRP().get(_owner) != null
					        ...
					        || _owner.isInsideZone(L2Character.ZONE_PVP))
					{
						...
						if (enemy != null)
						{
							...
							if (enemy.getPvpFlag() == 0
							        && !enemy.isInsideZone(L2Character.ZONE_SIEGE)
							        && !enemy.isInsideZone(L2Character.ZONE_PVP))
							{
								targetIt = false;
							}
			...
			// target
			List<L2Character> potentialTarget = new ArrayList<>();
			List<L2Character> potentialPvPTarget = new ArrayList<>();
	...
	private class Action implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				if (_id >= SMART_CUBIC_EVATEMPLAR
				        && _id <= SMART_CUBIC_SPECTRALMASTER)
				{
					...
					if (skill != null)
					{
						...
						if ((_target != null) && (!_target.isDead()))
						{
							if (Config.DEBUG)
							{
								...
								_log.info("Cubic Id: " + _id + " Target: "
								        + _target.getName() + " distance: "
							...
							if (handler != null)
							{
								if ((type == L2Skill.SkillType.PARALYZE)
								        || (type == L2Skill.SkillType.STUN))
								{
									if (Config.DEBUG)
									{
										_log.info("L2CubicInstance: Action.run() handler "
										        + type);
									}
								...
								else if ((type == L2Skill.SkillType.POISON)
								        || (type == L2Skill.SkillType.DEBUFF)
								        || (type == L2Skill.SkillType.DOT))
								{
									if (Config.DEBUG)
									{
										_log.info("L2CubicInstance: Action.run() handler "
							...
							else
							{
								if (type == L2Skill.SkillType.DRAIN)
								{
									if (Config.DEBUG)
									{
										_log.info("L2CubicInstance: Action.run() skill "
										        + type);
									}
			...
			for (L2Character partyMember : partyList)
			{
				if (!partyMember.isDead())
				{
					...
					if (isInCubicRange(_owner, partyMember))
					{
						...
						{
							if (percentleft > (partyMember.getCurrentHp()
							        / partyMember.getMaxHp()))
							{
								percentleft = (partyMember.getCurrentHp()
								        / partyMember.getMaxHp());
								target = partyMember;
							}
						}
				...
				// heal and if he have the lowest HP
					...
					{
						if (percentleft > (partyMember.getPet().getCurrentHp()
						        / partyMember.getPet().getMaxHp()))
						{
							percentleft = (partyMember.getPet().getCurrentHp()
							        / partyMember.getPet().getMaxHp());
		...
		else
		{
			...
			if (_owner.getPet() != null)
			{
				if (!_owner.getPet().isDead()
				        ...
				        && percentleft > (_owner.getPet().getCurrentHp()
				                / _owner.getPet().getMaxHp())
		...
		public void run()
		{
			...
			try
			{
				...
				if (skill != null)
				{
					...
					{
						double percentleft = (target.getCurrentHp()
						        / target.getMaxHp()) * 100.0;
