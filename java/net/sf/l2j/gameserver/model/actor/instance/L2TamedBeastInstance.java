...
public final class L2TamedBeastInstance extends L2FeedableBeastInstance
{
	...
	public L2TamedBeastInstance(int objectId, L2NpcTemplate template,
	        L2PcInstance owner, int foodSkillId, int x, int y, int z)
	{
		super(objectId, template);
	...
	public void setOwner(L2PcInstance owner)
	{
		if (owner != null)
		{
			...
			// instead of calculating this value each time, let's get this now
			// and pass it on
	...
	public void onOwnerGotAttacked(L2Character attacker)
	{
		...
		if (HPRatio >= 0.8)
		{
			...
			for (L2Skill skill : skills.values())
			{
				...
				if ((skill.getSkillType() == L2Skill.SkillType.DEBUFF)
				        && Rnd.get(3) < 1 && (attacker != null
	...
	/**
	 * Prepare and cast a skill: First smoothly prepare the beast for casting,
	 * by abandoning other actions Next, call super.doCast(skill) in order to
	 * actually cast the spell Finally, return to auto-following the owner.
	 *
	...
	private class CheckDuration implements Runnable
	{
		...
		public void run()
		{
			...
			_tamedBeast.setRemainingTime(_tamedBeast.getRemainingTime()
			...
			/*
			 * There are too many conflicting reports about whether distance
			 * from home should be taken into consideration. Disabled for now.
			 *
	...
	private class CheckOwnerBuffs implements Runnable
	{
		...
		public void run()
		{
			...
			// if the owner is too far away, stop anything else and immediately
			// run towards the owner.
			...
			// if the owner is dead, do nothing...
			// if the tamed beast is currently casting a spell, do not interfere
			// (do not attempt to cast anything new yet).
			...
			// if the owner has less than 60% of this beast's available buff,
			// cast a random buff
