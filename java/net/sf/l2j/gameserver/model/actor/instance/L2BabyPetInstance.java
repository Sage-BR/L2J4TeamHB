...
public final class L2BabyPetInstance extends L2PetInstance
{
	...
	public L2BabyPetInstance(int objectId, L2NpcTemplate template,
	...
	private class Heal implements Runnable
	{
		...
		public void run()
		{
			...
			// if the pet is still casting from the previous iteration, allow
			...
			if (!owner.isDead() && !_baby.isCastingNow() && !_baby.isBetrayed())
			{
				...
				if ((owner.getCurrentHp() / owner.getMaxHp() < 0.2)
				        && Rnd.get(4) < 3)
				{
					_baby.useMagic(_strongHeal, false, false);
				}
				else if ((owner.getCurrentHp() / owner.getMaxHp() < 0.8)
				        && Rnd.get(4) < 1)
				{
