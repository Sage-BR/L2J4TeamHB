...
public class L2PetInstance extends L2Summon
{
	...
	public synchronized static L2PetInstance spawnPet(L2NpcTemplate template,
	...
	public void onAction(L2PcInstance player)
	{
		...
		boolean thisIsTarget = player.getTarget() != null
		        && player.getTarget().getObjectId() == getObjectId();
		...
		try
		{
			...
			for (int i = 0; (i < items.length); i++)
			{
				...
				if (((giveit.getItem().getWeight() * giveit.getCount())
				        + getOwner().getInventory().getTotalWeight()) < getOwner().getMaxLoad())
				{
...
//
	...
	public synchronized void startFeed(boolean battleFeed)
	{
		...
		if (!isDead())
		{
			...
			_feedTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new FeedTask(), 60000
			        / _feedTime, 60000 / _feedTime);
		}
	}
	...
	public synchronized void unSummon(L2PcInstance owner)
	{
		...
		// Always remove from L2World, even if dead — otherwise stale pet
		// references persist in _petsInstance and cause ghost pet windows on
		// next login.
		L2World.getInstance().removePet(owner.getObjectId());

		// Ensure owner's pet reference is cleared regardless of alive/dead
		// state.
		// L2Summon.unSummon() skips setPet(null) when isDead(), so handle it
		// here.
		if (owner.getPet() == this)
		{
			owner.setPet(null);
		}
	}
	...
	public void addExpAndSp(long addToExp, int addToSp)
	{
		if (getNpcId() == 12564)
		{ // SinEater
			getStat().addExpAndSp(Math.round(addToExp
						        * Config.SINEATER_XP_RATE), addToSp);
		}
		else
		{
			getStat().addExpAndSp(Math.round(addToExp
			        * Config.PET_XP_RATE), addToSp);
		}
	}
