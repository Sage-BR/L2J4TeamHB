...
public class L2NpcBufferInstance extends L2NpcInstance
{
	...
	public void showChatWindow(L2PcInstance playerInstance, int val)
	{
		...
		if (val > 0)
		{
			htmContent = HtmCache.getInstance().getHtm("data/html/mods/NpcBuffer-"
			        + val + ".htm");
		}
	...
	public void onBypassFeedback(L2PcInstance playerInstance, String command)
	{
		...
		else if (command.startsWith("Buff"))
		{
			...
			for (String buffGroupList : buffGroupArray)
			{
				...
				{
					_log.warning("NPC Buffer Warning: npcId = " + npcId
				...
				if (npcBuffGroupInfo == null)
				{
					_log.warning("NPC Buffer Warning: npcId = " + npcId
					        + " Location: " + getX() + ", " + getY() + ", "
					        + getZ() + " Player: " + playerInstance.getName()
				...
				if (skillFeeId != 0)
				{
					...
					if (itemInstance == null || (!itemInstance.isStackable()
					        && playerInstance.getInventory().getInventoryItemCount(skillFeeId, -1) < skillFeeAmount))
					{
		...
		else if (command.startsWith("Heal"))
		{
			if (!playerInstance.isInCombat()
			        && !AttackStanceTaskManager.getInstance().getAttackStanceTask(playerInstance))
			{
