...
public class L2NpcInstance extends L2Character
{
	...
	protected class RandomAnimationTask implements Runnable
	{
		...
		public void run()
		{
			try
			{
				if (this != _rAniTask)
				{
					return; // Shouldn't happen, but who knows... just to make
				}
					        // sure every active npc has only one timer.
				if (isMob())
				{
		...
		if (player.getAccessLevel().isGm())
		{
			...
			else
			{
				html1.append("Respawn Time: "
				        + (getSpawn() != null ? (getSpawn().getRespawnDelay()
				                / 1000) + "  Seconds<br>" : "?  Seconds<br>"));
			}
			...
			html1.append("<tr><td>Level</td><td>" + getLevel()
			        + "</td><td>Aggro</td><td>"
			        + ((this instanceof L2Attackable attackable) ? attackable.getAggroRange() : 0)
			        + "</td></tr>");
			...
			if (getTemplate().getDropData() != null)
			{
				for (L2DropCategory cat : getTemplate().getDropData())
				{
					for (L2DropData drop : cat.getAllDrops())
					{
						...
						{
							html1.append("<tr><td><font color=\"ff0000\">"
							        + name + "</font></td><td>"
						...
						{
							html1.append("<tr><td><font color=\"00ff00\">"
							        + name + "</font></td><td>"
							        + (drop.isQuestDrop() ? "Quest" : (cat.isSweep() ? "Sweep" : "Drop"))
							        + "</td></tr>");
						}
						...
						{
							html1.append("<tr><td><font color=\"0000ff\">"
							        + name + "</font></td><td>"
							        + (drop.isQuestDrop() ? "Quest" : (cat.isSweep() ? "Sweep" : "Drop"))
							        + "</td></tr>");
						}
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		// if (canInteract(player))
		{
			...
			else if (command.startsWith("EnterRift"))
			{
				try
				{
					Byte b1 = Byte.parseByte(command.substring(10)); // Selected
					                                                 // Area:
					                                                 // Recruit,
					                                                 // Soldier
					                                                 // etc
			...
			{
				if (player.isInParty()
				        && player.getParty().isInDimensionalRift())
				{
	 ...
	 * data/scripts/quests/questId/stateId.htm</li>
	 * <li>Send a Server->Client NpcHtmlMessage containing the text of the
	...
	public void showQuestWindow(L2PcInstance player, String questId)
	{
		...
		else
		{
			if ((q.getQuestIntId() >= 1 && q.getQuestIntId() < 1000)
			        && (player.getWeightPenalty() >= 3
			                || player.getInventoryLimit()
			                        * 0.8 <= player.getInventory().getSize()))
			{
			...
			if (qs == null)
			{
				if (q.getQuestIntId() >= 1 && q.getQuestIntId() < 1000)
				{
					...
					if (questList.length >= 25) // if too many ongoing quests,
					                            // don't show window and send
					                            // message
					{
		...
		if (qs != null)
		{
			...
			String path = "data/scripts/quests/" + questId + "/" + stateId
			        + ".htm";
	...
	public void showLotoWindow(L2PcInstance player, int val)
	{
		...
		html.replace("%enddate%", ""
		        + DateFormat.getDateInstance().format(Lottery.getInstance().getEndDate()));
		player.sendPacket(html);
	...
	public void makeSupportMagic(L2PcInstance player)
	{
		...
		// Calculate the min and max level between which the player must be to
		// obtain buff
	...
	{
		String html = HtmCache.getInstance().getHtm("data/html/" + type + "/"
		        + getNpcId() + "-pk.htm");
	...
	{
		if (player.isCursedWeaponEquipped()
		        && (!(player.getTarget() instanceof L2ClanHallManagerInstance)
		...
		switch (npcId)
		{
			...
			case 31111: // Gatekeeper Spirit (Disciples)
				if (playerCabal == sealAvariceOwner
				        && playerCabal == compWinner)
				{
			...
			case 36402:
				...
				{
					filename = Olympiad.OLYMPIAD_HTML_FILE
					        + "olympiad_buffs.htm";
				}
				else
				{
					filename = Olympiad.OLYMPIAD_HTML_FILE
					        + "olympiad_nobuffs.htm";
				}
				break;
			default:
	...
	public boolean rechargeAutoSoulShot(boolean physical, boolean magic)
	{
		...
		if (weaponItem == null)
		{
			// _log.warning("NpcId "+getNpcId()+" missing weaponItem definition
			// in DP - or wrong use of shots.");
			return false;
		}
		if (magic)
		{
			...
			if (null != _inventory.destroyItemByItemId("Consume", 3947, weaponItem.getSpiritShotCount(), null, null))
			{
				...
				broadcastPacket(new MagicSkillUse(this, this, 2061, 1, 0, 0), 360000); // no
				                                                                       // grade
				return true;
			}
			else
		...
		if (physical)
		{
			...
			if (null != _inventory.destroyItemByItemId("Consume", 1835, weaponItem.getSoulShotCount(), null, null))
			{
				...
				broadcastPacket(new MagicSkillUse(this, this, 2039, 1, 0, 0), 360000); // no
				                                                                       // grade
				return true;
			}
			else
	...
	public boolean isUsingShot(boolean physical)
	{
		...
		if ((physical && _inventory.sshotInUse) || (!physical && _inventory.bshotInUse))
		{
			return true;
		}
		return false;
