...
public class L2RaidBossInfoInstance extends L2NpcInstance
{
	...
	public void showChatWindow(L2PcInstance player, int val)
	{
		...
		if (val != 0)
		{
			name = "data/html/mods/raidbossinfo/" + getNpcId() + "-" + val
			        + ".htm";
		}
	...
	private void showRaidBossInfo(L2PcInstance player, int pageId)
	{
		...
		final int max = infos.size() / limit
		        + (infos.size() % limit == 0 ? 0 : 1);
		infos = infos.subList((pageId - 1)
		        * limit, Math.min(pageId * limit, infos.size()));
		...
		for (int bossId : infos)
		{
			...
			{
				sb.append("<tr>");
				sb.append("<td><a action=\"bypass -h npc_%objectId%_RaidBossDrop "
				        + bossId + "\">" + bossName + "</a></td>");
			...
			{
				sb.append("<tr>");
				sb.append("<td width=\"159\" align=\"left\"><a action=\"bypass -h npc_%objectId%_RaidBossDrop "
				        + bossId + "\">" + bossName + "</a></td>");
				sb.append("<td width=\"159\" align=\"left\"><font color=\"FB5858\">Dead</font> "
				        + new SimpleDateFormat(Config.RAID_BOSS_DATE_FORMAT).format(new Date(respawnTime))
				        + "</td>");
				sb.append("</tr>");
			}
		}
		...
		for (int x = 0; x < max; x++)
		{
			...
			{
				sb.append("<td align=\"center\"><a action=\"bypass -h npc_%objectId%_RaidBossInfo "
				        + pageNr + "\">" + pageNr + "</a></td>");
			}
		}
		sb.append("</tr>");
	...
	private void showRaidBossDrop(L2PcInstance player, int npcId, int page)
	{
		...
		sb.append("<br>");
		sb.append("<table width=\"256\">");
		sb.append("<tr><td width=\"256\" align=\"left\">"
		        + MESSAGE[1][Rnd.get(MESSAGE.length)].replace("%boss%", template.getName())
		        + "</td></tr>");
		sb.append("</table>");
		sb.append("<br>");
		...
		for (L2DropCategory cat : list)
		{
			...
			{
				final double chance = Math.min(100, (((drop.getItemId() == 57) ? drop.getChance()
				        * Config.RATE_DROP_ADENA : drop.getChance()
				...
				sb.append(((shown
				        % 2) == 0 ? "<table width=\"280\" bgcolor=\"000000\"><tr>" : "<table width=\"280\"><tr>"));
				sb.append("<td width=44 height=41 align=center><table bgcolor="
				        + (cat.isSweep() ? "FF00FF" : "FFFFFF")
				        + " cellpadding=6 cellspacing=\"-5\"><tr><td><button width=32 height=32 back="
				        + IconTable.getIcon(item.getItemId()) + " fore="
				        + IconTable.getIcon(item.getItemId())
				        + "></td></tr></table></td>");
				sb.append("<td width=240>"
				        + (cat.isSweep() ? ("<font color=ff00ff>" + name
				                + "</font>") : name)
				        + "<br1><font color=B09878>"
				        + (cat.isSweep() ? "Spoil" : "Drop") + " Chance : "
				        + percent + "%</font></td>");
		...
		if (page > 1)
		{
			StringUtil.append(sb, "<td align=left width=70><a action=\"bypass -h npc_%objectId%_RaidBossDrop "
			        + npcId + " ", (page - 1), "\">Previous</a></td>");
		}
		else
		{
		...
		if (page < shown)
		{
			StringUtil.append(sb, "<td align=right width=70>"
			        + (hasMore ? "<a action=\"bypass -h npc_%objectId%_RaidBossDrop "
			                + npcId + " " + (page + 1) + "\">Next</a>" : "")
			        + "</td>");
		}
		else
		...
		sb.append("<tr>");
		sb.append("<td width=\"160\" align=\"center\"><a action=\"bypass -h npc_%objectId%_RaidBossInfo "
		        + _lastPage.get(player.getObjectId()) + "\">Return</a></td>");
		sb.append("</tr>");
		sb.append("</table>");
		sb.append("</center>");
		sb.append("</body>");
