...
public class PcStatus extends PlayableStatus
{
	...
	public final void reduceHp(double value, L2Character attacker,
	        boolean awake)
	{
		if ((getActiveChar().isInvul() && getActiveChar() != attacker) || getActiveChar().isDead())
		{
			return;
		}
		...
		if (attacker instanceof L2PcInstance)
		{
			if (getActiveChar().isInDuel())
			{
				...
				// cancel duel if player got hit by another player, that is not
			// part of the duel
		...
		{
			// if attacked by a non L2PcInstance & non L2SummonInstance the duel
			// gets canceled
			if (getActiveChar().isInDuel()
			        && !(attacker instanceof L2SummonInstance))
			{
				getActiveChar().setDuelState(Duel.DUELSTATE_INTERRUPTED);
			}
		}
		...
		if (attacker != null && attacker != getActiveChar())
		{
			...
			{
				int tDmg = (int) value
				        * (int) getActiveChar().getStat().calcStat(Stats.TRANSFER_DAMAGE_PERCENT, 0, null, null)
				        / 100;
