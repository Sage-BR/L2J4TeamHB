...
public class TvTManager
{
	...
	public void scheduleEventStart()
	{
		_task = new TvTStartTask(System.currentTimeMillis()
		        + 60000L * Config.TVT_EVENT_INTERVAL);
	...
	public void startReg()
	{
		...
		{
			Announcements.getInstance().announceToAll("TvT Event: Registration opened for "
			        + Config.TVT_EVENT_PARTICIPATION_TIME + " minute(s).");

			// schedule registration end
			_task.setStartTime(System.currentTimeMillis()
			        + 60000L * Config.TVT_EVENT_PARTICIPATION_TIME);
			ThreadPoolManager.getInstance().executeTask(_task);
		}
	}
	...
	public void startEvent()
	{
		...
		else
		{
			TvTEvent.sysMsgToAllParticipants("TvT Event: Teleporting participants to an arena in "
			        ...
			        + " second(s).");
			_task.setStartTime(System.currentTimeMillis()
			        + 60000L * Config.TVT_EVENT_RUNNING_TIME);
			ThreadPoolManager.getInstance().executeTask(_task);
		}
	}
	...
	class TvTStartTask implements Runnable
	{
		...
		public void run()
		{
			int delay = (int) Math.round((_startTime
			...
			if (delay > 0)
			{
				ThreadPoolManager.getInstance().scheduleGeneral(this, nextMsg
				        * 1000);
			}
		}
		...
		private void announce(long time)
		{
			if (time >= 3600 && time % 3600 == 0)
			{
				...
				{
					Announcements.getInstance().announceToAll("TvT Event: "
					        + (time / 60 / 60)
					        + " hour(s) until registration is closed!");
				}
				else if (TvTEvent.isStarted())
				{
					TvTEvent.sysMsgToAllParticipants("TvT Event: "
					        + (time / 60 / 60)
			...
			else if (time >= 60)
			{
				if (TvTEvent.isParticipating())
				{
					Announcements.getInstance().announceToAll("TvT Event: "
					        + (time / 60)
					        + " minute(s) until registration is closed!");
				}
				else if (TvTEvent.isStarted())
				{
					TvTEvent.sysMsgToAllParticipants("TvT Event: " + (time / 60)
			...
			else
			{
				if (TvTEvent.isParticipating())
				{
					Announcements.getInstance().announceToAll("TvT Event: "
					        + time
					        + " second(s) until registration is closed!");
				}
				else if (TvTEvent.isStarted())
				{
					TvTEvent.sysMsgToAllParticipants("TvT Event: " + time
					        + " second(s) until the event is finished!");
				}
			}
		}
	}
