...
public class FortSiege
{
			...
			try
			{
				...
				else if ((timeRemaining <= 3600000) && (timeRemaining > 600000))
				{
					announceToPlayer(Math.round(timeRemaining / 60000)
					        + " minute(s) until " + getFort().getName()
			...
			try
			{
				...
				else if ((timeRemaining <= 600000) && (timeRemaining > 300000))
				{
					announceToPlayer("The registration term for "
					        + getFort().getName() + " has ended.", false);
	...
	public void endSiege()
	{
		if (getIsInProgress())
		{
			announceToPlayer("The siege of " + getFort().getName()
			        + " has finished!", false);
	...
	public void startAutoTask(boolean setTime)
	{
		...
		System.out.println("Siege of " + getFort().getName() + ": "
		        + getFort().getSiegeDate().getTime());
		setIsScheduled(true);
		loadSiegeClan();
	...
	private void loadSiegeClan()
	{
		...
		try
		{
			...
			// Add fort owner as defender (add owner first so that they are on
		...
		try
		{
			if (typeId == 0 || typeId == 2 || typeId == -1)
			{
				if (getDefenderClans().size()
				        + getDefenderWaitingClans().size() >= FortSiegeManager.getInstance().getDefenderMaxClans())
				{
					return;
				}
			}
	...
	private void spawnCommander(int Id)
	{
		...
		for (SiegeSpawn _sp : FortSiegeManager.getInstance().getCommanderSpawnList(Id))
		{
			...
			commander.spawnMe(_sp.getLocation().getX(), _sp.getLocation().getY(), _sp.getLocation().getZ()
			        + 50);
...
//        }
	...
	{
		return (FortSiegeManager.getInstance().getDefenderRespawnDelay()
