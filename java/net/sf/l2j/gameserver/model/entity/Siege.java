...
public class Siege
{
			...
			try
			{
				...
				if (timeRemaining > 3600000)
				{
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_castleInst), timeRemaining
					        - 3600000); // Prepare task for 1 hr left.
				}
				else if ((timeRemaining <= 3600000) && (timeRemaining > 600000))
				{
					...
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_castleInst), timeRemaining
					        - 600000); // Prepare task for 10 minute left.
				}
				else if ((timeRemaining <= 600000) && (timeRemaining > 300000))
				{
					announceToPlayer(Math.round(timeRemaining / 60000)
					        ...
					        + " siege conclusion.", true);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_castleInst), timeRemaining
					        - 300000); // Prepare task for 5 minute left.
				}
				else if ((timeRemaining <= 300000) && (timeRemaining > 10000))
				{
					announceToPlayer(Math.round(timeRemaining / 60000)
					        ...
					        + " siege conclusion.", true);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_castleInst), timeRemaining
					        - 10000); // Prepare task for 10 seconds count down
				}
				else if ((timeRemaining <= 10000) && (timeRemaining > 0))
				{
					announceToPlayer(getCastle().getName() + " siege "
					        ...
					        + " second(s) left!", true);
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(_castleInst), timeRemaining); // Prepare
					                                                                                                       // task
					                                                                                                       // for
					                                                                                                       // second
					                                                                                                       // count
			...
			try
			{
				...
				if (timeRemaining > 86400000)
				{
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(_castleInst), timeRemaining
					        - 86400000); // Prepare task for 24 before siege
					                     // start to end registration
				}
				        ...
				        && (timeRemaining > 13600000))
				{
					...
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(_castleInst), timeRemaining
					        - 13600000); // Prepare task for 1 hr left before
					                     // siege start.
				}
				...
				else if ((timeRemaining <= 10000) && (timeRemaining > 0))
				{
					...
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(_castleInst), timeRemaining); // Prepare
					                                                                                                         // task
					                                                                                                         // for
					                                                                                                         // second
					                                                                                                         // count
	...
	public void endSiege()
	{
		if (getIsInProgress())
		{
			...
			removeFlags(); // Removes all flags. Note: Remove flag before
			               // teleporting players
			teleportPlayer(Siege.TeleportWhoType.Attacker, MapRegionTable.TeleportWhereType.Town); // Teleport
			                                                                                       // to
			                                                                                       // the
			                                                                                       // second
			                                                                                       // closest
			                                                                                       // town
			teleportPlayer(Siege.TeleportWhoType.DefenderNotOwner, MapRegionTable.TeleportWhereType.Town); // Teleport
			                                                                                               // to
			                                                                                               // the
			                                                                                               // second
			                                                                                               // closest
			                                                                                               // town
			teleportPlayer(Siege.TeleportWhoType.Spectator, MapRegionTable.TeleportWhereType.Town); // Teleport
			                                                                                        // to
			                                                                                        // the
			                                                                                        // second
			                                                                                        // closest
	...
	public void midVictory()
	{
		if (getIsInProgress()) // Siege still in progress
		{
			...
			if (getCastle().getOwnerId() > 0)
			{
				...
				teleportPlayer(Siege.TeleportWhoType.Attacker, MapRegionTable.TeleportWhereType.SiegeFlag); // Teleport
				                                                                                            // to
				                                                                                            // the
				                                                                                            // second
				                                                                                            // closest
				...
				getCastle().spawnDoor(true); // Respawn door to castle but make
				                             // them weaker (50% hp)
				removeControlTower(); // Remove all control tower from this
				                      // castle
				_controlTowerCount = 0;// Each new siege midvictory CT are
				                       // completely respawned.
	...
	public void startSiege()
	{
		if (!getIsInProgress())
		{
			...
			MercTicketManager.getInstance().deleteTickets(getCastle().getCastleId()); // remove
			                                                                          // the
			                                                                          // tickets
			                                                                          // from
			                                                                          // the
			...
			ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleEndSiegeTask(getCastle()), 1000); // Prepare
			                                                                                              // auto
			                                                                                              // end
			                                                                                              // task
	...
	{
		return (getIsInProgress() && (getCastle().checkIfInZone(x, y, z))); // Castle
		                                                                    // zone
		                                                                    // during
		                                                                    // siege
	}
	...
	public void clearSiegeWaitingClan()
	{
		...
		{
			_log.warning("Exception: clearSiegeWaitingClan(): "
			        + e.getMessage());
			e.printStackTrace();
		}
		finally
	...
	/**
	 * Return list of L2PcInstance registered as defender but not owner in the
	 * zone.
	 */
	...
	public void killedCT(L2NpcInstance ct)
	{
		...
		if (_controlTowerMaxCount > 0)
		{
			_defenderRespawnDelayPenalty = (_controlTowerMaxCount
			        - _controlTowerCount) / _controlTowerCount
			        * SiegeManager.getInstance().getControlTowerLosePenalty(); // Add
			                                                                   // respawn
			                                                                   // penalty
			                                                                   // to
			                                                                   // defenders
		...
		try
		{
			if (typeId == 0 || typeId == 2 || typeId == -1)
			{
				if (getDefenderClans().size()
				        + getDefenderWaitingClans().size() >= SiegeManager.getInstance().getDefenderMaxClans())
				{
					return;
				}
			}
			...
			if (typeId == 0 || typeId == -1)
			{
				...
				announceToPlayer(clan.getName()
				        + " has been registered to defend "
	...
	private void spawnControlTower(int Id)
	{
		...
		for (SiegeSpawn _sp : SiegeManager.getInstance().getControlTowerSpawnList(Id))
		{
			...
			ct.spawnMe(_sp.getLocation().getX(), _sp.getLocation().getY(), _sp.getLocation().getZ()
			        + 20);
			_controlTowerCount++;
