...
public abstract class Inventory extends ItemContainer
{
		...
		public void notifyEquiped(int slot, L2ItemInstance item)
		{
			...
			if (armorSet.containItem(slot, item.getItemId()))
			{
				if (armorSet.containAll(player))
				{
					...
					{
						_log.warning("Inventory.ArmorSetListener: Incorrect skill: "
						        + armorSet.getSkillId() + ".");
					}
	 ...
	 */
	public synchronized L2ItemInstance[] unEquipItemInBodySlotAndRecord(
	        int slot)
	{
	...
	private void unEquipItemInBodySlot(int slot)
	{
		...
		switch (slot)
		{
			...
			case L2Item.SLOT_HAIRALL:
				...
				setPaperdollItem(PAPERDOLL_HAIR2, null);// this should be the
				                                        // same as in DHAIR
	...
	public synchronized void equipItem(L2ItemInstance item)
	{
		...
		if (getOwner() instanceof L2PcInstance)
		{
			...
			if (player.getPkKills() > 0 && item.getItemId() >= 7816
			        && item.getItemId() <= 7831)
			{
	...
	public void restore()
	{
		...
		try
		{
			...
			while (inv.next())
			{
				...
				{
					_log.log(Level.WARNING, "Item: " + item.getObjectId()
					        + " Has Duplied on World And Cannot be Load");
