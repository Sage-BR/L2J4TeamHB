...
public class L2BossZone extends L2ZoneType
{
	...
	private boolean _enabled = true; // default value, unless overridden by
	                                 // xml...
	...
	public L2BossZone(int id)
	{
		...
		_playersAllowed = new L2ArrayList<>();
		_oustLoc = new int[3];
	}
	 ...
	 * currentTime - _timeInvade) AND the player was in the zone prior to
	 * reboot.
	 */
		...
		if (_enabled)
		{
			if (character instanceof L2PcInstance)
			{
				...
				if (player.isOnline() == 0
				        && _playersAllowed.contains(character.getObjectId()))
				{
	...
	/**
	 * This function is to be used by external sources, such as quests and AI in
	 * order to allow a player for entry into the zone for some time. Naturally
	...
	public void allowPlayerEntry(L2PcInstance player, int durationInSec)
	{
		if (!player.isGM())
		{
			...
			_playerAllowedReEntryTimes.put(player.getObjectId(), System.currentTimeMillis()
			        + durationInSec * 1000);
