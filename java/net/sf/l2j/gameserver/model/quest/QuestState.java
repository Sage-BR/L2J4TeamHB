...
public final class QuestState
{
	 ...
	 * <LI>Add/Update couple (var,val) in class variable ConcurrentHashMap
	 * "vars"</LI>
	 * <LI>If the key represented by "var" exists in ConcurrentHashMap "vars",
	...
	public String set(String var, String val)
	{
		...
		// ConcurrentHashMap.put() returns previous value associated with
	// specified key, or null if there was no mapping for key.
		...
		if (var == "cond")
		{
			...
			{
				_log.finer(getPlayer().getName() + ", " + getQuestName()
				        + " cond [" + val
	 ...
	 *
	 * @param cond
	 *            : int indicating the step number for the current quest
	...
	private void setCond(int cond, int old)
	{
		...
		{
			// if this is a push back to a previous step, clear all completion
			// flags ahead
			if (cond < old)
			{
	 ...
	 * "var" from the class variable ConcurrentHashMap "vars" and from the
	 * database.
	 *
	 * @param var
...
//		exitQuest(true);
	...
	public void giveItems(int itemId, int count, int enchantlevel)
	{
		...
		// reward
		if (itemId == 57 && !(questId >= 217 && questId <= 233)
		        && !(questId >= 401 && questId <= 418))
		{
	        ...
	        int neededCount, int dropChance, boolean sound)
	{
		dropChance *= Config.RATE_DROP_QUEST
		        / ((getPlayer().getParty() != null) ? getPlayer().getParty().getMemberCount() : 1);
		...
		if (itemCount > 0)
		{
			...
			if (sound)
			{
				playSound((currentCount
				        + itemCount < neededCount) ? "Itemsound.quest_itemget" : "Itemsound.quest_middle");
			}
		}
...
// END STUFF THAT WILL PROBABLY BE CHANGED
	...
	{
		getPlayer().addExpAndSp((int) getPlayer().calcStat(Stats.EXPSP_RATE, exp
		        * Config.RATE_QUESTS_REWARD, null, null), (int) getPlayer().calcStat(Stats.EXPSP_RATE, sp
	...
	{
		String text = HtmCache.getInstance().getHtm("data/scripts/quests/255_Tutorial/"
		        + html);
		if (text == null)
		{
			_log.warning("missing html page data/scripts/quests/255_Tutorial/"
			        + html);
			text = "<html><body>File data/scripts/quests/255_Tutorial/" + html
	...
	public void dropItem(L2MonsterInstance npc, L2PcInstance player, int itemId,
	        int count)
	{
