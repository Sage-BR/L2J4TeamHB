...
public class CursedWeapon
{
	...
	// this should be false unless if the cursed weapon is dropped, in that case
	// it would be true.
	private boolean _isDropped = false;

	// this sets the cursed weapon status to true only if a player has the
	// cursed weapon, otherwise this should be false.
	...
	public void endOfLife()
	{
		...
		{
			// either this cursed weapon is in the inventory of someone who has
			// another cursed weapon equipped,
			// OR this cursed weapon is on the ground.
			if ((_player != null)
			        && (_player.getInventory().getItemByItemId(_itemId) != null))
			{
				// Destroy
				L2ItemInstance removedItem = _player.getInventory().destroyItemByItemId("", _itemId, 1, _player, null);
				if (!Config.FORCE_INVENTORY_UPDATE)
	        ...
	        L2Character killer, boolean fromMonster)
	{
		...
		{
			sm.addZoneName(killer.getX(), killer.getY(), killer.getZ()); // Region
		}
			                                                             // Name
		sm.addItemName(_itemId);
	...
	public void giveSkill()
	{
		...
		if (Config.DEBUG)
		{
			_log.info("Player " + _player.getName()
	...
	public void reActivate()
	{
		...
		{
			_removeTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new RemoveTask(), _durationLost
			        * 12000L, _durationLost * 12000L);
		}
	...
	{
		// if the player is mounted, attempt to unmount first. Only allow
		// picking up
	...
	public void goTo(L2PcInstance player)
	{
		...
		if (_isActivated)
		{
			...
			player.teleToLocation(_player.getX(), _player.getY(), _player.getZ()
			        + 20, true);
		}
		else if (_isDropped)
		{
			// Go to item on the ground
			player.teleToLocation(_item.getX(), _item.getY(), _item.getZ()
			        + 20, true);
		}
		else
		{
