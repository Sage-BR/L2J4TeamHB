...
public abstract class L2Skill
{
	...
	private final int _baseCritRate; // percent of success for skill critical
	                                 ...
	                                 // they're not affected by rCrit values or
	                                 // buffs). Default loads -1 for all other
	                                 // skills but 0 to PDAM & BLOW
	...
	private final int _lethalEffect2; // percent of success for lethal 2nd
	                                  // effect (hit cp,hp to 1 or if mob hp to
	                                  // 1) (only for PDAM skills)
	...
	private final float _sSBoost; // If true skill will have SoulShot boost
	                              // (power*2)
	...
	protected L2Skill(StatsSet set)
	{
		...
		if (Config.ENABLE_MODIFY_SKILL_REUSE
		        && Config.SKILL_REUSE_LIST.containsKey(_id))
		{
			if (Config.DEBUG)
			{
				_log.info("*** Skill " + _name + " (" + _level
				        + ") changed reuse from "
				        + set.getInteger("reuseDelay", 0) + " to "
				        + Config.SKILL_REUSE_LIST.get(_id) + " seconds.");
			}
		...
		_baseCritRate = set.getInteger("baseCritRate", (_skillType == SkillType.PDAM
		        || _skillType == SkillType.BLOW) ? 0 : -1);
	...
	public final double getPower(L2Character activeChar)
	{
		...
		{
			return _power * Math.pow(1.7165
			        - activeChar.getCurrentHp() / activeChar.getMaxHp(), 2)
			        * 0.577;
		}
		else
		{
		...
		switch (targetType)
		{
			...
			case TARGET_ONE:
			{
				...
				if (target == null || target.isDead()
				        || (target == activeChar && !canTargetSelf))
				{
					...
					for (L2Object obj : objs)
					{
						        ...
						        || obj instanceof L2PlayableInstance)
						{
							...
							if (src != null)
							{
								...
								{
									if (!src.checkPvpSkill(obj, this) || ((src.getParty() != null
									...
									if (!srcInArena
									        && !(((L2Character) obj).isInsideZone(L2Character.ZONE_PVP)
				...
				// synchronized (activeChar.getKnownList().getKnownObjects())
				{
					for (L2Object obj : objs)
					{
						        ...
						        || obj instanceof L2PlayableInstance)
						{
							...
							if (src != null)
							{
								if (!((L2Character) obj).isInFrontOf(activeChar) || !GeoData.getInstance().canSeeTarget(activeChar, obj))
								{
									continue;
								}
				...
				// synchronized (activeChar.getKnownList().getKnownObjects())
				{
					for (L2Object obj : objs)
					{
						        ...
						        || obj instanceof L2PlayableInstance)
						{
							...
							if (src != null)
							{
								if (!((L2Character) obj).isBehind(activeChar) || !GeoData.getInstance().canSeeTarget(activeChar, obj))
								{
									continue;
								}
			...
			case TARGET_AREA:
			{
				if ((!(target instanceof L2Attackable
				        || target instanceof L2PlayableInstance)) || // Target
				                                                     // is
				                                                     // not
				                                                     // L2Attackable
				                                                     // or
				...
				// synchronized (activeChar.getKnownList().getKnownObjects())
				{
					...
					{
						if (!(obj instanceof L2Attackable
						        || obj instanceof L2PlayableInstance) || (obj == cha))
						{
							continue;
						}
						...
						if (!target.isDead() && (target != activeChar))
						{
							                 ...
							                 // and exists
							{
								...
								if (obj instanceof L2PcInstance)
								{
									...
									if (!srcInArena
									        && !(trg.isInsideZone(L2Character.ZONE_PVP)
			...
			case TARGET_FRONT_AREA:
			{
				...
				if (getCastRange() >= 0)
				{
					...
					if (!onlyFirst)
					{
						targetList.add(cha); // Add target to target
						                     // list
					}
					else
					{
				...
				// synchronized (activeChar.getKnownList().getKnownObjects())
				{
					for (L2Object obj : objs)
					{
						if ((obj == cha) || !(obj instanceof L2Attackable
						        || obj instanceof L2PlayableInstance))
						{
							continue;
						}
						...
						if (!target.isDead() && (target != activeChar))
						{
							if (!Util.checkIfInRange(radius, obj, activeChar, true) || !((L2Character) obj).isInFrontOf(activeChar) || !GeoData.getInstance().canSeeTarget(activeChar, obj))
							{
								continue;
							}
			...
			case TARGET_BEHIND_AREA:
			{
				...
				// synchronized (activeChar.getKnownList().getKnownObjects())
				{
					for (L2Object obj : objs)
					{
						...
						if (!target.isDead() && (target != activeChar))
						{
							if (!Util.checkIfInRange(radius, obj, activeChar, true) || !((L2Character) obj).isBehind(activeChar) || !GeoData.getInstance().canSeeTarget(activeChar, obj))
							{
								continue;
							}
			...
			case TARGET_PARTY:
			{
				...
				{
					// Get all visible objects in a spherical area near the
					// L2Character
				        ...
				        && activeChar.getParty().getPartyLeaderOID() == target.getParty().getPartyLeaderOID())
				{
					if (!target.isDead())
					{
						if (target instanceof L2PcInstance)
						{
							...
							switch (getId())
							{
								// FORCE BUFFS may cancel here but there should
								// be a proper condition
								case 426:
			...
			case TARGET_ALLY:
			{
				if (activeChar instanceof L2PcInstance)
				{
					...
					if (clan != null)
					{
						...
						Collection<L2Object> objs = activeChar.getKnownList().getKnownObjects().values();
						// synchronized
						// (activeChar.getKnownList().getKnownObjects())
						{
			...
			case TARGET_CLAN:
			{
				...
				{
					// for buff purposes, returns one unbuffed friendly mob
					// nearby or mob itself?
					...
					case SUMMON:
					{
						if (DecayTaskManager.getInstance().getTasks().containsKey(target)
						        && (System.currentTimeMillis()
						                - DecayTaskManager.getInstance().getTasks().get(target)) > DecayTaskManager.ATTACKABLE_DECAY_TIME
						                        / 2)
						{
				...
				// synchronized (activeChar.getKnownList().getKnownObjects())
				{
					for (L2Object obj : objs)
					{
						if (!(obj instanceof L2Attackable
						        || obj instanceof L2PlayableInstance)
						        || ((L2Character) obj).isDead()
						        || ((L2Character) obj) == activeChar || !Util.checkIfInRange(radius, target, obj, true))
						{
							continue;
						}
						...
						if (obj instanceof L2PcInstance && src != null)
						{
							...
							if (((src.getParty() != null
							        && trg.getParty() != null)
							        && src.getParty().getPartyLeaderOID() == trg.getParty().getPartyLeaderOID()) || trg.isInsideZone(L2Character.ZONE_PEACE))
							{
								continue;
							}
			...
			case TARGET_ENEMY_SUMMON:
			{
				if (target instanceof L2Summon)
				{
					...
					{
						if ((activePlayer.getPet() != targetSummon
						        && !targetSummon.isDead()
	        ...
	        L2Character effected)
	{
		...
		if (isPassive() || (_effectTemplates == null) || effected instanceof L2DoorInstance
		        || effected instanceof L2SiegeFlagInstance)
		{
	...
	public final L2Effect[] getEffects(L2CubicInstance effector,
	        L2Character effected)
	{
		if (isPassive() || (_effectTemplates == null) || ((!effector.equals(effected)) && effected.isInvul()))
		{
			return _emptyEffectSet;
		}
