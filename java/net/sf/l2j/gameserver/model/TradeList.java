...
public class TradeList
{
		...
		if (item.isStackable())
		{
			for (TradeItem exclItem : _items)
			{
				if (exclItem.getItem().getItemId() == item.getItemId())
				{
					...
					{
						return new TradeItem(item, item.getCount()
						        - exclItem.getCount(), item.getReferencePrice());
					}
				}
			}
		}
	...
	public synchronized TradeItem addItem(int objectId, int count, int price)
	{
		...
		if (!item.isTradeable() || item.getItemType() == L2EtcItemType.QUEST || (count > item.getCount()))
		{
			return null;
		}
	...
	private boolean validate()
	{
		...
		if (_owner == null
		        || L2World.getInstance().findObject(_owner.getObjectId()) == null)
		{
