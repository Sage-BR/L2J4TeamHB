...
{
	private static final double[] BONUS_EXP_SP = { 1, 1.30, 1.39, 1.50, 1.54,
	        1.58, 1.63, 1.67, 1.71 };
	...
	public void removePartyMember(L2PcInstance player)
	{
		if (getPartyMembers().contains(player))
		{
			...
			try
			{
				...
				for (L2Character character : player.getKnownList().getKnownCharacters())
				{
					if (character.getForceBuff() != null
					        && character.getForceBuff().getTarget() == player)
					{
						character.abortCast();
					}
				}
			...
			if (getPartyMembers().size() == 1)
			{
				if (isInCommandChannel())
				{
					// delete the whole Commandchannel when the party who opened
					// the channel is disbanded
		...
		if (player != null && !player.isInDuel())
		{
			if (getPartyMembers().contains(player))
			{
				...
				else
				{
					...
					if (isInCommandChannel()
					        && temp.equals(_commandChannel.getChannelLeader()))
					{
	 ...
	 * SystemMessage(SystemMessageId.PARTY_DISPERSED); for(int i = 0; i <
	 ...
	 * <BR>
	 * Exception are L2PetInstances that leech from the owner's XP; they get the
	 * exp indirectly, via the owner's exp gain<BR>
	 *
	 * @param xpReward
	...
	public void distributeXpAndSp(long xpReward, int spReward,
	        List<L2PlayableInstance> rewardedMembers, int topLvl,
		...
		synchronized (rewardedMembers)
		{
			for (L2Character member : rewardedMembers)
			{
				...
				if (validMembers.contains(member))
				{
					...
					if (!member.isDead())
					{
						long addexp = Math.round(member.calcStat(Stats.EXPSP_RATE, xpReward
						        * preCalculation, null, null));
						int addsp = (int) member.calcStat(Stats.EXPSP_RATE, spReward
						        * preCalculation, null, null);
						if (target != null
						        && member instanceof L2PcInstance player)
						{
	...
	private List<L2PlayableInstance> getValidMembers(
	        List<L2PlayableInstance> members, int topLvl)
	{
...
//		Automatic cutoff method
		else if (Config.PARTY_XP_CUTOFF_METHOD.equalsIgnoreCase("auto"))
		{
			...
			{
				int sqLevel = member.getLevel() * member.getLevel();
				if (sqLevel >= sqLevelSum
				        * (1 - 1 / (1 + BONUS_EXP_SP[i] - BONUS_EXP_SP[i - 1])))
				{
					validMembers.add(member);
				}
			}
	 ...
	 * player join party or player decline to join
	 */
	public void setPendingInvitation(int val)
	{
		_pendingInvitation = val;
		_pendingInviteTimeout = GameTimeController.getGameTicks()
		        + L2PcInstance.REQUEST_TIMEOUT
	 ...
	 * @see l2jserver.gameserver.model.actor.instance.L2PcInstance#isRequestExpired()
	 */
