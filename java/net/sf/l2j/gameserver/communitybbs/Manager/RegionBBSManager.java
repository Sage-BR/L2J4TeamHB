...
public class RegionBBSManager extends BaseBBSManager
{
	...
	private void showOldCommunityPI(L2PcInstance activeChar, String name)
	{
		...
		if (player != null)
		{
			        ...
			        || Config.SHOW_LEVEL_COMMUNITYBOARD))
			{
				...
				htmlCode.append("<tr><td>Level: " + player.getLevel()
				        + "</td></tr>");
		...
		{
			ShowBoard sb = new ShowBoard("<html><body><br><br><center>No player with name "
			        + name + "</center><br><br></body></html>", "101");
			activeChar.sendPacket(sb);
			activeChar.sendPacket(new ShowBoard(null, "102"));
			activeChar.sendPacket(new ShowBoard(null, "103"));
		}
			...
			try
			{
				        ...
				        && !BlockList.isBlocked(receiver, activeChar))
				{
					if (!receiver.getMessageRefusal())
					{
						...
						activeChar.sendPacket(new CreatureSay(activeChar.getObjectId(), Say2.TELL, "->"
					...
					else
					{
						...
						parsecmd("_bbsloc;playerinfo;"
						        + receiver.getName(), activeChar);
					}
				}
				else
				{
	...
	private static ConcurrentHashMap<Integer, ArrayList<L2PcInstance>> _onlinePlayers = new ConcurrentHashMap<>();

	private static ConcurrentHashMap<Integer, ConcurrentHashMap<String, String>> _communityPages = new ConcurrentHashMap<>();
	...
	private void writeCommunityPages()
	{
		...
		{
			ConcurrentHashMap<String, String> communityPage = new ConcurrentHashMap<>();
			StringBuilder htmlCode = new StringBuilder("<html><body><br>");
			String tdClose = "</td>";
			...
			htmlCode.append("<td align=left valign=top>Server Restarted: "
			...
			htmlCode.append(tdOpen + "Party XP Rate: x"
			        + Config.RATE_XP * Config.RATE_PARTY_XP + tdClose);
			htmlCode.append(colSpacer);
			htmlCode.append(tdOpen + "XP Exponent: "
			        + Config.ALT_GAME_EXPONENT_XP + tdClose);
			htmlCode.append(trClose);

			htmlCode.append(trOpen);
			htmlCode.append(tdOpen + "SP Rate: x" + Config.RATE_SP + tdClose);
			htmlCode.append(colSpacer);
			htmlCode.append(tdOpen + "Party SP Rate: x"
			        + Config.RATE_SP * Config.RATE_PARTY_SP + tdClose);
			htmlCode.append(colSpacer);
			htmlCode.append(tdOpen + "SP Exponent: "
			        + Config.ALT_GAME_EXPONENT_SP + tdClose);
			htmlCode.append(trClose);

			htmlCode.append(trOpen);
			htmlCode.append(tdOpen + "Drop Rate: " + Config.RATE_DROP_ITEMS
			        + tdClose);
			htmlCode.append(colSpacer);
			htmlCode.append(tdOpen + "Spoil Rate: " + Config.RATE_DROP_SPOIL
			        + tdClose);
			htmlCode.append(colSpacer);
			htmlCode.append(tdOpen + "Adena Rate: " + Config.RATE_DROP_ADENA
			        + tdClose);
			htmlCode.append(trClose);
			...
			htmlCode.append(tdOpen
			        + L2World.getInstance().getAllVisibleObjectsCount()
			        + " Object count</td>");
			htmlCode.append(trClose);

			htmlCode.append(trOpen);
			htmlCode.append(tdOpen + getOnlineCount("gm")
			        + " Player(s) Online</td>");
			htmlCode.append(trClose);
			htmlCode.append("</table>");
			...
			if (getOnlineCount("gm") > Config.NAME_PAGE_SIZE_COMMUNITYBOARD)
			{
				...
				htmlCode.append("<td align=center valign=top width=200>Displaying "
				        + (((page - 1) * Config.NAME_PAGE_SIZE_COMMUNITYBOARD)
				                + 1)
				        + " - "
				        + (((page - 1) * Config.NAME_PAGE_SIZE_COMMUNITYBOARD)
				                + getOnlinePlayers(page).size())
				        + " player(s)</td>");
				htmlCode.append("<td FIXWIDTH=10></td>");
				if (getOnlineCount("gm") <= (page
			...
			htmlCode.append(colSpacer);
			htmlCode.append(tdOpen + "Party XP Rate: " + Config.RATE_PARTY_XP
			        + tdClose);
			htmlCode.append(colSpacer);
			htmlCode.append(tdOpen + "XP Exponent: "
			        + Config.ALT_GAME_EXPONENT_XP + tdClose);
			...
			htmlCode.append(colSpacer);
			htmlCode.append(tdOpen + "Party SP Rate: " + Config.RATE_PARTY_SP
			        + tdClose);
			htmlCode.append(colSpacer);
			htmlCode.append(tdOpen + "SP Exponent: "
			        + Config.ALT_GAME_EXPONENT_SP + tdClose);
			...
			htmlCode.append(trOpen);
			htmlCode.append(tdOpen + getOnlineCount("pl")
			        + " Player(s) Online</td>");
			htmlCode.append(trClose);
			htmlCode.append("</table>");
			...
			if (Config.BBS_SHOW_PLAYERLIST)
			{
				...
				{
					if ((player == null)
					        || (player.getAppearance().getInvisible()))
					{
						continue; // Go to next
					}
