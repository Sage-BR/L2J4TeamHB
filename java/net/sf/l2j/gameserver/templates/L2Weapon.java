...
public final class L2Weapon extends L2Item
{
	...
	private L2Skill _enchant4Skill = null; // skill that activates when item is
	        ...
	        boolean crit)
	{
		...
		if (!_skillsOnCritCondition.test(env) || !Formulas.getInstance().calcSkillSuccess(caster, target, _skillsOnCrit, false, false, false))
		{
	...
	public L2Effect[] getSkillEffects(L2Character caster, L2Character target,
	        L2Skill trigger)
	{
		if ((_skillsOnCast == null) || (trigger.isOffensive() != _skillsOnCast.isOffensive()) || (trigger.isToggle()
		        && _skillsOnCast.getSkillType() == SkillType.BUFF))
		{
		...
		if (!_skillsOnCastCondition.test(env) || (_skillsOnCast.isOffensive()
		        && !Formulas.getInstance().calcSkillSuccess(caster, target, _skillsOnCast, false, false, false)))
		{
			return _emptyEffectSet;
		}
