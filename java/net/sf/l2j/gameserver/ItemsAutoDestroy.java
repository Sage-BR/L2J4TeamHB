...
public class ItemsAutoDestroy
{
	...
	private ItemsAutoDestroy()
	{
		...
		if (_sleep == 0)
		{ // it should not happend as it is not called when
	...
	public synchronized void removeItems()
	{
		...
		{
			if (item == null || item.getDropTime() == 0
			        || item.getLocation() != L2ItemInstance.ItemLocation.VOID)
			{
				_items.remove(item);
			}
			else
			{
				...
				{
					if ((curtime
					        - item.getDropTime()) > Config.HERB_AUTO_DESTROY_TIME)
					{
