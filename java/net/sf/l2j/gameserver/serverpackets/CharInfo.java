 ...
 * c cddd d
 * dddddSdddddddddddddddhhhhhhhhhhhhhhhhhhhhhhhhddddddddddddddffffdddSdddddccccccch
 * [h] c (ddhd) ddc c ddc cddd d d dd d d d
 *
	...
	protected final void writeImpl()
	{
		...
		if (_activeChar.getPoly().isMorphed())
		{
			...
			if (template != null)
			{
				...
				writeD(_mAtkSpd);
				writeD(_pAtkSpd);
				writeD(_runSpd); // TODO: the order of the speeds should be
				                 // confirmed
				writeD(_walkSpd);
				...
				if (gmSeeInvis)
				{
					writeD((_activeChar.getAbnormalEffect()
					        | L2Character.ABNORMAL_EFFECT_STEALTH));
				}
				else
				{
			...
			{
				_log.warning("Character " + _activeChar.getName() + " ("
				        + _activeChar.getObjectId() + ") morphed in a Npc ("
				        + _activeChar.getPoly().getPolyId()
				        + ") w/o template.");
			}
		}
		else
		{
			...
			writeC(_activeChar.getMountType()); // 1-on Strider, 2-on Wyvern,
			...
			writeC(_activeChar.isHero()
			        || (_activeChar.isGM() && Config.GM_HERO_AURA) ? 1 : 0); // Hero
			                                                                 // Aura

			writeC(_activeChar.isFishing() ? 1 : 0); // 0x01: Fishing Mode (Cant
			                                         // be undone by setting
			                                         // back to 0)
