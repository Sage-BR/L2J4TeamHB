...
public final class NpcInfo extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		if (_idTemplate > 13070 && _idTemplate < 13077)
		{
			...
			if (((L2Decoy) _activeChar).getOwner().getMountType() != 0
			        && pet != null)
			{
			...
			writeC(((L2Decoy) _activeChar).getOwner().isSitting() ? 0 : 1); // standing
			                                                                // =
			                                                                // 1
			                                                                // sitting
			                                                                // =
			                                                                // 0
			writeC(((L2Decoy) _activeChar).getOwner().isRunning() ? 1 : 0); // running
			                                                                // =
			                                                                // 1
			                                                                // walking
			                                                                // =
			...
			writeC(((L2Decoy) _activeChar).getOwner().getAppearance().getInvisible() ? 1 : 0); // invisible
			                                                                                   // =
			                                                                                   // 1
			                                                                                   // visible
			                                                                                   // =0

			writeC(((L2Decoy) _activeChar).getOwner().getMountType()); // 1 on
			                                                           // strider
			                                                           // 2 on
			                                                           // wyvern
			                                                           // 3 on
			                                                           ...
			                                                           // no
			                                                           // mount
			writeC(((L2Decoy) _activeChar).getOwner().getPrivateStoreType()); // 1
			                                                                  // -
			                                                                  // sellshop
			...
			writeC(((L2Decoy) _activeChar).getOwner().getRecomLeft()); // Changed
			                                                           // by
			                                                           // Thorgrim
			writeH(((L2Decoy) _activeChar).getOwner().getRecomHave()); // Blue
			                                                           // value
			                                                           // for
			                                                           // name
			                                                           // (0 =
			...
			writeC(((L2Decoy) _activeChar).getOwner().isNoble() ? 1 : 0); // Symbol
			                                                              // on
			                                                              // char
			                                                              // menu
			                                                              // ctrl+I
			writeC(((L2Decoy) _activeChar).getOwner().isHero() ? 1 : 0); // Hero
			                                                             // Aura

			writeC(((L2Decoy) _activeChar).getOwner().isFishing() ? 1 : 0); // 0x01:
			                                                                // Fishing
			                                                                // Mode
			                                                                // (Cant
			                                                                // be
		...
		else
		{
			...
			writeC(_isSummoned ? 2 : 0); // invisible ?? 0=false 1=true
			                             // 2=summoned (only works if model has
			                             // a summon animation)
			writeS(_name);
			writeS(_title);
			if (_activeChar instanceof L2Summon)
