...
public class SortedWareHouseWithdrawalList extends L2GameServerPacket
{
	 ...
	 */
	public SortedWareHouseWithdrawalList(L2PcInstance player, int type,
	        WarehouseListType itemtype, byte sortorder)
	{
		_activeChar = player;
		_whType = type;
		_itemtype = itemtype;
			...
			switch (_sortorder)
			{
				...
				case GRADE:
					if (_itemtype == WarehouseListType.ARMOR
					        || _itemtype == WarehouseListType.WEAPON)
					{
	...
	/**
	 * This public method return the integer of the Sortorder by its name. If
	 * you want to have another, add the Comparator and the Constant.
	 *
	 * @param order
	        ...
	        implements Comparator<L2WarehouseItem>
	{
		...
		{
			if (o1.getType2() == L2Item.TYPE2_MONEY
			        && o2.getType2() != L2Item.TYPE2_MONEY)
			{
				return (order == A2Z ? Z2A : A2Z);
			}
			if (o2.getType2() == L2Item.TYPE2_MONEY
			        && o1.getType2() != L2Item.TYPE2_MONEY)
			{
	        ...
	        implements Comparator<L2WarehouseItem>
	{
		...
		public int compare(L2WarehouseItem o1, L2WarehouseItem o2)
		{
			...
			if ((o1.isEtcItem() && o1.getItemType() == L2EtcItemType.RECEIPE)
			        && (o2.isEtcItem()
			                && o2.getItemType() == L2EtcItemType.RECEIPE))
			{
				try
				{
	...
	/**
	 * This Comparator will sort by Item Type. Unfortunatly this will only have
	 * a good result if the Database Table for the ETCITEM.TYPE column is fixed!
	 */
		...
		for (L2ItemInstance item : _items)
		{
			if (item.isEtcItem()
			        && (item.getEtcItem().getItemType() == L2EtcItemType.SPELLBOOK
			                && item.getItemName().toUpperCase().startsWith("AMULET"))
			        || item.getItem().getType2() == L2Item.TYPE2_MONEY)
			{
				if (_list.size() < MAX_SORT_LIST_ITEMS)
				{
	...
	/**
	 * This method is used to limit the given Warehouse List to:
	 * <li>Everything which is no Weapon/Armor, Material, Recipe, Spellbook,
	 * Scroll or Shot</li>
	 * <li>Money</li>
	 *
	 * @param _items
	...
	/**
	 * This method is used to limit the given Warehouse List to:
	 * <li>no limit</li> This may sound strange but we return the given Array as
	 * a List<L2WarehouseItem>
	 *
	 * @param _items
	 *            complete Warehouse List
	 * @return limited Item List
