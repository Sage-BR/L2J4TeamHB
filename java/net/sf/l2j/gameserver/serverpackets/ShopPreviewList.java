...
public class ShopPreviewList extends L2GameServerPacket
{
	...
	public ShopPreviewList(L2TradeList list, int currentMoney,
	        int expertiseIndex)
	{
	...
	public ShopPreviewList(Collection<L2TradeItem> lst, int listId,
	        int currentMoney)
	{
		_listId = listId;
		_list = lst;
	...
	protected final void writeImpl()
	{
		...
		{
			if (item.getTemplate().getCrystalType() <= _expertise
			        && item.getTemplate().isEquipable())
			{
				newlength++;
			}
		}
		...
		for (L2TradeItem item : _list)
		{
			        ...
			        && item.getTemplate().isEquipable())
			{
				...
				{
					writeH(item.getTemplate().getBodyPart()); // rev 415 slot
					                                          // 0006-lr.ear
					                                          // 0008-neck
					                                          // 0030-lr.finger
					                                          // 0040-head
