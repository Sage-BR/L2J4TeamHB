 ...
 *
 * $Author: luisantonioa $ $Date: 2/08/2005 14:21:01 $ $Revision: 1 $ $Log:
 * MultiSellList.java,v $ Revision 1 2/08/2005 14:21:01 luisantonioa Added
 * copyright notice
 *
 *
	...
	protected void writeImpl()
	{
		...
		if (_list != null)
		{
			for (MultiSellEntry ent : _list.getEntries())
			{
				...
				{
					writeD(ItemTable.getInstance().getTemplate(i.getItemId()).getDisplayId());
					writeD(0);
				...
				for (MultiSellIngredient i : ent.getIngredients())
				{
					...
					if (items != -200)
					{
						typeE = ItemTable.getInstance().getTemplate(items).getType2();
						displayId = ItemTable.getInstance().getTemplate(items).getDisplayId();
					}
					writeD(displayId); // ID
					writeH(typeE);
