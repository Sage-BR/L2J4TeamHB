 ...
 *
 * d - manor id d - size [ d - crop id d - seed level c d - reward 1 id c d -
 * reward 2 id d - next sale limit d d - min crop price d - max crop price d -
 * today buy d - today price c - today reward d - next buy d - next price c -
 * next reward ]
 *
 * @author l3x
 */
public class ExShowCropSetting extends L2GameServerPacket
{
	...
	public ExShowCropSetting(int manorId)
	{
		...
		for (int cr : crops)
		{
			_cropData[i * 14 + 0] = cr;
			_cropData[i * 14
			        + 1] = L2Manor.getInstance().getSeedLevelByCrop(cr);
			_cropData[i * 14 + 2] = L2Manor.getInstance().getRewardItem(cr, 1);
			_cropData[i * 14 + 3] = L2Manor.getInstance().getRewardItem(cr, 2);
			_cropData[i * 14
			        + 4] = L2Manor.getInstance().getCropPuchaseLimit(cr);
			_cropData[i * 14 + 5] = 0; // Looks like not used
			_cropData[i * 14 + 6] = L2Manor.getInstance().getCropBasicPrice(cr)
			        * 60 / 100;
			_cropData[i * 14 + 7] = L2Manor.getInstance().getCropBasicPrice(cr)
			        * 10;
