 ...
 *
 * d - manor id d - size [ d - seed id d - level c d - reward 1 id c d - reward
 * 2 id d - next sale limit d - price for castle to produce 1 d - min seed price
 * d - max seed price d - today sales d - today price d - next sales d - next
 * price ]
 *
 * @author l3x
 */
public class ExShowSeedSetting extends L2GameServerPacket
{
	...
	public ExShowSeedSetting(int manorId)
	{
		...
		for (int s : seeds)
		{
			...
			_seedData[i * 12
			        + 2] = L2Manor.getInstance().getRewardItemBySeed(s, 1);
			_seedData[i * 12
			        + 3] = L2Manor.getInstance().getRewardItemBySeed(s, 2);
			_seedData[i * 12 + 4] = L2Manor.getInstance().getSeedSaleLimit(s);
			_seedData[i * 12 + 5] = L2Manor.getInstance().getSeedBuyPrice(s);
			_seedData[i * 12 + 6] = L2Manor.getInstance().getSeedBasicPrice(s)
			        * 60 / 100;
			_seedData[i * 12 + 7] = L2Manor.getInstance().getSeedBasicPrice(s)
			        * 10;
