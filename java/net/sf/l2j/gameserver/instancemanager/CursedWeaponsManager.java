...
public class CursedWeaponsManager
{
	...
	public CursedWeaponsManager()
	{
		...
		_cursedWeapons = new ConcurrentHashMap<>();
	...
	private final void controlPlayers()
	{
		...
		try
		{
			...
			// This entire for loop should NOT be necessary, since it is already
			// handled by
			...
			// by someone
				...
				try
				{
					...
					if (rset.next())
					{
						...
						/*
						 * statement = con.
						 * prepareStatement("DELETE FROM character_skills WHERE charId=? AND skill_id="
						...
						if (statement.executeUpdate() != 1)
						{
							_log.warning("Error while updating karma & pkkills for userId "
							        + cw.getPlayerId());
						}
	...
	public void activate(L2PcInstance player, L2ItemInstance item)
	{
		...
		if (player.isCursedWeaponEquipped()) // cannot own 2 cursed swords
		{
			...
			/*
			 * TODO: give the bonus level in a more appropriate manner. The
			 * following code adds "_stageKills" levels. This will also show in
			 * the char status. I do not have enough info to know if the bonus
			 * should be shown in the pk count, or if it should be a full
			 * "_stageKills" bonus or just the remaining from the current count
			 * till the of the current stage... This code is a TEMP fix, so that
			 * the cursed weapon's bonus level can be observed with as little
			 * change in the code as possible, until proper info arises.
			 */
			cw2.setNbKills(cw2.getStageKills() - 1);
			cw2.increaseKills();

			// erase the newly obtained cursed weapon
			cw.setPlayer(player); // NECESSARY in order to find which inventory
			                      // the weapon is in!
	...
	public void checkPlayer(L2PcInstance player)
	{
		...
		for (CursedWeapon cw : _cursedWeapons.values())
		{
			if (cw.isActivated() && player.getObjectId() == cw.getPlayerId())
			{
				...
				sm.addNumber((int) ((cw.getEndTime()
