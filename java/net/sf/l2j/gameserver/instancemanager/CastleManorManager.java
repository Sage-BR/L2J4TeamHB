...
public class CastleManorManager
{
				...
				if (!isDisabled())
				{
					...
					_scheduledMaintenanceEnd = ThreadPoolManager.getInstance().scheduleGeneral(new Runnable() {
						...
						public void run()
						{
							...
							{
								_log.info("Manor System: Failed to save manor data: "
								        + e);
							}
	...
	public long getMillisToManorRefresh()
	{
		...
		{
			return (_manorRefresh.getTimeInMillis()
	...
	public long setNewManorRefresh()
	{
		...
		_log.info("Manor System: New Schedule for manor refresh @ "
		        + _manorRefresh.getTime());
	...
	public long getMillisToNextPeriodApprove()
	{
		...
		{
			return (_periodApprove.getTimeInMillis()
			        - Calendar.getInstance().getTimeInMillis());
		}
	...
	public long setNewPeriodApprove()
	{
		...
		_log.info("Manor System: New Schedule for period approve @ "
		        + _periodApprove.getTime());
	...
	public void setNextPeriod()
	{
		for (Castle c : CastleManager.getInstance().getCastles())
		{
			...
			{
				_log.info("Can't get clan warehouse for clan "
				        + ClanTable.getInstance().getClan(c.getOwnerId()));
				return;
			}
	...
	public SeedProduction getNewSeedProduction(int id, int amount, int price,
	        int sales)
	{
		return new SeedProduction(id, amount, price, sales);
	}

	public CropProcure getNewCropProcure(int id, int amount, int type,
	        int price, int buy)
	{
