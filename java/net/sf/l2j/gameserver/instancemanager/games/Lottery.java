...
public class Lottery
{
	...
	public void increasePrize(int count)
	{
		...
		{
			_log.warning("Lottery: Could not increase current lottery prize: "
			        + e);
		}
		finally
		{
	...
	private class startLottery implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				if (rset.next())
				{
					...
					else
					{
						...
						{
							_isStarted = true;
							ThreadPoolManager.getInstance().scheduleGeneral(new finishLottery(), _enddate
							        ...
							        + 12 * MINUTE)
							{
								...
								ThreadPoolManager.getInstance().scheduleGeneral(new stopSellingTickets(), _enddate
								        - System.currentTimeMillis()
								        - 10 * MINUTE);
							}
							rset.close();
			...
			Announcements.getInstance().announceToAll("Lottery tickets are now available for Lucky Lottery #"
			        + getId() + ".");
			...
			ThreadPoolManager.getInstance().scheduleGeneral(new stopSellingTickets(), _enddate
			        - System.currentTimeMillis() - 10 * MINUTE);
			ThreadPoolManager.getInstance().scheduleGeneral(new finishLottery(), _enddate
			        - System.currentTimeMillis());
	...
	private class finishLottery implements Runnable
	{
		...
		public void run()
		{
			...
			{
				_log.warning("Lottery: Could not store finished lottery data: "
				        + e);
			}
			finally
			{
