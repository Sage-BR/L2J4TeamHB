...
public class ClanHallManager
{
	...
	{
		_clanHall = new ConcurrentHashMap<>();
		_freeClanHall = new ConcurrentHashMap<>();
		_allClanHalls = new ConcurrentHashMap<>();
		load();
	}
	 ...
	 * _clanHall.clear(); _freeClanHall.clear(); load(); }
	 */
	...
	private final void load()
	{
		...
		try
		{
			...
			_log.info("Loaded: " + getFreeClanHalls().size()
			        + " free clan halls");
			_loaded = true;
		}
		catch (Exception e)
		{
			_log.warning("Exception: ClanHallManager.load(): "
			        + e.getMessage());
			e.printStackTrace();
		}
		finally
	...
	/** Get Clan Hall by x,y,z *//*
	                              * public final ClanHall getClanHall(int x, int
	                              * y, int z) { for (Map.Entry<Integer,
	                              * ClanHall> ch : _clanHall.entrySet()) if
	                              * (ch.getValue().getZone().isInsideZone(x, y,
	                              * z)) return ch.getValue();
	                              *
	                              * for (Map.Entry<Integer, ClanHall> ch :
	                              * _freeClanHall.entrySet()) if
	                              * (ch.getValue().getZone().isInsideZone(x, y,
	                              * z)) return ch.getValue();
	                              *
	                              * return null; }
