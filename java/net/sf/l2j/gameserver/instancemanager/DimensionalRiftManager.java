...
public class DimensionalRiftManager
{
	...
	private ConcurrentHashMap<Byte, ConcurrentHashMap<Byte, DimensionalRiftRoom>> _rooms = new ConcurrentHashMap<>();
	...
	private void loadRooms()
	{
		...
		_log.info("DimensionalRiftManager: Loaded " + typeSize
	...
	public void loadSpawns()
	{
								...
								if ("room".equalsIgnoreCase(room.getNodeName()))
								{
									...
									for (Node spawn = room.getFirstChild(); spawn != null; spawn = spawn.getNextSibling())
									{
										if ("spawn".equalsIgnoreCase(spawn.getNodeName()))
										{
											...
											if (template == null)
											{
												_log.warning("Template " + mobId
												        + " not found!");
											}
											if (!_rooms.containsKey(type))
											{
												_log.warning("Type " + type
												        + " not found!");
											}
											else if (!_rooms.get(type).containsKey(roomId))
											{
												_log.warning("Room " + roomId
												        + " in Type " + type
												        + " not found!");
											}
		...
		_log.info("DimensionalRiftManager: Loaded " + countGood
	...
	public boolean checkIfInRiftZone(int x, int y, int z,
	        boolean ignorePeaceZone)
	{
		if (ignorePeaceZone)
		{
	...
	public class DimensionalRiftRoom
	{
		...
		protected final ArrayList<L2NpcInstance> _roomMobs;

		public DimensionalRiftRoom(byte type, byte room, int xMin, int xMax,
		        int yMin, int yMax, int zMin, int zMax, int xT, int yT, int zT,
		        boolean isBossRoom)
		{
			_type = type;
			_room = room;
			...
			_s = new Polygon(new int[] { xMin, xMax, xMax, xMin }, new int[] {
