...
public class FortSiegeGuardManager
{
	 ...
	 * Presumably, this is used when a fort lord picks up a previously dropped
	 * ticket
	 */
	...
	public void removeMercs()
	{
		...
		{
			_log.warning("Error deleting hired siege guard for fort "
			        + getFort().getName() + ":" + e1);
		}
		finally
		{
	...
	public void spawnSiegeGuard()
	{
		...
		{
			// hiredMax =
			// MercTicketManager.getInstance().getMaxAllowedMerc(_fort.getFortId());
		...
		{
			_log.warning("Error spawning siege guards for fort "
			        + getFort().getName() + ":" + t.toString());
		}
	}
	...
	private void loadSiegeGuard()
	{
		...
		{
			_log.warning("Error loading siege guard for fort "
			        + getFort().getName() + ":" + e1);
		}
		finally
		{
	        ...
	        int isHire)
	{
		...
		{
			_log.warning("Error adding siege guard for fort "
			        + getFort().getName() + ":" + e1);
		}
		finally
		{
