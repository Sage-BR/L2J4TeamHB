...
public final class L2GameClient extends MMOClient<MMOConnection<L2GameClient>>
{
	...
	public L2GameClient(MMOConnection<L2GameClient> con)
	{
		...
		{
			_autoSaveInDB = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new AutoSaveTask(), 300000L, (Config.CHAR_STORE_INTERVAL
			        * 60000L));
		}
		else
		{
	...
	public String toString()
	{
		try
		{
			...
			switch (getState())
			{
				...
				case IN_GAME:
					return "[Character: "
					        + (getActiveChar() == null ? "disconnected" : getActiveChar().getName())
					        + " - Account: " + getAccountName() + " - IP: "
					        + (address == null ? "disconnected" : address.getHostAddress())
					        + "]";
				default:
