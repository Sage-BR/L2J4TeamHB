 ...
 * attached. (Potential errors handling the packet).</li>
 * <li>Clients sends a RequestAuthLogin being already authed. (Potential
 * exploit).</li> <BR>
 * <BR>
 ...
 * @author KenM
 */
public final class L2GamePacketHandler implements IPacketHandler<L2GameClient>,
        IClientFactory<L2GameClient>, IMMOExecutor<L2GameClient>
{
	...
	// implementation
	@Override
	public ReceivablePacket<L2GameClient> handlePacket(ByteBuffer buf,
	        L2GameClient client)
	{
		...
		switch (state)
		{
			case CONNECTED:
				...
				{
					case 0x00:
						_log.warning("Client " + client.toString()
						        + " is trying to connect using Interlude Client");
						break;
					case 0x0e:
			...
			case AUTHED:
				switch (opcode)
				{
					...
					case 0xd0:
						...
						{
							_log.warning("Client: " + client.toString()
							        + " sent a 0xd0 without the second opcode.");
							break;
						}
...
//						THESE ARE NOW TEMPORARY DISABLED
					...
					case 0x6f:
						...
						break;
					case 0x73: // send when talking to trainer npc, to show list
					           // of available skills
...
//						break;
					case 0xa6:
						...
						/*
						 * if (Config.DEBUG)
						 * _log.info("Request Skill Cool Time .. ignored"); msg
						 * = null;
						 */
						break;
					case 0xa7:
...
//						break;
					...
					case 0xb4:
						msg = new SnoopQuit();
						break;
					case 0xb5: // we still need this packet to handle BACK
					...
					case 0x6c:
						...
						break;
					case 0x6d: // MSN dialogs so that you dont see them in the
					           // console.
						break;
					...
					case 0xd0:
						...
						switch (id2)
						{
							...
							case 0x21:
								...
								break;
							case 0x22:
								// TODO implement me (just disabling warnings
								// for this packet)
								break;
							case 0x24:
	...
	private void printDebug(int opcode, ByteBuffer buf, GameClientState state,
	        L2GameClient client)
	{
		int size = buf.remaining();
		_log.warning("Unknown Packet: " + Integer.toHexString(opcode)
		        + " on State: " + state.name() + " Client: "
		        + client.toString());
	...
	private void printDebugDoubleOpcode(int opcode, int id2, ByteBuffer buf,
	        GameClientState state, L2GameClient client)
	{
		int size = buf.remaining();
		_log.warning("Unknown Packet: " + Integer.toHexString(opcode) + ":"
		        + Integer.toHexString(id2) + " on State: " + state.name()
	...
	public void execute(ReceivablePacket<L2GameClient> rp)
	{
		...
		catch (RejectedExecutionException e)
		{
			...
			{
				_log.severe("Failed executing: " + rp.getClass().getSimpleName()
				        + " for Client: " + rp.getClient().toString());
			}
		}
	}
