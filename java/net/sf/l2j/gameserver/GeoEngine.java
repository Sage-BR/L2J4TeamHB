...
public class GeoEngine extends GeoData
{
	...
	private static Map<Short, MappedByteBuffer> _geodata = new ConcurrentHashMap<>();

	private static Map<Short, ABlock[]> _geoBlocks = new ConcurrentHashMap<>();
	...
	{
		return nGetType((x - L2World.MAP_MIN_X) >> 4, (y
		        - L2World.MAP_MIN_Y) >> 4);
	}
	...
	{
		return nGetHeight((x - L2World.MAP_MIN_X) >> 4, (y
		        - L2World.MAP_MIN_Y) >> 4, z);
	}
	...
	{
		return nGetSpawnHeight((x - L2World.MAP_MIN_X) >> 4, (y
		        - L2World.MAP_MIN_Y) >> 4, zmin, zmax, spawnid);
	}
	...
	{
		return nTraceTerrainZ((x - L2World.MAP_MIN_X) >> 4, (y
		        - L2World.MAP_MIN_Y) >> 4, z, (tx - L2World.MAP_MIN_X) >> 4, (ty
		                - L2World.MAP_MIN_Y) >> 4);
	}
	...
	public boolean canSeeTargetDebug(L2PcInstance gm, L2Object target)
	{
		...
		{
			return canSeeDebug(gm, (gm.getX()
			        - L2World.MAP_MIN_X) >> 4, (gm.getY()
			                - L2World.MAP_MIN_Y) >> 4, z, (target.getX()
			                        - L2World.MAP_MIN_X) >> 4, (target.getY()
			                                - L2World.MAP_MIN_Y) >> 4, z2);
		}
		else
		{
			return canSeeDebug(gm, (target.getX()
			        - L2World.MAP_MIN_X) >> 4, (target.getY()
			                - L2World.MAP_MIN_Y) >> 4, z2, (gm.getX()
			                        - L2World.MAP_MIN_X) >> 4, (gm.getY()
			                                - L2World.MAP_MIN_Y) >> 4, z);
		}
	}
	...
	{
		return nGetNSWE((x - L2World.MAP_MIN_X) >> 4, (y
		        - L2World.MAP_MIN_Y) >> 4, z);
	}
	...
	public Location moveCheck(int x, int y, int z, int tx, int ty, int tz)
	{
		...
		return moveCheck(startpoint, destiny, (x - L2World.MAP_MIN_X) >> 4, (y
		        - L2World.MAP_MIN_Y) >> 4, z, (tx - L2World.MAP_MIN_X) >> 4, (ty
	...
	{
		return canSee((x - L2World.MAP_MIN_X) >> 4, (y
		        - L2World.MAP_MIN_Y) >> 4, z, (tx - L2World.MAP_MIN_X) >> 4, (ty
		                - L2World.MAP_MIN_Y) >> 4, tz);
	}
	        ...
	        int tz)
	{
		...
		else if (distance2 < 82)
		{
			...
			if (dz * dz > 22500)
			{
				...
				// geodata is loaded for region and mobs should have correct Z
				// coordinate...
		...
		if (dx >= dy)// dy/dx <= 1
		{
			...
			for (int i = 0; i < dx; i++)
			{
				...
				if (d > 0)
				{
					...
					if (!nLOS(x, y, (int) z, inc_x, inc_y, inc_z_directionx
					        + inc_z_directiony, tz, false))
					{
						return false;
					}
				}
	...
	/*
	 * Debug function for checking if there's a line of sight between two
	 * coordinates.
	 *
	 * Creates points for line of sight check (x,y,z towards target) and in each
	 * point, layer and movement checks are made with NLOS function.
	 *
	 ...
	 * MoveCheck
	 */
	private static Location moveCheck(Location startpoint, Location destiny,
	        int x, int y, double z, int tx, int ty, int tz)
	{
		int dx = (tx - x);
		int dy = (ty - y);
		final int distance2 = dx * dx + dy * dy;
		...
		// gm.sendMessage("MoveCheck: from X: "+x+ "Y: "+y+ "--->> X: "+tx+" Y:
		// "+ty);
		...
		if (dx >= dy)// dy/dx <= 1
		{
			...
			for (int i = 0; i < dx; i++)
			{
				...
				if (d > 0)
				{
					...
					{
						return new Location((x << 4)
						        + L2World.MAP_MIN_X, (y << 4)
						                + L2World.MAP_MIN_Y, (int) z);
					}
					else
					{
						z = tempz;
		...
		{ // geodata hasn't modified Z in any
			// coordinate, i.e. doesn't exist
						return destiny;
		}
		else
		{
	...
	private static void nInitGeodata()
	{
		...
		try
		{
			...
			{
				_log.info("Geo Engine: - Loaded " + _geoLoadedCount
				        + " geodata archives.");
			}
		}
		catch (Exception e)
	...
	public static boolean loadGeodataFile(byte rx, byte ry)
	{
		        ...
		        FileChannel roChannel = raf.getChannel())
		{
			...
			{ // Force O/S to Loads this buffer's
			...
			if (size > 196608)
			{
				...
				// Keep _geodata for lightweight block reads (no pre-decoded
				// byte[] anymore)
			}
			else
			{
				// Small file: all blocks are flat (3 bytes each = 1 type + 2
				// height)
				ABlock[] blocks = new ABlock[65536];
				geo.position(0);
				for (int bi = 0; bi < 65536; bi++)
	        ...
	        int spawnid)
	{
		...
		else if (block instanceof BlockMultilayer)
		{
			...
			if (temph > zmax + 200 || temph < zmin - 200)
			{
				if (Config.DEBUG)
				{
					_log.warning("SpawnHeight Error - Couldnt find correct layer to spawn NPC - GeoData or Spawnlist Bug!: zmin: "
			...
			if (temph > zmax + 1000 || temph < zmin - 1000)
			{
				if (Config.DEBUG)
				{
					_log.warning("SpawnHeight Error - Spawnlist z value is wrong or GeoData error: zmin: "
					        + zmin + " zmax: " + zmax + " value: " + temph
					        + " SpawnId: " + spawnid + " at: " + geox + " : "
					        + geoy);
				}
	 ...
	 */
	private static short nTraceTerrainZ(int geox, int geoy, int z, int tgeox,
	        int tgeoy)
	{
		int dx = tgeox - geox;
		int dy = tgeoy - geoy;
	        ...
	        int tz)
	{
		...
		else if (block instanceof BlockComplex)
		{
			...
			if (!checkNSWE((short) (nswe & 0xFF), x, y, tx, ty))
			{
				...
				{
					_log.info("[GEO] nCanMoveNext BLOCKED (complex): from=(" + x
					        + "," + y + "," + z + ") to=(" + tx + "," + ty
					        + ") NSWE=" + nswe + " type=complex height="
					        + height);
				}
		...
		else if (block instanceof BlockMultilayer)
		{
			...
			// em vez do checkMove() opaco da JAR que tem bug de seleção de
			// layer.
			byte nswe = block.getNsweNearest(x, y, z);
			if (!checkNSWE((short) (nswe & 0xFF), x, y, tx, ty))
			{
				// NSWE bloqueou. Verifica se é rampa (dado corrompido) ou
				// parede real.
				// O threshold de 96 cobre rampas íngremes (diff 48-88
				// observados)
				// mas ainda bloqueia paredes reais (diff tipicamente 200+).
				short srcH = nGetHeight(x, y, z);
				short dstH = nGetHeight(tx, ty, z);

				// Correcao de descida/subida de rampa: quando dstH > srcH
				// significa que
				// getHeightNearest retornou a layer acima (teto da rampa) em
				// vez do chão.
				// Tenta buscar uma layer abaixo de srcH no destino para a
				// descida real.
				if (dstH > srcH + 16)
				{
				...
				{
					// Se dstH ainda está acima do z atual do personagem,
					// retornar srcH
					// para nao jogar o personagem para dentro da geometria
					// acima.
					// O sistema de movimento vai ajustar progressivamente.
					short safeH = (dstH > z) ? srcH : dstH;
					if (Config.MOVE_DEBUG)
					{
						_log.info("[GEO] nCanMoveNext RAMP-IGNORE-NSWE: from=("
						        + x + "," + y + "," + z + ") to=(" + tx + ","
						        + ty + ") NSWE=" + nswe + " srcH=" + srcH
						        + " dstH=" + dstH + " safeH=" + safeH);
					}
					return safeH;
				}
				if (Config.MOVE_DEBUG)
				{
					_log.info("[GEO] nCanMoveNext BLOCKED (multilevel): from=("
					        + x + "," + y + "," + z + ") to=(" + tx + "," + ty
					        + ") NSWE=" + nswe + " srcH=" + srcH + " dstH="
					        + dstH + " diff=" + Math.abs(dstH - srcH));
				}
				return Double.MIN_VALUE;
			}
	 ...
	 */
	private static boolean nLOS(int x, int y, int z, int inc_x, int inc_y,
	        double inc_z, int tz, boolean debug)
	{
		ABlock block = getABlock(x, y);
		if (!block.hasGeoPos())
		{
		...
		else if (block instanceof BlockMultilayer)
		{
			...
			if (debug)
			{
				_log.warning("z:" + z + " x: " + cellX + " y:" + cellY + " la "
				        + layers + " lo:" + lowerHeight + " up:" + upperHeight);
			}
			// Check if LOS goes under a layer/floor
			if ((z - upperHeight) < -10 && (z - upperHeight) > inc_z - 10
			        && (z - lowerHeight) > 40)
			{
				if (debug)
				{
			...
			if (!highestlayer)
			{
				if (!checkNSWE((short) (currentNSWE & 0xFF), x, y, x + inc_x, y
				        + inc_y))
				{
					if (debug)
					{
						_log.warning("block and next in x" + inc_x + " y"
						        + inc_y + " is:" + nGetUpperHeight(x + inc_x, y
						                + inc_y, lowerHeight));
					}
	...
	private static boolean checkNSWE(short NSWE, int x, int y, int tx, int ty)
	{
		...
		if (tx != x && ty != y)
		{
			if (ty > y)
			{
				if (tx > x)
				{
					return checkNSWE(NSWE, x, y, tx, y)
					        && checkNSWE(NSWE, x, y, x, ty);
				}
				else
				{
					return checkNSWE(NSWE, x, y, tx, y)
