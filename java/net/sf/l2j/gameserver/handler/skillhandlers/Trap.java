...
{
	private static final SkillType[] SKILL_IDS = { SkillType.DETECT_TRAP,
	        SkillType.REMOVE_TRAP };

	/**
	 *
	 * @see net.sf.l2j.gameserver.handler.ISkillHandler#useSkill(net.sf.l2j.gameserver.model.L2Character,
		...
		switch (skill.getSkillType())
		{
			case DETECT_TRAP:
			{
				for (L2Object target2 : targets)
				{
					...
					if (!(target instanceof L2TrapInstance) || target.isAlikeDead())
					{
						continue;
					}
			...
			case REMOVE_TRAP:
			{
				for (L2Object target2 : targets)
				{
					...
					if (!(target instanceof L2Trap) || !((L2Trap) target).isDetected() || (((L2Trap) target).getLevel() > skill.getPower()))
					{
						continue;
					}
