...
public class Disablers implements ISkillHandler
{
	private static final SkillType[] SKILL_IDS = { L2Skill.SkillType.STUN,
	        L2Skill.SkillType.ROOT, L2Skill.SkillType.SLEEP,
	        L2Skill.SkillType.CONFUSION, L2Skill.SkillType.AGGDAMAGE,
	        L2Skill.SkillType.AGGREDUCE, L2Skill.SkillType.AGGREDUCE_CHAR,
	        L2Skill.SkillType.AGGREMOVE, L2Skill.SkillType.UNBLEED,
	        L2Skill.SkillType.UNPOISON, L2Skill.SkillType.MUTE,
	        L2Skill.SkillType.FAKE_DEATH, L2Skill.SkillType.CONFUSE_MOB_ONLY,
	        L2Skill.SkillType.NEGATE, L2Skill.SkillType.CANCEL,
	        L2Skill.SkillType.CANCEL_DEBUFF, L2Skill.SkillType.PARALYZE,
		...
		for (L2Object target2 : targets)
		{
			...
			switch (type)
			{
				...
				case AGGDAMAGE:
				{
					if (target instanceof L2Attackable)
					{
						target.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, activeChar, (int) ((150
						        * skill.getPower()) / (target.getLevel() + 7)));
					}
				...
				case AGGREDUCE:
				{
					...
					// when fail,
					// target.getAI().notifyEvent(CtrlEvent.EVT_ATTACKED,
					// activeChar);
					break;
				}
				case AGGREDUCE_CHAR:
				...
				case CANCEL_DEBUFF:
				{
					...
					for (L2Effect e : effects)
					{
						if (e.getSkill().isDebuff()
						        && count < skill.getMaxNegatedEffects())
						{
							e.exit();
							if (count > -1)
							{
						...
						for (String stat : _negateStats)
						{
							...
							if (stat == "buff")
							{
								...
								if (Rnd.get(100) < landrate)
								{
							        ...
							        && removedBuffs < skill.getMaxNegatedEffects())
							{
	...
	private int negateEffect(L2Character target, SkillType type, double power,
	        int maxRemoved)
	{
