...
public class AdminEditNpc implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS = { "admin_edit_npc",
	        "admin_save_npc", "admin_show_droplist", "admin_edit_drop",
	        "admin_add_drop", "admin_del_drop", "admin_showShop",
	        "admin_showShopList", "admin_addShopItem", "admin_delShopItem",
	        "admin_box_access", "admin_editShopItem", "admin_close_window" };
		...
		else if (command.startsWith("admin_edit_drop "))
		{
			...
			try
			{
				...
				else if (st.countTokens() == 6)
				{
					...
					{
						_log.fine("admin_edit_drop parements error: "
						        + command);
					}
				}
				else
	...
	private void editShopItem(L2PcInstance activeChar, String[] args)
	{
		...
		if (args.length > 3)
		{
			...
			activeChar.sendMessage("Updated price for " + item.getName()
		...
		replyMSG.append("<button value=\"Save\" action=\"bypass -h admin_editShopItem "
		        + tradeListID + " " + itemID
		        + " $price\"  width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
		replyMSG.append("<br><button value=\"Back\" action=\"bypass -h admin_showShopList "
		        + tradeListID
	...
	private void delShopItem(L2PcInstance activeChar, String[] args)
	{
		...
		if (args.length > 3)
		{
			...
			activeChar.sendMessage("Deleted "
		...
		replyMSG.append("</table>");
		replyMSG.append("<center><br><br><br>");
		replyMSG.append("<button value=\"Confirm\" action=\"bypass -h admin_delShopItem "
		        + tradeListID + " " + itemID
		        + " 1\"  width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
		replyMSG.append("<br><button value=\"Back\" action=\"bypass -h admin_showShopList "
		        + tradeListID
	...
	private void addShopItem(L2PcInstance activeChar, String[] args)
	{
		...
		replyMSG.append("</table>");
		replyMSG.append("<center><br><br><br>");
		replyMSG.append("<button value=\"Save\" action=\"bypass -h admin_addShopItem "
		        + tradeListID
	...
	private void showShopList(L2PcInstance activeChar, int tradeListID,
	        int page)
	{
		L2TradeList tradeList = TradeController.getInstance().getBuyList(tradeListID);
	...
	private StringBuilder itemListHtml(L2TradeList tradeList, int page)
	{
		...
		replyMSG.append("<html><title>Merchant Shop List Page: " + page
		        + "</title>");
		replyMSG.append("<body>");
		...
		int end = Math.min(((page - 1) * PAGE_LIMIT)
		        + (PAGE_LIMIT - 1), tradeList.getItems().size() - 1);
		for (L2TradeItem item : tradeList.getItems(start, end + 1))
		{
			...
			replyMSG.append("<td><button value=\"Del\" action=\"bypass -h admin_delShopItem "
			        + tradeList.getListId() + " " + item.getItemId()
		...
		replyMSG.append("</table>");
		replyMSG.append("<center>");
		replyMSG.append("<button value=\"Add\" action=\"bypass -h admin_addShopItem "
		        + tradeList.getListId()
	...
	private void showShop(L2PcInstance activeChar, int merchantID)
	{
		...
		for (L2TradeList tradeList : tradeLists)
		{
			if (tradeList != null)
			{
				replyMSG.append("<tr><td><a action=\"bypass -h admin_showShopList "
	        ...
	        int order)
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement stmt = con.prepareStatement("UPDATE merchant_buylists SET `price`='"
	...
	private void deleteTradeList(int tradeListID, int order)
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement stmt = con.prepareStatement("DELETE FROM merchant_buylists WHERE `shop_id`='"
	...
	private List<L2TradeList> getTradeLists(int merchantID)
	{
		...
		String content = HtmCache.getInstance().getHtm("data/html/merchant/"
		        + merchantID + ".htm");
		...
		for (String line : lines)
		{
			...
			if (pos >= 0)
			{
				int tradeListID = Integer.decode((line.substring(pos
				        + target.length() + 1)).split("\"")[0]);
	...
	private void showNpcDropList(L2PcInstance activeChar, int npcId)
	{
		...
		StringBuilder replyMSG = new StringBuilder("<html><title>NPC: "
		        + npcData.name + "(" + npcData.npcId
		        + ") 's drop manage</title>");
		replyMSG.append("<body>");
			...
			for (L2DropCategory cat : npcData.getDropData())
			{
				for (L2DropData drop : cat.getAllDrops())
				{
					replyMSG.append("<tr><td><a action=\"bypass -h admin_edit_drop "
					        ...
					        + " " + drop.getItemId() + " "
					        + cat.getCategoryType() + "</a></td>" + "<td>"
					        + ItemTable.getInstance().getTemplate(drop.getItemId()).getName()
					        + "[" + drop.getItemId() + "]" + "</td><td>"
					        + (drop.isQuestDrop() ? "Q" : (cat.isSweep() ? "S" : "D"))
					        + "</td><td>"
	...
	private void showEditDropData(L2PcInstance activeChar, int npcId,
	        int itemId, int category)
	{
		...
		try
		{
			...
			StringBuilder replyMSG = new StringBuilder("<html><title>the detail of dropdata: ("
			        + npcId + " " + itemId + " " + category + ")</title>");
			replyMSG.append("<body>");
			...
			if (dropData.next())
			{
				...
				replyMSG.append("<tr><td>Appertain of NPC</td><td>"
				        + NpcTable.getInstance().getTemplate(dropData.getInt("mobId")).name
				        + "</td></tr>");
				...
				replyMSG.append("<tr><td>Category</td><td>"
				        + ((category == -1) ? "sweep" : Integer.toString(category))
				        + "</td></tr>");
	...
	private void showAddDropData(L2PcInstance activeChar, L2NpcTemplate npcData)
	{
		...
		StringBuilder replyMSG = new StringBuilder("<html><title>Add dropdata to "
	...
	private void updateDropData(L2PcInstance activeChar, int npcId, int itemId,
	        int min, int max, int category, int chance)
	{
		...
		try
		{
			...
			if (npcId > 0)
			{
				...
				replyMSG.append("<body>");
				replyMSG.append("<center><button value=\"DropList\" action=\"bypass -h admin_show_droplist "
				        + npcId
	...
	private void addDropData(L2PcInstance activeChar, int npcId, int itemId,
	        int min, int max, int category, int chance)
	{
		java.sql.Connection con = null;
		...
		try
		{
			...
			replyMSG.append("<body>");
			replyMSG.append("<center><button value=\"Continue add\" action=\"bypass -h admin_add_drop "
			        + npcId
			        + "\" width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
			replyMSG.append("<br><br><button value=\"DropList\" action=\"bypass -h admin_show_droplist "
			        + npcId
			        + "\" width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
			replyMSG.append("</center></body></html>");
	...
	private void deleteDropData(L2PcInstance activeChar, int npcId, int itemId,
	        int category)
	{
		java.sql.Connection con = null;
		try
		{
			...
			if (npcId > 0)
			{
				...
				NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
				StringBuilder replyMSG = new StringBuilder("<html><title>Delete drop data("
				        + npcId + ", " + itemId + ", " + category
				        + ")complete</title>");
				replyMSG.append("<body>");
				replyMSG.append("<center><button value=\"DropList\" action=\"bypass -h admin_show_droplist "
