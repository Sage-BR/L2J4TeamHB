...
public class AdminCreateItem implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS = { "admin_l2jfrozen",
	        "admin_itemcreate", "admin_create_item", "admin_mass_create",
	...
	@Override
	public boolean useAdminCommand(final String command,
	        final L2PcInstance activeChar)
	{
		/*
		 * if(!AdminCommandAccessRights.getInstance().hasAccess(command,
		 * activeChar.getAccessLevel())){ return false; } if(Config.GMAUDIT) {
		 * Logger _logAudit = Logger.getLogger("gmaudit"); LogRecord record =
		 * new LogRecord(Level.INFO, command); record.setParameters(new Object[]
		 * { "GM: " + activeChar.getName(), " to target [" +
		 * activeChar.getTarget() + "] " }); _logAudit.LOGGER(record); }
		 */
		...
		switch (comm)
		{
			...
			/*
			 * Command //l2jfrozen, it gives useful items to Gm character
			 * Crystals, Gemstones, Bss-ss, scrolls, elixirs, etc To be
			 * complete...
			 */
			case admin_l2jfrozen:
				...
				Player.getInventory().addItem("Admin", 1458, 5000, Player, activeChar); // Cry
				                                                                        // d
				Player.getInventory().addItem("Admin", 1459, 5000, Player, activeChar); // Cry
				                                                                        // c
				Player.getInventory().addItem("Admin", 1460, 5000, Player, activeChar); // Cry
				                                                                        // b
				Player.getInventory().addItem("Admin", 1461, 5000, Player, activeChar); // Cry
				                                                                        // a
				Player.getInventory().addItem("Admin", 1462, 5000, Player, activeChar); // Cry
				                                                                        // s

				Player.getInventory().addItem("Admin", 2130, 200, Player, activeChar); // Gem
				                                                                       // d
				Player.getInventory().addItem("Admin", 2131, 200, Player, activeChar); // Gem
				                                                                       // c
				Player.getInventory().addItem("Admin", 2132, 200, Player, activeChar); // Gem
				                                                                       // b
				Player.getInventory().addItem("Admin", 2133, 200, Player, activeChar); // Gem
				                                                                       // a
				Player.getInventory().addItem("Admin", 2134, 200, Player, activeChar); // Gem
				                                                                       // s

				Player.getInventory().addItem("Admin", 736, 10, Player, activeChar); // Scroll
				                                                                     // of
				                                                                     // Escape
				Player.getInventory().addItem("Admin", 737, 10, Player, activeChar); // Scroll
				                                                                     // of
				                                                                     // Resurrection
				Player.getInventory().addItem("Admin", 1538, 10, Player, activeChar); // Blessed
				                                                                      // Scroll
				                                                                      // of
				                                                                      // Escape
				Player.getInventory().addItem("Admin", 1829, 10, Player, activeChar); // Scroll
				                                                                      // of
				                                                                      // Escape:
				                                                                      // Clan
				                                                                      // Hall
				Player.getInventory().addItem("Admin", 1830, 10, Player, activeChar); // Scroll
				                                                                      // of
				                                                                      // Escape:
				                                                                      // Castle
				Player.getInventory().addItem("Admin", 3936, 10, Player, activeChar); // Blessed
				                                                                      // Scroll
				                                                                      // of
				                                                                      // Resurrection
				Player.getInventory().addItem("Admin", 5858, 10, Player, activeChar); // Blessed
				                                                                      // Scroll
				                                                                      // of
				                                                                      // Escape:
				                                                                      // Clan
				                                                                      // Hall
				Player.getInventory().addItem("Admin", 5859, 10, Player, activeChar); // Blessed
				                                                                      // Scroll
				                                                                      // of
				                                                                      // Escape:
				                                                                      // Castle

				Player.getInventory().addItem("Admin", 1467, 1000, Player, activeChar); // Soulshot:
				                                                                        // S-grade
				Player.getInventory().addItem("Admin", 2514, 1000, Player, activeChar); // Spiritshot:
				                                                                        // S-grade
				Player.getInventory().addItem("Admin", 3952, 1000, Player, activeChar); // Blessed
				                                                                        // Spiritshot:
				                                                                        // S
				                                                                        // Grade

				Player.getInventory().addItem("Admin", 8627, 10, Player, activeChar); // Elixir
				                                                                      // of
				                                                                      // Life
				                                                                      // (S-Grade)
				Player.getInventory().addItem("Admin", 8633, 10, Player, activeChar); // Elixir
				                                                                      // of
				                                                                      // Mental
				                                                                      // Strength
				                                                                      // (S-Grade)
				Player.getInventory().addItem("Admin", 8639, 10, Player, activeChar); // Elixir
				                                                                      // of
				                                                                      // CP
				                                                                      // (S-Grade)

				Player.getInventory().addItem("Admin", 8874, 10, Player, activeChar); // Einhasad's
				                                                                      // Holy
				                                                                      // Water
			...
			case admin_create_item:
				...
				{
					AdminHelpPage.showHelpPage(activeChar, "itemcreation.htm");
					// activeChar.sendMessage("Usage: //itemcreate <itemId>
					// [amount]");
					return true;
				}
				return false;
	...
	private void createItem(final L2PcInstance activeChar, final int id,
	        final int num)
	{
		if (num > 20)
		{
		...
		if (activeChar.getTarget() != null)
		{
			if (activeChar.getTarget() instanceof L2PcInstance)
			{
				if (activeChar.getAccessLevel().getLevel() > 0
				        && activeChar.getAccessLevel().getLevel() < 3)
				{
		...
		{
			activeChar.sendMessage("You have spawned " + num
			        + " item(s) number " + id + " in your inventory.");
		}
		else
		{
			activeChar.sendMessage("You have spawned " + num
			        + " item(s) number " + id + " in " + Player.getName()
			        + "'s inventory.");
			Player.sendMessage("Admin has spawned " + num + " item(s) number "
			        + id + " in your inventory.");
		}
	...
	private void massCreateItem(final L2PcInstance activeChar, final int id,
	        final int num)
	{
		if (num > 20)
		{
		...
		activeChar.sendMessage("Mass-created items in the inventory of " + i
		        + " player(s).");
		LOGGER.info("GM " + activeChar.getName() + " mass_created item Id: "
		        + id + " (" + num + ")");
	}
