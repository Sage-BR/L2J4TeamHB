...
/**
 * This class handles following admin commands: - handles every admin menu
 * command
 *
...
public class AdminMenu implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS = { "admin_char_manage",
	        "admin_teleport_character_to_menu", "admin_recall_char_menu",
	        "admin_recall_party_menu", "admin_recall_clan_menu",
	        "admin_goto_char_menu", "admin_kick_menu", "admin_kill_menu",
	        "admin_ban_menu", "admin_unban_menu" };
	...
	private void handleKill(L2PcInstance activeChar, String player)
	{
		...
		if (target != null)
		{
			...
			{
				target.reduceCurrentHp(target.getMaxHp()
				        * Config.L2JMOD_CHAMPION_HP + 1, activeChar);
			}
			else
			{
	...
	private void teleportCharacter(L2PcInstance player, int x, int y, int z,
	        L2PcInstance activeChar, String message)
	{
		if (player != null)
		{
	...
	private void teleportToCharacter(L2PcInstance activeChar, L2Object target)
	{
		...
		else
		{
			...
			activeChar.sendMessage("You're teleporting yourself to character "
			        + player.getName());
		}
		showMainPage(activeChar);
	}
	...
	private void setAccountAccessLevel(String player, L2PcInstance activeChar,
	        int banLevel)
	{
		...
		try
		{
			...
			if (result.next())
			{
				...
				if (acc_name.length() > 0)
				{
					...
					text = "Account Access Level for " + player + " set to "
					        + banLevel + ".";
				}
				else
				{
