 ...
 * menus - gmliston/gmlistoff = includes/excludes active character from /gmlist
 * results - silence = toggles private messages acceptance mode - diet = toggles
 * weight penalty mode - tradeoff = toggles trade acceptance mode - reload =
 * reloads specified component from multisell|skill|npc|htm|item|instancemanager
 * - set/set_menu/set_mod = alters specified server setting - saveolymp = saves
 * olympiad state manually - manualhero = cycles olympiad and calculate new
 * heroes.
 *
...
public class AdminAdmin implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS = { "admin_admin",
	        "admin_admin1", "admin_admin2", "admin_admin3", "admin_admin4",
	        "admin_admin5", "admin_gmliston", "admin_gmlistoff",
	        "admin_silence", "admin_diet", "admin_tradeoff", "admin_reload",
	        "admin_set", "admin_set_menu", "admin_set_mod", "admin_saveolymp",
	        "admin_manualhero", "admin_sethero", "admin_endolympiad" };
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_endolympiad"))
		{
			...
			activeChar.sendMessage("Heroes formed");
		        ...
		        || command.startsWith("admin_sethero"))
		{
		...
		else if (command.startsWith("admin_set"))
		{
			...
			try
			{
				...
				{
					activeChar.sendMessage("parameter " + pName
					        + " succesfully set to " + pValue);
				}
				else
				{
