 ...
 * current_player - character_list - show_characters - find_character - find_ip
 * - find_account - rec - nokarma - setkarma - settitle - changename - setsex -
 * setclass - fullfood - save_modifications
 *
...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS = { "admin_edit_character",
	        "admin_current_player", "admin_nokarma", // this is to remove karma
	                                                 // from selected char...
			...
			try
			{
				...
				if (valid && (player.getClassId().getId() != classidval))
				{
					...
					player.sendMessage("A GM changed your class to "
					        + newclass);
					player.broadcastUserInfo();
					activeChar.sendMessage(player.getName() + " is a "
					        + newclass);
				}
	...
	private void listCharacters(L2PcInstance activeChar, int page)
	{
		...
		{
			int pagenr = x + 1;
			replyMSG.append("<center><a action=\"bypass -h admin_show_characters "
			        + x + "\">Page " + pagenr + "</a></center>");
		}
	 ...
	 * @param player
	 */
	private void gatherCharacterInfo(L2PcInstance activeChar,
	        L2PcInstance player, String filename)
	{
		String ip = "N/A";
		String account = "N/A";
		try
		...
		adminReply.replace("%percent%", String.valueOf(Util.roundTo(((float) player.getCurrentLoad()
		        / (float) player.getMaxLoad()) * 100, 2)));
	...
	private void setTargetKarma(L2PcInstance activeChar, int newKarma)
	{
		...
		if (newKarma >= 0)
		{
			...
			// Admin information
			activeChar.sendMessage("Successfully Changed karma for "
			        + player.getName() + " from (" + oldKarma + ") to ("
			        + newKarma + ").");
			if (Config.DEBUG)
			{
	        ...
	        String modifications)
	{
		...
		player.sendMessage("Admin has changed your stats." + "  HP: " + hpval
		        + "  MP: " + mpval + "  CP: " + cpval + "  PvP Flag: "
		        + pvpflagval + " PvP/PK " + pvpkillsval + "/" + pkkillsval);
	        ...
	        throws IllegalArgumentException
	{
		...
		{
			adminReply.replace("%number%", " more than "
			        + String.valueOf(CharactersFound));
	 ...
	 * @throws IllegalArgumentException
	 */
	private void findCharactersPerAccount(L2PcInstance activeChar,
	        String characterName) throws IllegalArgumentException
	{
