...
public class AdminEventEngine implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS = { "admin_event",
	        "admin_event_new", "admin_event_choose", "admin_event_store",
	        "admin_event_set", "admin_event_change_teams_number",
	        "admin_event_announce", "admin_event_panel",
	        "admin_event_control_begin", "admin_event_control_teleport",
	        "admin_add", "admin_event_see", "admin_event_del",
	        "admin_delete_buffer", "admin_event_control_sit",
	        "admin_event_name", "admin_event_control_kill",
	        "admin_event_control_res", "admin_event_control_poly",
	        "admin_event_control_unpoly", "admin_event_control_prize",
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_event_see"))
		{
			...
			try (FileInputStream fis = new FileInputStream("data/events/"
			        + eventName);
		...
		else if (command.startsWith("admin_event_store"))
		{
			...
			try
			{
				FileOutputStream file = new FileOutputStream("data/events/"
				        + tempName);
		...
		else if (command.startsWith("admin_event_announce"))
		{
			...
			while (st.hasMoreElements())
			{
				...
				if (!temp2.equals(" "))
				{
					L2Event.names.put(i, temp2.substring(1, temp2.length()
					        - 1));
					i++;
				}
			}
	...
	public void showEventParameters(L2PcInstance activeChar, int teamnumbers)
	{
		...
		replyMSG.append("<br><br>Announcer NPC id<edit var=\"event_npcid\" width=100 height=20><br><br><button value=\"Announce Event!!\" action=\"bypass -h admin_event_announce $event_npcid "
		        + teamnumbers + " ");
		for (int i = 0; i < teamnumbers; i++)
		{
				...
				// synchronized (player.getKnownList().getKnownPlayers())
				{
					for (L2PcInstance playertemp : plrs)
					{
						if ((Math.abs(playertemp.getX() - player.getX()) < 500)
						        && (Math.abs(playertemp.getY()
						                - player.getY()) < 500)
						        && (Math.abs(playertemp.getZ()
						                - player.getZ()) < 500))
						{
							temp.add(playertemp);
						}
	...
	void showEventControl(L2PcInstance activeChar)
	{
		...
		replyMSG.append("<center><font color=\"LEVEL\">[ L2J EVENT ENGINE ]</font></center><br><font color=\"LEVEL\">"
		        + L2Event.eventName + "</font><br><br><table width=200>");
		...
		replyMSG.append("<tr><td>&nbsp;</td></tr>");
		replyMSG.append("<tr><td><button value=\"UnPolymorph\" action=\"bypass -h admin_event_control_unpoly $team_number\" width=90 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td><td><font color=\"LEVEL\">Unpolymorph the team</font></td></tr>");
		replyMSG.append("<tr><td>&nbsp;</td></tr>");
		replyMSG.append("<tr><td>&nbsp;</td></tr>");
