...
public class AdminSkill implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS = { "admin_show_skills",
	        "admin_remove_skills", "admin_skill_list", "admin_skill_index",
	        "admin_add_skill", "admin_remove_skill", "admin_get_skills",
	        "admin_reset_skills", "admin_give_all_skills",
	        "admin_remove_all_skills", "admin_add_clan_skill" };
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_skill_index"))
		{
			try
			{
				...
				AdminHelpPage.showHelpPage(activeChar, "skills/" + val
				        + ".htm");
			}
			catch (StringIndexOutOfBoundsException e)
			{
		...
		else if (command.equals("admin_remove_all_skills"))
		{
			if (activeChar.getTarget() instanceof L2PcInstance)
			{
				...
				activeChar.sendMessage("You removed all skills from "
				        + player.getName());
	...
	private void adminGiveAllSkills(L2PcInstance activeChar)
	{
		...
		activeChar.sendMessage("You gave " + skillCounter + " skills to "
		        + player.getName());
		player.sendSkillList();
	}
	...
	{ // TODO: Externalize HTML
		...
		for (int i = SkillsStart; i < SkillsEnd; i++)
		{
			replyMSG.append("<tr><td width=80><a action=\"bypass -h admin_remove_skill "
			        ...
			        + "</td><td width=40>" + skills[i].getId() + "</td></tr>");
		}
	...
	private void adminGetSkills(L2PcInstance activeChar)
	{
		...
		else
		{
			...
			activeChar.sendMessage("You now have all the skills of "
			        + player.getName() + ".");
	...
	private void adminResetSkills(L2PcInstance activeChar)
	{
		...
		else
		{
			...
			player.sendMessage("[GM]" + activeChar.getName()
			        + " updated your skills.");
	...
	private void adminAddSkill(L2PcInstance activeChar, String val)
	{
		...
		else
		{
			...
			if (skill != null)
			{
				...
				// Admin information
				activeChar.sendMessage("You gave the skill " + name + " to "
				        + player.getName() + ".");
				if (Config.DEBUG)
				{
					_log.fine("[GM]" + activeChar.getName() + " gave skill "
	...
	private void adminRemoveSkill(L2PcInstance activeChar, int idval)
	{
		...
		if (skill != null)
		{
			...
			player.sendMessage("Admin removed the skill " + skillname
			        + " from your skills list.");
			player.removeSkill(skill);
			// Admin information
			activeChar.sendMessage("You removed the skill " + skillname
			        + " from " + player.getName() + ".");
			if (Config.DEBUG)
			{
				_log.fine("[GM]" + activeChar.getName() + " removed skill "
				        + skillname + " from " + player.getName() + ".");
			}
			activeChar.sendSkillList();
		}
		else
	...
	private void adminAddClanSkill(L2PcInstance activeChar, int id, int level)
	{
		...
		else
		{
			...
			if (skill != null)
			{
				...
				activeChar.sendMessage("You gave the Clan Skill: " + skillname
				        + " to the clan " + player.getClan().getName() + ".");
