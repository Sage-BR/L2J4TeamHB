...
/**
 * This class handles following admin commands: - cw_info = displays cursed
 * weapon status - cw_remove = removes a cursed weapon from the world, item id
 * or name must be provided - cw_add = adds a cursed weapon into the world, item
 * id or name must be provided. Target will be the weilder - cw_goto = teleports
 * GM to the specified cursed weapon - cw_reload = reloads instance manager
 *
 * @version $Revision: 1.1.6.3 $ $Date: 2007/07/31 10:06:06 $
 */
public class AdminCursedWeapons implements IAdminCommandHandler
{
	private static final String[] ADMIN_COMMANDS = { "admin_cw_info",
	        "admin_cw_remove", "admin_cw_goto", "admin_cw_reload",
	        "admin_cw_add", "admin_cw_info_menu" };
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		if (command.startsWith("admin_cw_info"))
		{
			if (!command.contains("menu"))
			{
				...
				{
					activeChar.sendMessage("> " + cw.getName() + " ("
					        + cw.getItemId() + ")");
					if (cw.isActivated())
					{
						L2PcInstance pl = cw.getPlayer();
						activeChar.sendMessage("  Player holding: "
						        + pl == null ? "null" : pl.getName());
						activeChar.sendMessage("    Player karma: "
						        + cw.getPlayerKarma());
						activeChar.sendMessage("    Time Remaining: "
						        + (cw.getTimeLeft() / 60000) + " min.");
						activeChar.sendMessage("    Kills : "
						        + cw.getNbKills());
					}
				...
				for (CursedWeapon cw : cwm.getCursedWeapons())
				{
					...
					if (cw.isActivated())
					{
						...
						replyMSG.append("<tr><td>Kills:</td><td>"
						        ...
						        + "</td></tr>");
						replyMSG.append("<tr><td>Time remaining:</td><td>"
