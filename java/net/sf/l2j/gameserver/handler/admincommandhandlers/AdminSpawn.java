...
public class AdminSpawn implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS = { "admin_show_spawns",
	        "admin_spawn", "admin_spawn_monster", "admin_spawn_index",
	        "admin_unspawnall", "admin_respawnall", "admin_spawn_reload",
	        "admin_npc_index", "admin_spawn_once", "admin_show_npcs",
	...
	private void spawnMonster(L2PcInstance activeChar, String monsterId,
	        int respawnTime, int mobCount, boolean permanent)
	{
		...
		try
		{
			...
			{
				activeChar.sendMessage("You cannot spawn another instance of "
				        + template1.name + ".");
			}
			else
			{
				...
				activeChar.sendMessage("Created " + template1.name + " on "
				        + target.getObjectId());
			}
		}
		catch (Exception e)
	...
	private void showMonsters(L2PcInstance activeChar, int level, int from)
	{
		...
		String end1 = "<br><center><button value=\"Next\" action=\"bypass -h admin_spawn_index "
		        + level
		        + " $from$\" width=40 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center></body></html>";
