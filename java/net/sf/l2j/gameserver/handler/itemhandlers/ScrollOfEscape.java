...
public class ScrollOfEscape implements IItemHandler
{
	...
	private static final int[] ITEM_IDS = { 736, 1830, 1829, 1538, 3958, 5858,
	        5859, 7117, 7118, 7119, 7120, 7121, 7122, 7123, 7124, 7125, 7126,
	        7127, 7128, 7129, 7130, 7131, 7132, 7133, 7134, 7135, 7554, 7555,
	        7556, 7557, 7558, 7559, 7618, 7619, 9716, 10129, 10130 };
	...
	public void useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		        ...
		        && !activeChar.isGM())
		{
			...
			// SystemMessage(SystemMessageId.CANNOT_USE_SCROLL_OF_ESCAPE_INSIDE_BOSS_ZONE));
		...
		int escapeSkill = (itemId == 1538 || itemId == 5858 || itemId == 5859
		        || itemId == 3958 || itemId == 10130) ? 2036 : 2013;
		...
		activeChar.setSkillCastEndTime(10 + GameTimeController.getGameTicks()
		        + skill.getHitTime() / GameTimeController.MILLIS_IN_TICK);
	}
		...
		public void run()
		{
			...
			try
			{
				if ((_itemId == 1830 || _itemId == 5859)
				        && CastleManager.getInstance().getCastleByOwner(_activeChar.getClan()) != null) // escape
				                                                                                        // to
				                                                                                        // castle
				                                                                                        // if
				                                                                                        // own's
				...
				else if ((_itemId == 10129 || _itemId == 10130)
				        && FortManager.getInstance().getFortByOwner(_activeChar.getClan()) != null) // escape
				                                                                                    // to
				                                                                                    // fortress
				                                                                                    // if
				                                                                                    // own's
				                                                                                    // one
				{
					_activeChar.teleToLocation(MapRegionTable.TeleportWhereType.Fortress);
				        ...
				        && ClanHallManager.getInstance().getClanHallByOwner(_activeChar.getClan()) != null) // escape
				                                                                                            // to
				                                                                                            // clan
				                                                                                            // hall
				                                                                                            // if
				...
				{
					// _activeChar.sendPacket(new
					// SystemMessage(SystemMessageId.CLAN_HAS_NO_CLAN_HALL));
					...
					else
					{
						switch (_itemId)
						{
							...
							case 7619:
								...
								break;
							case 9716:
								_activeChar.teleToLocation(-117251, 46771, 360, true); // Kamael
								                                                       // Village
								break;
							default:
								_activeChar.teleToLocation(MapRegionTable.TeleportWhereType.Town);
