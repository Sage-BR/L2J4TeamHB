...
public class Potions implements IItemHandler
{
	...
	private static final int[] ITEM_IDS = { 65, 725, 726, 727, 728, 734, 735,
	        1060, 1061, 1073, 1374, 1375, 1539, 4677, 4679, 1540, 5283, 5591,
	        5592, 6035, 6036, 6652, 6553, 6554, 6555, 8193, 8194, 8195, 8196,
	        8197, 8198, 8199, 8200, 8201, 8202, 8600, 8601, 8602, 8603, 8604,
	        8605, 8606, 8607, 8608, 8609, 8610, 8611, 8612, 8613, 8614, 10155,
	        10157,
	        // Attribute Potion
	        ...
	        L2ItemInstance item)
	{
		...
		switch (itemId)
		{
			...
			case 65: // red_potion, xml: 2001
				...
				break;
			case 726: // healing_drug, xml: 2003
			...
			case 727: // _healing_potion, xml: 2032
				...
				break;
			case 728: // _healing_potion, xml: 2005
			...
			case 8627:
			{
				...
				{
					res = usePotion(activeChar, 2287, (expIndex > 5 ? expIndex : expIndex
					        + 1));
				}
				else
				{
			...
			case 8633:
			{
				...
				{
					res = usePotion(activeChar, 2288, (expIndex > 5 ? expIndex : expIndex
					        + 1));
				}
				else
				{
			...
			case 8639:
			{
				...
				{
					res = usePotion(activeChar, 2289, (expIndex > 5 ? expIndex : expIndex
					        + 1));
				}
				else
				{
	...
	private boolean isEffectReplaceable(L2PcInstance activeChar,
	        Enum<?> effectType, int itemId)
	{
		...
		for (L2Effect e : effects)
		{
			if (e.getEffectType() == effectType)
			{
				...
				if (e.getTaskTime() > (e.getSkill().getBuffDuration() * 67)
				        / 100000)
				{
					return true;
				}
	...
	public boolean usePotion(L2PcInstance activeChar, int magicId, int level)
	{
		...
		{
			if (magicId > 2277 && magicId < 2286 && _herbstask >= 100)
			{
				_herbstask -= 100;
			}
