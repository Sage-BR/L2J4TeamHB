...
public class MoveBackwardToLocation extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (_moveMovement == 0 && Config.GEODATA < 1) // cursor movement without
		                                              // geodata is disabled
		{
			activeChar.sendPacket(ActionFailed.STATIC_PACKET);
		}
		else
		{
			...
			{
				_log.info("[MOVE] MoveBackwardToLocation: cur=(" + _curX + ","
				        + _curY + "," + _curZ + ") target=(" + _targetX + ","
				        + _targetY + "," + _targetZ + ") char="
				        + activeChar.getName() + " moving="
				        + activeChar.isMoving());
			}
