...
public final class RequestExEnchantSkillUntrain extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (((trainer == null
		        || !player.isInsideRadius(trainer, L2NpcInstance.INTERACTION_DISTANCE, false, false))
		        && !player.isGM()) || (player.getClassId().level() < 3))
		{ // requires to have 3rd class quest
			// completed
						return;
		}
		...
		if (!skill.canTeachBy(npcid) || !skill.getCanLearn(player.getClassId()))
		{
			if (!Config.ALT_GAME_SKILL_LEARN)
			{
				...
				Util.handleIllegalPlayerAction(player, "Client "
				        + this.getClient()
		...
		if (currentLevel - 1 != _skillLvl
		        && (currentLevel % 100 != 1 || _skillLvl != s.getBaseLevel()))
		{
			return;
		}
		...
		if (player.getSp() >= requiredSp)
		{
			...
			if (player.getExp() >= requiredExp
			        && expAfter >= Experience.LEVEL[player.getLevel()])
			{
				...
				if (Config.DEBUG)
				{
					_log.fine("Learned skill ID: " + _skillId + " Level: "
					        + _skillLvl + " for " + requiredSp + " SP, "
					        + requiredExp + " EXP.");
				}
