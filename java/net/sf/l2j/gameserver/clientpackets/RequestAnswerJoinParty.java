...
public final class RequestAnswerJoinParty extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (_response == 1)
		{
			...
			if (requestor.isInPartyMatchRoom() && player.isInPartyMatchRoom())
			{
				...
				if (list != null
				        && (list.getPlayerRoomId(requestor) == list.getPlayerRoomId(player)))
				{
			...
			else if (requestor.isInPartyMatchRoom()
			        && !player.isInPartyMatchRoom())
			{
				final PartyMatchRoomList list = PartyMatchRoomList.getInstance();
				if (list != null)
				{
		...
		if (requestor.isInParty())
		{
			requestor.getParty().setPendingInvitation(1); // if party is null,
			                                              // there is no need of
			                                              // decreasing
		}
	 ...
	 * (non-Javadoc)
	 *
	 * @see l2jserver.gameserver.clientpackets.ClientBasePacket#getType()
	 */
	@Override
