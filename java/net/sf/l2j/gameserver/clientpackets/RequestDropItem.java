...
public final class RequestDropItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if ((activeChar == null) || !FloodProtector.getInstance().tryPerformAction(activeChar.getObjectId(), FloodProtector.PROTECTED_DROPITEM))
		{
			return;
		}
		...
		if ((_count > item.getCount()) || (Config.PLAYER_SPAWN_PROTECTION > 0 && activeChar.isInvul()
		        && !activeChar.isGM()))
		{
			activeChar.sendPacket(new SystemMessage(SystemMessageId.CANNOT_DISCARD_THIS_ITEM));
			return;
		...
		if (_count < 0)
		{
			Util.handleIllegalPlayerAction(activeChar, "[RequestDropItem] count <= 0! ban! oid: "
			        + _objectId + " owner: "
		...
		{
			Util.handleIllegalPlayerAction(activeChar, "[RequestDropItem] count > 1 but item is not stackable! ban! oid: "
			        + _objectId + " owner: "
			        + activeChar.getName(), IllegalPlayerAction.PUNISH_KICK);
			return;
		}
		...
		if (activeChar.isGM())
		{
			...
			GMAudit.auditGMAction(activeChar.getName(), "Drop", target, "(id: "
			        + dropedItem.getItemId() + " name: "
			        + dropedItem.getItemName() + " objId: "
			        + dropedItem.getObjectId() + " x: " + activeChar.getX()
			        + " y: " + activeChar.getY() + " z: " + activeChar.getZ()
			        + ")");
		}

		if (dropedItem != null && dropedItem.getItemId() == 57
		        && dropedItem.getCount() >= 1000000)
		{
