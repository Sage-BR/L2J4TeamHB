...
public class MultiSellChoose extends L2GameClientPacket
{
	        ...
	        boolean applyTaxes, boolean maintainEnchantment, int enchantment)
	{
		...
		for (MultiSellIngredient e : entry.getIngredients())
		{
			...
			for (MultiSellIngredient ex : _ingredientsList)
			{
				...
				if ((ex.getItemId() == e.getItemId())
				        && (ex.getEnchantmentLevel() == e.getEnchantmentLevel()))
				{
					if ((double) ex.getItemCount()
					        + e.getItemCount() > Integer.MAX_VALUE)
					{
			...
			if (newIng)
			{
				// If there is a maintainIngredient, then we do not need to
				// check the enchantment parameter
		...
		// enchantment is not kept from the client packet, as it may have been
		// forged
		...
		for (MultiSellIngredient e : _ingredientsList)
		{
			...
			if (e.getItemId() != -200)
			{
				...
				if (inv.getInventoryItemCount(e.getItemId(), maintainEnchantment ? e.getEnchantmentLevel() : -1) < ((Config.ALT_BLACKSMITH_USE_RECIPES
				        || !e.getMantainIngredient()) ? (e.getItemCount()
				                * _amount) : e.getItemCount()))
				{
		...
		for (MultiSellIngredient e : entry.getIngredients())
		{
			if (e.getItemId() != -200)
			{
				...
				{// Player trying to buy something from the Multisell store with
				 // an item that's just being used from the Wear option from
				 // merchants.
				...
				if (Config.ALT_BLACKSMITH_USE_RECIPES
				        || !e.getMantainIngredient())
				{
					// if it's a stackable item, just reduce the amount from the
					// first (only) instance that is found in the inventory
					if (itemToTake.isStackable())
					{
						if (!player.destroyItem("Multisell", itemToTake.getObjectId(), (e.getItemCount()
						        * _amount), player.getTarget(), true))
						{
							return;
						}
					}
					else
					{
						...
						{
							// loop through this list and remove (one by one)
						...
						else // b) enchantment is not maintained. Get the
						     // instances with the LOWEST enchantment level
						{
							/*
							 * NOTE: There are 2 ways to achieve the above goal.
							 * 1) Get all items that have the correct itemId,
							 * loop through them until the lowest enchantment
							 * level is found. Repeat all this for the next item
							 * until proper count of items is reached. 2) Get
							 * all items that have the correct itemId, sort them
							 * once based on enchantment level, and get the
							 * range of items that is necessary. Method 1 is
							 * faster for a small number of items to be
							 * exchanged. Method 2 is faster for large amounts.
							 *
							 * EXPLANATION: Worst case scenario for algorithm 1
							 * will make it run in a number of cycles given by:
							 * m*(2n-m+1)/2 where m is the number of items to be
							 * exchanged and n is the total number of inventory
							 * items that have a matching id. With algorithm 2
							 * (sort), sorting takes n*log(n) time and the
							 * choice is done in a single cycle for case b (just
							 * grab the m first items) or in linear time for
							 * case a (find the beginning of items with correct
							 * enchantment, index x, and take all items from x
							 * to x+m). Basically, whenever m > log(n) we have:
							 * m*(2n-m+1)/2 = (2nm-m*m+m)/2 >
							 * (2nlogn-logn*logn+logn)/2 = nlog(n) - log(n*n) +
							 * log(n) = nlog(n) + log(n/n*n) = nlog(n) +
							 * log(1/n) = nlog(n) - log(n) = (n-1)log(n) So for
							 * m < log(n) then m*(2n-m+1)/2 > (n-1)log(n) and
							 * m*(2n-m+1)/2 > nlog(n)
							 *
							 * IDEALLY: In order to best optimize the
							 * performance, choose which algorithm to run, based
							 * on whether 2^m > n if ( (2<<(e.getItemCount() *
							 * _amount)) < inventoryContents.length ) // do
							 * Algorithm 1, no sorting else // do Algorithm 2,
							 * sorting
							 *
							 * CURRENT IMPLEMENTATION: In general, it is going
							 * to be very rare for a person to do a massive
							 * exchange of non-stackable items For this reason,
							 * we assume that algorithm 1 will always suffice
							 * and we keep things simple. If, in the future, it
							 * becomes necessary that we optimize, the above
							 * discussion should make it clear what optimization
							 * exactly is necessary (based on the comments under
							 * "IDEALLY").
							 */

							// choice 1. Small number of items exchanged. No
							// sorting.
							for (int i = 1; i <= (e.getItemCount()
							        * _amount); i++)
							{
			...
			{
				int repCost = player.getClan().getReputationScore()
				        - e.getItemCount();
		...
		for (MultiSellIngredient e : entry.getProducts())
		{
			...
			{
				inv.addItem("Multisell", e.getItemId(), (e.getItemCount()
				        * _amount), player, player.getTarget());
			}
			else
			{
	        ...
	        boolean maintainEnchantment, int enchantLevel)
	{
		...
		for (MultiSellIngredient ing : templateEntry.getIngredients())
		{
			        ...
			        && newIngredient.isTaxIngredient())
			{
				...
				_transactionTax = (int) Math.round(newIngredient.getItemCount()
				        * taxRate);
