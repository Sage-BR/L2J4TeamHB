		...
		switch (_actionId)
		{
			case 0:
				...
				if (Config.DEBUG)
				{
					_log.fine("new wait type: "
					        + (activeChar.isSitting() ? "SITTING" : "STANDING"));
				}
			...
			case 22: // pet attack
				if (target != null && pet != null && pet != target
				        && !pet.isAttackingDisabled() && !pet.isBetrayed())
				{
					if (pet instanceof L2PetInstance
					        && (pet.getLevel() - activeChar.getLevel() > 20))
					{
					...
					if (target.isAutoAttackable(activeChar) || _ctrlPressed)
					{
						if (target instanceof L2DoorInstance)
						{
							if (((L2DoorInstance) target).isAttackable(activeChar)
							        && pet.getNpcId() != L2SiegeSummonInstance.SWOOP_CANNON_ID)
							{
				...
				if (pet != null)
				{
					...
					else
					{
						...
						if (pet instanceof L2PetInstance)
						{
							...
							if (petInst.getCurrentFed() > (petInst.getMaxFed()
							        * 0.40))
							{
			...
			case 38: // pet mount
				...
				if (pet != null && pet.isMountable()
				        && pet.isMountableOverTime() && !activeChar.isMounted()
				        && !activeChar.isBetrayed())
				{
					        ...
					        || activeChar.getPvpFlag() != 0)
					{
						...
						msg = null;
					        ...
					        || activeChar.isInsideZone(L2Character.ZONE_WATER))
					{
	...
	private void useSkill(int skillId, L2Object target)
	{
		...
		if (activeSummon != null && !activeSummon.isBetrayed())
		{
			if (activeSummon instanceof L2PetInstance
			        && (activeSummon.getLevel() - activeChar.getLevel() > 20))
			{
				activeChar.sendPacket(new SystemMessage(SystemMessageId.PET_TOO_HIGH_TO_CONTROL));
				return;
			}
