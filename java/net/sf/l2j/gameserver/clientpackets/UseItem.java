...
public final class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if ((activeChar == null) || !FloodProtector.getInstance().tryPerformAction(activeChar.getObjectId(), FloodProtector.PROTECTED_USEITEM))
		{
			return;
		}
		...
		/*
		 * Alt game - Karma punishment // SOE 736 Scroll of Escape 1538 Blessed
		 * Scroll of Escape 1829 Scroll of Escape: Clan Hall 1830 Scroll of
		 * Escape: Castle 3958 L2Day - Blessed Scroll of Escape 5858 Blessed
		 * Scroll of Escape: Clan Hall 5859 Blessed Scroll of Escape: Castle
		 * 6663 Scroll of Escape: Orc Village 6664 Scroll of Escape: Silenos
		 * Village 7117 Scroll of Escape to Talking Island 7118 Scroll of Escape
		 * to Elven Village 7119 Scroll of Escape to Dark Elf Village 7120
		 * Scroll of Escape to Orc Village 7121 Scroll of Escape to Dwarven
		 * Village 7122 Scroll of Escape to Gludin Village 7123 Scroll of Escape
		 * to the Town of Gludio 7124 Scroll of Escape to the Town of Dion 7125
		 * Scroll of Escape to Floran 7126 Scroll of Escape to Giran Castle Town
		 * 7127 Scroll of Escape to Hardin's Private Academy 7128 Scroll of
		 * Escape to Heine 7129 Scroll of Escape to the Town of Oren 7130 Scroll
		 * of Escape to Ivory Tower 7131 Scroll of Escape to Hunters Village
		 * 7132 Scroll of Escape to Aden Castle Town 7133 Scroll of Escape to
		 * the Town of Goddard 7134 Scroll of Escape to the Rune Township 7135
		 * Scroll of Escape to the Town of Schuttgart. 7554 Scroll of Escape to
		 * Talking Island 7555 Scroll of Escape to Elven Village 7556 Scroll of
		 * Escape to Dark Elf Village 7557 Scroll of Escape to Orc Village 7558
		 * Scroll of Escape to Dwarven Village 7559 Scroll of Escape to Giran
		 * Castle Town 7618 Scroll of Escape - Ketra Orc Village 7619 Scroll of
		 * Escape - Varka Silenos Village 10129 Scroll of Escape : Fortress
		...
		if (!Config.ALT_GAME_KARMA_PLAYER_CAN_TELEPORT
		        ...
		        && (itemId == 736 || itemId == 1538 || itemId == 1829
		                ...
		                || (itemId >= 7554 && itemId <= 7559) || itemId == 7618
		                || itemId == 7619 || itemId == 10129
		                || itemId == 10130))
		{
			return;
		}
		...
		// A shield that can only be used by the members of a clan that owns a
		// castle.
		if ((cl == null || cl.getHasCastle() == 0) && itemId == 7015
		        && Config.CASTLE_SHIELD)
		{
		...
		// Apella armor used by clan members may be worn by a Baron or a higher
		// level Aristocrat.
		if ((itemId >= 7860 && itemId <= 7879) && Config.APELLA_ARMORS
		        && (cl == null || activeChar.getPledgeClass() < 5))
		{
			SystemMessage sm = new SystemMessage(SystemMessageId.CANNOT_EQUIP_ITEM_DUE_TO_BAD_CONDITION);
			activeChar.sendPacket(sm);
			sm = null;
		...
		if ((itemId >= 7850 && itemId <= 7859) && Config.OATH_ARMORS
		        && (cl == null))
		{
			SystemMessage sm = new SystemMessage(SystemMessageId.CANNOT_EQUIP_ITEM_DUE_TO_BAD_CONDITION);
			activeChar.sendPacket(sm);
		...
		if (itemId == 6841 && Config.CASTLE_CROWN && (cl == null
		        || (cl.getHasCastle() == 0 || !activeChar.isClanLeader())))
		{
			SystemMessage sm = new SystemMessage(SystemMessageId.CANNOT_EQUIP_ITEM_DUE_TO_BAD_CONDITION);
			activeChar.sendPacket(sm);
			sm = null;
		...
		if (Config.CASTLE_CIRCLETS && ((itemId >= 6834 && itemId <= 6840)
		        || itemId == 8182 || itemId == 8183))
		{
			if (cl == null)
			{
				SystemMessage sm = new SystemMessage(SystemMessageId.CANNOT_EQUIP_ITEM_DUE_TO_BAD_CONDITION);
		...
		if (item.getItem().isForWolf() || item.getItem().isForGreatWolf()
		        || item.getItem().isForHatchling()
		        || item.getItem().isForStrider()
		        || item.getItem().isForBabyPet())
		{
			SystemMessage sm = new SystemMessage(SystemMessageId.CANNOT_EQUIP_PET_ITEM); // You
			                                                                             // cannot
			                                                                             // equip
			                                                                             // a
			                                                                             // pet
		...
		if (item.isEquipable())
		{
			...
			switch (item.getItem().getBodyPart())
			{
				...
				case L2Item.SLOT_R_HAND:
				{
					...
					if (!item.isEquipped() && item.getItem() instanceof L2Weapon
					        && !activeChar.isGM())
					{
				...
				{
					SystemMessage sm = new SystemMessage(SystemMessageId.CANNOT_EQUIP_ITEM_DUE_TO_BAD_CONDITION);
					if ((item.getItemId() >= 9605 && item.getItemId() <= 9615)
					        || item.getItemId() == 10018)
					{
			...
			if (activeChar.isCursedWeaponEquipped() && itemId == 6408) // Don't
			                                                           // allow
			                                                           // to put
			                                                           // formal
			                                                           // wear
