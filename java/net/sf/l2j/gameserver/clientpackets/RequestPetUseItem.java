...
public final class RequestPetUseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (Config.DEBUG)
		{
			_log.finest(activeChar.getObjectId() + ": pet use item "
			        + _objectId);
		}
		...
		if (item.isEquipable())
		{
			...
			else if (L2PetDataTable.isWGreatWolf(pet.getNpcId()) && // White
			                                                        // Greatwolf
			        item.getItem().isForGreatWolf())
			{
				useItem(pet, item, activeChar);
			...
			else if (L2PetDataTable.isWFenrirWolf(pet.getNpcId()) && // white
			                                                         // fenrir
			        item.getItem().isForGreatWolf())
			{
				useItem(pet, item, activeChar);
			...
			else if (L2PetDataTable.isBaby(pet.getNpcId()) && // baby pets
			                                                  // (buffalo,
			                                                  // cougar,
			                                                  // kookaboora)
			...
			else if (L2PetDataTable.isImprovedBaby(pet.getNpcId()) && // Improved
			                                                          // baby
			                                                          // pets
			                                                          // (buffalo,
			                                                          // cougar,
		...
		else if (L2PetDataTable.isPetFood(itemId))
		{
			if ((L2PetDataTable.isWolf(pet.getNpcId())
			        && L2PetDataTable.isWolfFood(itemId)) || (L2PetDataTable.isGreatWolf(pet.getNpcId())
			        && L2PetDataTable.isGreatWolfFood(itemId)))
			{
				feed(activeChar, pet, item);
				return;
			}
			if (L2PetDataTable.isWGreatWolf(pet.getNpcId())
			        && L2PetDataTable.isWGreatWolfFood(itemId))
			{
				feed(activeChar, pet, item);
				return;
			}
			if (L2PetDataTable.isBlackWolf(pet.getNpcId())
			        && L2PetDataTable.isBlackWolfFood(itemId))
			{
				feed(activeChar, pet, item);
				return;
			}
			if (L2PetDataTable.isFenrirWolf(pet.getNpcId())
			        && L2PetDataTable.isFenrirWolfFood(itemId))
			{
				feed(activeChar, pet, item);
				return;
			}
			if (L2PetDataTable.isWFenrirWolf(pet.getNpcId())
			        && L2PetDataTable.isWFenrirWolfFood(itemId))
			{
				feed(activeChar, pet, item);
				return;
			}
			if (L2PetDataTable.isSinEater(pet.getNpcId())
			        && L2PetDataTable.isSinEaterFood(itemId))
			{
				feed(activeChar, pet, item);
				return;
			}
			...
			else if (L2PetDataTable.isImprovedBaby(pet.getNpcId())
			        && L2PetDataTable.isImprovedBabyFood(itemId))
			{
				feed(activeChar, pet, item);
				return;
			}
