			...
			switch (_requestedPointType)
			{
				case 1: // to clanhall
					if (activeChar.getClan() == null
					        || activeChar.getClan().getHasHideout() == 0)
					{
						_log.warning("Player [" + activeChar.getName()
				...
				case 3: // to fortress
					...
					if ((activeChar.getClan() == null
					        || activeChar.getClan().getHasFort() == 0)
					        && !isInDefense)
					{
						_log.warning("Player [" + activeChar.getName()
				...
				case 5: // Fixed or Player is a festival participant
					...
					{
						_log.warning("Player [" + activeChar.getName()
						        + "] called RestartPointPacket - Fixed and he isn't festival participant!");
						return;
					}
					loc = new Location(activeChar.getX(), activeChar.getY(), activeChar.getZ()); // spawn
					                                                                             // them
					                                                                             // where
					                                                                             // they
					                                                                             // died
	...
	protected void runImpl()
	{
		...
		if (castle != null && castle.getSiege().getIsInProgress())
		{
			        ...
			        && castle.getSiege().checkIsAttacker(activeChar.getClan()))
			{
				...
				{
					activeChar.sendMessage("You will be re-spawned in "
