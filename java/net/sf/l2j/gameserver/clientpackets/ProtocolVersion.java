...
public final class ProtocolVersion extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (_version < Config.MIN_PROTOCOL_REVISION
		        || _version > Config.MAX_PROTOCOL_REVISION)
		{
			_log.info("Client: " + getClient().toString()
			        + " -> Protocol Revision: " + _version + " is invalid.");
			getClient().closeNow();
			return;
		}
		...
		if (ConfigProtection.ALLOW_GUARD_SYSTEM)
		{
			...
			{
				_log.warning("HWID payload not found in ProtocolVersion from "
				        + getClient().toString());
				getClient().closeNow();
				return;
			}
			...
			if (parts.length < 4)
			{
				_log.warning("Invalid HWID payload format from "
				        + getClient().toString());
				getClient().closeNow();
				return;
			}
			...
			{
				_log.warning("HWID INVALID - Connection blocked for "
				        + getClient().toString());
				getClient().closeNow();
				return;
			}
