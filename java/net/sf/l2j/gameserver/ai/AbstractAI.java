...
abstract class AbstractAI implements Ctrl
{
	...
	class FollowTask implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				// (checkZ=false) for the actual "am I in range" gate. The
				// anti-teleport
				// gate 3000 stays 3D to catch large Z-only teleports. Using 3D
				// for the
				// range gate previously caused a mismatch vs thinkAttack()'s 2D
				// check
				// that left the FollowTask broadcasting MoveToPawn during the
				// attack
				// swing even though thinkAttack considered the actor "in
				// range".
				if (!_actor.isInsideRadius(followTarget, _range, false, false))
				{
					...
					// Do not start movement while the actor is in the middle of
					// an attack
					// swing. Broadcasting MoveToPawn during the attack
					// animation would
					// cancel the client-side swing. _attackTimeToMove now
					// covers the
					// full swing + reuse window (set in doAttack), so this is
					// the
					// secondary safety net — the primary is thinkAttack()
					// stopping the
	 ...
	 * ThreadPoolManager.getInstance().executeAi(new InformAIMsg(this, intent,
	 * arg0)); }
	 *
	 ...
	 * null)); }
	 *
	 * public class InformAIMsg implements Runnable { private AbstractAI _ai;
	 * private CtrlIntention _intent; private Object _arg0; public
	 * InformAIMsg(AbstractAI ai, CtrlIntention intention, Object arg0) {
	 * _ai=ai; _intent = intention; _arg0 = arg0; } public final void run() {
	 ...
	 * attack again after the stunned period)</B></FONT><BR>
	 * <BR>
	 *
	 * @param evt
	 ...
	 * ThreadPoolManager.getInstance().executeAi(new InformAIEvent(this, evt,
	 * null, null)); }
	 *
	 ...
	 * arg0, arg1)); }
	 *
	 * public class InformAIEvent implements Runnable { private AbstractAI _ai;
	 * private CtrlEvent _evt; private Object _arg0, _arg1;
	 *
	 * public InformAIEvent(AbstractAI ai, CtrlEvent evt, Object arg0, Object
	 * arg1) { _ai=ai; _evt = evt; _arg0 = arg0; _arg1 = arg1; }
	 *
	 * public final void run() { _ai.notifyEvent(_evt, _arg0, _arg1); } }
	 */
	/**
	 * Launch the L2CharacterAI onEvt method corresponding to the Event.<BR>
	 * <BR>
	...
	public void moveToPawn(L2Object pawn, int offset)
	{
		...
		if (!_actor.isMovementDisabled())
		{
			...
			// If can't see target, move directly without offset (L2J Maneco2
			// pattern).
