...
public class L2CharacterAI extends AbstractAI
{
	...
	{
		if ((getIntention() == AI_INTENTION_REST && skill.isMagic()) || _actor.isAllSkillsDisabled())
		{
			// Cancel action client side by sending Server->Client packet
			// ActionFailed to the L2PcInstance actor
			clientActionFailed();
	...
	{
		if ((getIntention() == AI_INTENTION_REST) || _actor.isAllSkillsDisabled() || _actor.isImmobilized() || _actor.isRooted())
		{
			// Cancel action client side by sending Server->Client packet
			// ActionFailed to the L2PcInstance actor
			clientActionFailed();
	...
	protected void onIntentionPickUp(L2Object object)
	{
		                                              ...
		                                              // drop&spawn bug
		{
			...
			object.setXYZ(getActor().getX(), getActor().getY(), getActor().getZ()
			        + 5);
		}
	...
	protected boolean maybeMoveToPosition(Point3D worldPosition, int offset)
	{
		...
		if (!_actor.isInsideRadius(worldPosition.getX(), worldPosition.getY(), offset
		        + _actor.getTemplate().collisionRadius, false))
		{
	...
	protected boolean maybeMoveToPawn(L2Object target, int offset)
	{
		...
		{
			// Caller should be L2Playable and
			// thinkAttack/thinkCast/thinkInteract/thinkPickUp
			if ((getFollowTarget() != null) || _actor.isMovementDisabled())
			{
				return true;
			}
		...
		public void init()
		{
			...
			for (L2Skill sk : _actor.getAllSkills())
			{
				...
				switch (sk.getSkillType())
				{
					...
					case PARALYZE:
					case STUN:
						// hardcoding petrification until improvements are made
						// to
						// EffectTemplate... petrification is totally different
						// for
						// AI than paralyze
						switch (sk.getId())
						{
			...
			// Because of missing skills, some mages/balanced cannot play like
			// mages
