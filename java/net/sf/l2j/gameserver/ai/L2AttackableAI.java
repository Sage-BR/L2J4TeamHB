...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target.isInvul())
		{
			...
			if ((target instanceof L2PcInstance
			        && ((L2PcInstance) target).isGM()) || (target instanceof L2Summon
	...
	// same height
		...
		if (target instanceof L2PlayableInstance)
		{
			...
			if (!(me instanceof L2RaidBossInstance)
			        && ((L2PlayableInstance) target).isSilentMoving())
			{
				return false;
			}
		}
		...
		if (target instanceof L2PcInstance)
		{
			...
			if ((((L2PcInstance) target).isGM()
			        && !((L2PcInstance) target).getAccessLevel().canTakeAggro()) || (me.getFactionId() == "varka"
			...
			if ((me.getFactionId() == "ketra"
			        && ((L2PcInstance) target).isAlliedWithKetra()) || ((L2PcInstance) target).isRecentFakeDeath())
			{
				return false;
			}
		...
		if (target instanceof L2Summon)
		{
			...
			if (owner != null)
			{
				...
				if (owner.isGM() && (owner.isInvul()
				        || !owner.getAccessLevel().canTakeAggro()))
				{
					return false;
				}
				// Check if player is an ally (comparing mem addr)
				if ((me.getFactionId() == "varka" && owner.isAlliedWithVarka()) || (me.getFactionId() == "ketra" && owner.isAlliedWithKetra()))
				{
					return false;
				}
			}
		...
		{ // The actor is a L2MonsterInstance
			...
			// Check if the target isn't another L2NpcInstance
			// depending on config, do not allow mobs to attack _new_ players in
			// peacezones,
			// unless they are already following those players from outside the
			// peacezone.
			if ((target instanceof L2NpcInstance) || (!Config.ALT_MOB_AGRO_IN_PEACEZONE
	...
	protected void onIntentionAttack(L2Character target)
	{
		...
		// self and buffs
		if (_selfAnalysis.lastBuffTick
		        + 100 < GameTimeController.getGameTicks())
		{
			for (L2Skill sk : _selfAnalysis.buffSkills)
			{
				...
				{
					// no clan buffs here?
					if ((_actor.getCurrentMp() < sk.getMpConsume()) || _actor.isSkillDisabled(sk.getId()) || (sk.getTargetType() == L2Skill.SkillTargetType.TARGET_CLAN))
					{
						continue;
					}
	...
	private void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			...
			// synchronized (npc.getKnownList().getKnownObjects())
			{
				for (L2Object obj : objs)
				{
					 ...
					 * ON_AGGRO_RANGE_ENTER) !=null) for (Quest quest:
					 * npc.getTemplate().getEventQuests(Quest.QuestEventType.
		        ...
		        && ((L2MinionInstance) _actor).getLeader() != null)
		{
			...
			if (_actor.getPlanDistanceSq(((L2MinionInstance) _actor).getLeader()) > offset
			        * offset)
			{
				int x1, y1, z1;
				...
				for (L2Skill sk : _selfAnalysis.buffSkills)
				{
					if (_actor.getFirstEffect(sk.getId()) == null)
					{
						...
						if ((sk.getTargetType() != L2Skill.SkillTargetType.TARGET_SELF
						        && Rnd.nextInt(2) != 0) || (_actor.getCurrentMp() < sk.getMpConsume()) || _actor.isSkillDisabled(sk.getId()))
						{
							continue;
						}
						L2Object OldTarget = _actor.getTarget();
	...
	private void thinkAttack()
	{
		...
		if (originalAttackTarget == null || originalAttackTarget.isAlikeDead()
		        || (originalAttackTarget instanceof L2PcInstance
		                && (((L2PcInstance) originalAttackTarget).isInOfflineMode()
		                        || ((L2PcInstance) originalAttackTarget).isOnline() == 0))
					...
					if (obj instanceof L2NpcInstance)
					{
						        ...
						        && npc.getAI() != null)
						{
							...
							if (_selfAnalysis.hasHealOrResurrect
							        ...
							        && _actor.getCurrentHp() > _actor.getMaxHp()
							                / 2
							...
							)
							{
								        ...
								        && _actor instanceof L2MinionInstance)
								{
									if (((L2MinionInstance) _actor).getLeader() == npc)
									{
										...
										{
											if ((_actor.getCurrentMp() < sk.getMpConsume()) || _actor.isSkillDisabled(sk.getId())
		        ...
		        && _secondMostHatedAnalysis.character != null)
		{
			if (Util.checkIfInRange(900, _actor, hated.get(1), true))
			{
				...
				((L2Attackable) _actor).reduceHate(hated.get(0), 2
				        * (((L2Attackable) _actor).getHating(hated.get(0))
				                - ((L2Attackable) _actor).getHating(hated.get(1))));
				// Calculate a new attack timeout
				_attackTimeout = MAX_ATTACK_TIMEOUT
				        + GameTimeController.getGameTicks();
			}
		...
		if (_actor.isRooted() && _secondMostHatedAnalysis.character != null)
		{
			if (_selfAnalysis.isMage
			        ...
			        && _actor.getPlanDistanceSq(_secondMostHatedAnalysis.character.getX(), _secondMostHatedAnalysis.character.getY()) < _selfAnalysis.maxCastRange
			                * _selfAnalysis.maxCastRange)
			{
				((L2Attackable) _actor).reduceHate(hated.get(0), 1
				        + (((L2Attackable) _actor).getHating(hated.get(0))
				                - ((L2Attackable) _actor).getHating(hated.get(1))));
			}
			else if (dist2 > range * range
			        && _actor.getPlanDistanceSq(_secondMostHatedAnalysis.character.getX(), _secondMostHatedAnalysis.character.getY()) < range
			                * range)
			{
				((L2Attackable) _actor).reduceHate(hated.get(0), 1
				        + (((L2Attackable) _actor).getHating(hated.get(0))
		...
		if ((dist2 < 10000 + combinedCollision * combinedCollision)
		        ...
		        && (_mostHatedAnalysis.isBalanced
		                || _mostHatedAnalysis.isFighter)
		        && (_mostHatedAnalysis.character.isRooted()
		                || _mostHatedAnalysis.isSlower)
		...
		if (_mostHatedAnalysis.character.isMoving() || _actor.isMoving())
		{
			range += 50;
		}
		...
		{
			if (!_actor.isMuted() && (_selfAnalysis.hasLongRangeSkills
			        || !_selfAnalysis.healSkills.isEmpty()))
			{
				...
				if (this._selfAnalysis.lastDebuffTick
				        + 60 < GameTimeController.getGameTicks())
				{
				...
				if (_secondMostHatedAnalysis.character != null
				        ...
				        && (_secondMostHatedAnalysis.isMage
				                || _secondMostHatedAnalysis.isBalanced))
				{
				...
				if (!_mostHatedAnalysis.character.isAttackingDisabled())
				{
					for (L2Skill sk : _selfAnalysis.generalDisablers)
					{
						...
						if (Rnd.nextInt(100) <= ((_selfAnalysis.isFighter
						        && _actor.isRooted()) ? 15 : 7))
						{
							clientStopMoving(null);
							_accessor.doCast(sk);
							_attackTimeout = MAX_ATTACK_TIMEOUT
		...
		else
		{
			...
			if (Rnd.nextInt(100) <= 33) // check it once per 3 seconds
			{
				for (L2Object nearby : _actor.getKnownList().getKnownCharactersInRadius(10))
				{
					...
					{
						int diffx = Rnd.get(combinedCollision, combinedCollision
						        + 40);
						if (Rnd.get(10) < 5)
						{
							diffx = -diffx;
						}
						int diffy = Rnd.get(combinedCollision, combinedCollision
						        + 40);
						if (Rnd.get(10) < 5)
						{
							diffy = -diffy;
						}
						moveTo(_mostHatedAnalysis.character.getX()
						        + diffx, _mostHatedAnalysis.character.getY()
