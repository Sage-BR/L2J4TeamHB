...
public class L2PlayerAI extends L2CharacterAI
{
	...
	// private Stack<IntentionCommand> _interruptedIntentions = new
	// Stack<IntentionCommand>();
	...
	void saveNextIntention(CtrlIntention intention, Object arg0, Object arg1)
	{
		/*
		 * if (Config.DEBUG)
		 * _log.warning("L2PlayerAI: changeIntention -> Saving next intention: "
	        ...
	        Object arg1)
	{
		...
		if (intention != AI_INTENTION_CAST
		        || (arg0 != null && ((L2Skill) arg0).isOffensive()))
		{
			_nextIntention = null;
		...
		if (intention == _intention && arg0 == _intentionArg0
		        && arg1 == _intentionArg1)
		{
			super.changeIntention(intention, arg0, arg1);
			return;
	 ...
	 * <BR>
	 *
	 * When intention is ATTACK, calls thinkAttack() directly to bypass the
	 * _thinking flag guard in onEvtThink(), preventing a ~2s delay on
	 * attack-start after arriving at a pawn (Revellion pattern).
	 */
	@Override
	...
	protected void onEvtFinishCasting()
	{
		if (getIntention() == AI_INTENTION_CAST)
		{
			...
			if (_nextIntention != null)
			{
				if (_nextIntention._crtlIntention != AI_INTENTION_CAST) // previous
				                                                        // state
				                                                        // shouldn't
				                                                        // be
				                                                        // casting
	...
	protected void onIntentionMoveTo(L2CharPosition pos)
	{
		...
		{
			// During melee wind-up the swing animation must finish client-side,
			// otherwise
			// movement would cancel the visual. Queue the move intent for after
			// the hit.
			// Once isAttackingNow() returns false (backswing), movement is
			// fully free for kiting.
			clientActionFailed();
	...
	private void thinkAttack()
	{
		...
		// This mirrors Brproject thinkAttack: once follow is engaged, the
		// FollowTask
		// (running every 500ms) drives positioning; the AI layer here just
		// waits and
		// re-evaluates on the next think tick. This avoids competing MoveToPawn
		// broadcasts vs attack-broadcasts that manifest as client-side
		// micro-teleports.
		...
		// MoveToPawn packets.
		// Brproject pattern: range comparison is 2D (checkZ=false), consistent
		// with the
		// main range gate above. _attackTimeToMove covering the full swing (set
		// in
		// doAttack) prevents the FollowTask from broadcasting MoveToPawn during
		// attack.
		...
		// Final range check defensive layer (in case client/server position
		// drifted)
		if (!_actor.isInsideRadius(target, totalRange, false, false))
		{
			clientActionFailed();
	...
	private void thinkCast()
	{
		...
		if (_skill.getTargetType() == SkillTargetType.TARGET_GROUND
		        && _actor instanceof L2PcInstance)
		{
		...
		else
		{
			...
			if (target != null
			        && maybeMoveToPawn(target, _actor.getMagicalAttackRange(_skill)))
			{
				return;
			}
		}
