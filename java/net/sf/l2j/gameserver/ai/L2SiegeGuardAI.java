...
public class L2SiegeGuardAI extends L2CharacterAI implements Runnable
{
	...
	private final void factionNotifyAndSupport()
	{
		...
		// for (L2Character cha :
		// _actor.getKnownList().getKnownCharactersInRadius(((L2NpcInstance)
		// _actor).getFactionRange()+_actor.getTemplate().collisionRadius))
		for (L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(1000))
		{
			...
			if (!(cha instanceof L2NpcInstance))
			{
				        ...
				        && ((L2NpcInstance) _actor).getCastle().getSiege().checkIsDefender(((L2PcInstance) cha).getClan()))
				{
					        ...
					        && cha.isInCombat())
					{
						...
						{
							if ((_actor.getCurrentMp() < sk.getMpConsume()) || _actor.isSkillDisabled(sk.getId()) || !Util.checkIfInRange(sk.getCastRange(), _actor, cha, true))
							{
								continue;
							}
			...
			if (npc.getAI() != null) // TODO: possibly check not needed
			{
				        ...
				        && npc.isInCombat())
				{
					for (L2Skill sk : _selfAnalysis.healSkills)
					{
						if ((_actor.getCurrentMp() < sk.getMpConsume()) || _actor.isSkillDisabled(sk.getId()) || !Util.checkIfInRange(sk.getCastRange(), _actor, npc, true))
						{
							continue;
						}
	...
	private void attackPrepare()
	{
		...
		if ((_attackTarget instanceof L2PcInstance
		        && sGuard.getCastle().getSiege().checkIsDefender(((L2PcInstance) _attackTarget).getClan())) || !GeoData.getInstance().canSeeTarget(_actor, _attackTarget))
		{
		...
		if (!_actor.isMuted() && dist_2 > range * range)
		{
			...
			for (L2Skill sk : skills)
			{
				...
				if ((dist_2 <= castRange * castRange) && castRange > 70
				        ...
				        && !sk.isPassive())
				{
					        ...
					        || sk.getSkillType() == L2Skill.SkillType.HEAL)
					{
						...
						if (sk.getSkillType() == L2Skill.SkillType.HEAL
						        && _actor.getCurrentHp() > (int) (_actor.getMaxHp()
						                / 1.5))
						{
							useSkillSelf = false;
							break;
			...
			else
			{
				...
				double homeX = _attackTarget.getX()
				        - sGuard.getSpawn().getLocx();
				double homeY = _attackTarget.getY()
				        - sGuard.getSpawn().getLocy();
				     ...
				     // sending Server->Client packet MoveToPawn (broadcast)
				{
					...
					// before replacing this with effective geodata checks and
					// AI modification
