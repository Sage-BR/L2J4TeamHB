...
public class L2ControllableMobAI extends L2AttackableAI
{
	...
	protected void thinkFollow()
	{
		...
		if (!Util.checkIfInRange(MobGroupTable.FOLLOW_RANGE, me, getForcedTarget(), true))
		{
			...
			moveTo(getForcedTarget().getX()
			        + signX * randX, getForcedTarget().getY()
			                + signY * randY, getForcedTarget().getZ());
		}
	}
	...
	protected void thinkForceAttack()
	{
		...
		try
		{
			...
			range = _actor.getPhysicalAttackRange()
			        + _actor.getTemplate().collisionRadius
			        + getForcedTarget().getTemplate().collisionRadius;
			max_range = range;
		}
		catch (NullPointerException e)
		{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target.isAlikeDead()
		        ...
		        || Math.abs(_actor.getZ() - target.getZ()) > 100 || target.isInvul())
		{
			return false;
		}
