 ...
 * @author JOJO Update by rocknow Updated by L2jOff team Updated by L2jFrozen
 *         team <BR>
 *         Warn: be careful with adding new spawns {@link #getXFix(int)}
 */
public class Frintezza extends Quest implements Runnable
{
	private static final int[][] _invadeLoc = { { 174102, -76039, -5105 },
	        { 173235, -76884, -5105 }, { 175003, -76933, -5105 },
	        { 174196, -76190, -5105 }, { 174013, -76120, -5105 },
	        { 173263, -75161, -5105 } };

	private static final int[][] _skill = { { 5015, 1, 5000 },
	        { 5015, 4, 5000 }, { 5015, 2, 5000 }, { 5015, 5, 5000 },
	        { 5018, 1, 10000 }, { 5016, 1, 5000 }, { 5015, 3, 5000 },
	        { 5015, 6, 5000 }, { 5018, 2, 10000 }, { 5019, 1, 10000 },
	        { 5016, 1, 5000 } };
	...
	private static final byte DORMANT = 0; // Frintezza is spawned and no one
	                                       // has entered yet. Entry is unlocked

	private static final byte WAITING = 1; // Frintezza is spawend and someone
	                                       // has entered, triggering a 30
	                                       // minute window for additional
	                                       // people to enter
	// before he unleashes his attack. Entry is unlocked

	private static final byte FIGHTING = 2; // Frintezza is engaged in battle,
	                                        // annihilating his foes. Entry is
	                                        // locked

	private static final byte DEAD = 3; // Frintezza has been killed. Entry is
	                                    // locked
	...
	private L2MonsterInstance demon1, demon2, demon3, demon4, portrait1,
	        portrait2, portrait3, portrait4;

	private L2NpcInstance _frintezzaDummy, _overheadDummy, _portraitDummy1,
	        _portraitDummy3, _scarletDummy;
	...
	{
		super(id, name, descr);
		final int[] mob = { SCARLET1, SCARLET2, FRINTEZZA, 18328, 18329, 18330,
		...
		// for (int i = 0; i < 8; i++)
		// DoorTable.getInstance().getDoor(25150051 + i).setRange(0, 0, 0, 0, 0,
		// 0);
		}
	}
	        ...
	        final L2PcInstance player)
	{
		...
		/*
		 * else if (event.equalsIgnoreCase("loc_check")) { Integer status =
		 * GrandBossManager.getInstance().getBossStatus(FRINTEZZA); if (status
		 * == FIGHTING) { if (!_Zone.isInsideZone(npc))
		 * npc.teleToLocation(getXFix(174232),getYFix(-88020),getZFix(-5116));
		 * if (npc.getX() < getXFix(171932) || npc.getX() > getXFix(176532) ||
		 * npc.getY() < getYFix(-90320) || npc.getY() > getYFix(-85720) ||
		 * npc.getZ() < getZFix(-5130))
		 * npc.teleToLocation(getXFix(174232),getYFix(-88020),getZFix(-5116)); }
		 * }
		 */
		...
		else if (event.equalsIgnoreCase("camera_22"))
		{
			...
			startQuestTimer("songs_play", 10000
			        + Rnd.get(10000), frintezza, null);
			startQuestTimer("skill01", 10000
			        + Rnd.get(10000), weakScarlet, null);
		}
		...
		else if (event.equalsIgnoreCase("morph_15"))
		{
			...
			startQuestTimer("songs_play", 10000
			        + Rnd.get(10000), frintezza, null);
			startQuestTimer("skill03", 10000
			        + Rnd.get(10000), strongScarlet, null);
		}
		else if (event.equalsIgnoreCase("morph_16"))
		{
			_Zone.broadcastPacket(new SpecialCamera(strongScarlet.getObjectId(), 300, _Angle
			        - 180, 5, 0, 7000));
		...
		else if (event.equalsIgnoreCase("songs_play"))
		{
			if (frintezza != null && !frintezza.isDead() && _OnMorph == 0)
			{
				...
				// Name of the songs are custom, named with client side
				// description.
				switch (_OnSong)
				{
					case 1:
				...
				if (_OnSong == 1 && _ThirdMorph == 1
				        && strongScarlet.getCurrentHp() < strongScarlet.getMaxHp()
				                * 0.6
				        && Rnd.get(100) < 80)
				{
					...
					startQuestTimer("songs_play", 32000
					        + Rnd.get(10000), frintezza, null);
				}
				...
				else if (_OnSong == 4 && _SecondMorph == 1)
				{
					...
					startQuestTimer("songs_effect", 5000, frintezza, null);
					startQuestTimer("songs_play", 31000
					        + Rnd.get(10000), frintezza, null);
				}
				else if (_OnSong == 5 && _ThirdMorph == 1 && _Abnormal == 0)
				{
					...
					startQuestTimer("songs_effect", 5000, frintezza, null);
					startQuestTimer("songs_play", 35000
					        + Rnd.get(10000), frintezza, null);
				}
				else
				{
					startQuestTimer("songs_play", 5000
					        + Rnd.get(5000), frintezza, null);
				}
			}
		}
		else if (event.equalsIgnoreCase("songs_effect"))
		{
			...
			{
				/*
				 * TODO: Frintezza songs to be fixed like retail if (frintezza
				 * != null && !frintezza.isDead() && activeScarlet != null &&
				 * !activeScarlet.isDead()) skill.getEffects(frintezza,
				 * activeScarlet, false, false, false);
				 */
			}
			else if (_OnSong == 4)
			{
		...
		else if (event.equalsIgnoreCase("skill01"))
		{
			if (weakScarlet != null && !weakScarlet.isDead()
			        && _SecondMorph == 0 && _ThirdMorph == 0 && _OnMorph == 0)
			{
				...
				startQuestTimer("skill01", _skill[i][2] + 5000
				        + Rnd.get(10000), npc, null);
			}
		}
		else if (event.equalsIgnoreCase("skill02"))
		{
			if (weakScarlet != null && !weakScarlet.isDead()
			        && _SecondMorph == 1 && _ThirdMorph == 0 && _OnMorph == 0)
			{
				int i = 0;
				if (_Abnormal == 0)
				{
				...
				startQuestTimer("skill02", _skill[i][2] + 5000
				        + Rnd.get(10000), npc, null);
		...
		else if (event.equalsIgnoreCase("skill03"))
		{
			if (strongScarlet != null && !strongScarlet.isDead()
			        && _SecondMorph == 1 && _ThirdMorph == 1 && _OnMorph == 0)
			{
				int i = 0;
				if (_Abnormal == 0)
				{
				...
				startQuestTimer("skill03", _skill[i][2] + 5000
				        + Rnd.get(10000), npc, null);
		...
		{
			if (npc != null && !npc.isDead() && frintezza != null
			        && !frintezza.isDead())
			{
				final L2NpcInstance mob = addSpawn(npc.getNpcId()
				        + 2, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), false, 0);
	...
	public String onTalk(final L2NpcInstance npc, final L2PcInstance player)
	{
		...
		else if (status == DORMANT)
		{
			...
			if (!Config.BYPASS_FRINTEZZA_PARTIES_CHECK)
			{
				...
				else if (player.getParty().getCommandChannel().getPartys().size() < Config.FRINTEZZA_MIN_PARTIES
				        || player.getParty().getCommandChannel().getPartys().size() > Config.FRINTEZZA_MAX_PARTIES)
				{
					htmltext = "<html><body>Your command channel needs to have at least "
					        + Config.FRINTEZZA_MIN_PARTIES
					        + " parties and a maximum of "
					        + Config.FRINTEZZA_MAX_PARTIES + ".</body></html>";
					party_check_success = false;
				}
					...
					if (Config.BYPASS_FRINTEZZA_PARTIES_CHECK)
					{
							...
							{ // teleport all parties into CC
								...
								for (final L2Party party : CC.getPartys())
								{
									...
									synchronized (_PlayersInside)
									{
										...
										{
											if (member == null
											        || member.getLevel() < 74 || !member.isInsideRadius(npc, 700, false, false))
											{
												continue;
											}
											...
											member.teleToLocation(getXFix(_invadeLoc[_LocCycle][0])
											        + Rnd.get(50), getYFix(_invadeLoc[_LocCycle][1])
											                + Rnd.get(50), getZFix(_invadeLoc[_LocCycle][2]));
										}
										if (_PlayersInside.size() > 45)
										{
											break;
						...
						{ // teleport just player
							...
							if (player.isInsideRadius(npc, 700, false, false))
							{
								...
								player.teleToLocation(getXFix(_invadeLoc[_LocCycle][0])
								        + Rnd.get(50), getYFix(_invadeLoc[_LocCycle][1])
								                + Rnd.get(50), getZFix(_invadeLoc[_LocCycle][2]));
	        ...
	        final int damage, final boolean isPet)
	{
		...
		if (npc.getNpcId() == SCARLET1 && _SecondMorph == 0 && _ThirdMorph == 0
		        && _OnMorph == 0 && npc.getCurrentHp() < npc.getMaxHp() * 0.75
		        && status == FIGHTING)
		{
			startQuestTimer("attack_stop", 0, frintezza, null);
		...
		else if (npc.getNpcId() == SCARLET1 && _SecondMorph == 1
		        && _ThirdMorph == 0 && _OnMorph == 0
		...
		else if (npc.getNpcId() == SCARLET2 && _SecondMorph == 1
		        && _ThirdMorph == 1 && _OnCheck == 0
		        && damage >= npc.getCurrentHp() && status == FIGHTING)
		{
			_OnCheck = 1;
			startQuestTimer("check_hp", 0, npc, null);
		}
		else if ((npc.getNpcId() == 29050 || npc.getNpcId() == 29051)
		        && _Bomber == 0)
		{
	        ...
	        final boolean isPet)
	{
		...
		else if (npc.getNpcId() == SCARLET2 && _OnCheck == 0
		        && status == FIGHTING)
		{
			_OnCheck = 1;
			startQuestTimer("stop_pc", 0, npc, null);
		...
		else if (npc.getNpcId() == SCARLET2 && _OnCheck == 1
		        && status == FIGHTING)
		{
			if (!npc.getSpawn().is_customBossInstance())
			{
				...
				final long respawnTime = (long) (Config.FRINTEZZA_RESP_FIRST
				        + Rnd.get(Config.FRINTEZZA_RESP_SECOND)) * 3600000;
		...
		else if (npc.getNpcId() == 18328)
		{
			...
			/*
			 * _KillHallAlarmDevice++; if (_KillHallAlarmDevice == 4) {
			 * startQuestTimer("room1_del", 100, npc, null);
			 * startQuestTimer("room2_spawn", 100, npc, null);
			 ...
			 * DoorTable.getInstance().getDoor(17130046).openMe(); for (int i =
			 * 17130051; i <= 17130058; i++)
			 * DoorTable.getInstance().getDoor(i).openMe(); }
			 */
		}
		else if (npc.getNpcId() == 18339)
		{
			...
			if (_KillDarkChoirPlayer == 2)
			{
				...
				synchronized (_PlayersInside)
				{
					...
					{
						if (_Zone.isInsideZone(room2_pc)
						        && room2_pc.getY() > -86130)
						{
							outside++;
						}
	 ...
	 */
	@Override
	public L2NpcInstance addSpawn(final int npcId, final int x, final int y,
	        final int z, final int heading, final boolean randomOffset,
	...
	/*
	 * public static void main(String[] args) { // now call the constructor
	 * (starts up the ai) new Frintezza(-1,"Frintezza","ai"); }
	 */
	/*
	 * (non-Javadoc)
	 *
