...
public class Sailren extends Quest implements Runnable
{
	...
	public String onTalk(L2NpcInstance npc, L2PcInstance player)
	{
		...
		if (GrandBossManager.getInstance().getBossStatus(SAILREN) == DORMANT
		        || GrandBossManager.getInstance().getBossStatus(SAILREN) == WAITING)
		{
			if (player.isFlying())
			{
			...
			else if (player.getQuestState("sailren").getQuestItemsCount(8784) > 0)
			{
				...
				player.teleToLocation(27734 + Rnd.get(-80, 80), -6938
				        + Rnd.get(-80, 80), -1982);
				if (GrandBossManager.getInstance().getBossStatus(SAILREN) == DORMANT)
				{
	        ...
	        boolean isPet)
	{
		...
		if ((npc.getNpcId() == VELOCIRAPTOR || npc.getNpcId() == PTEROSAUR
		        || npc.getNpcId() == TYRANNOSAURUS)
		        && GrandBossManager.getInstance().getBossStatus(SAILREN) == FIGHTING)
		{
	...
	public String onKill(L2NpcInstance npc, L2PcInstance killer, boolean isPet)
	{
		        ...
		        && npc.getNpcId() == SAILREN)
		{
			...
			GrandBossManager.getInstance().setStatsSet(SAILREN, info);
		        ...
		        && npc.getNpcId() == VELOCIRAPTOR)
		{
			this.cancelQuestTimer("sailren_despawn", npc, null);
			this.startQuestTimer("waiting2", 15000, npc, null);
		}
		else if (GrandBossManager.getInstance().getBossStatus(SAILREN) == FIGHTING
		        && npc.getNpcId() == PTEROSAUR)
		{
			this.cancelQuestTimer("sailren_despawn", npc, null);
