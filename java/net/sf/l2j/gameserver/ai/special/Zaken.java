...
public class Zaken extends Quest implements Runnable
{
	...
	private int _ai1 = 0; // used for X coord tracking for non-random
	                      // teleporting in zaken's self teleport skill

	private int _ai2 = 0; // used for Y coord tracking for non-random
	                      // teleporting in zaken's self teleport skill

	private int _ai3 = 0; // used for Z coord tracking for non-random
	                      // teleporting in zaken's self teleport skill
	...
	private static final int[] Xcoords = { 53950, 55980, 54950, 55970, 53930,
	        55970, 55980, 54960, 53950, 53930, 55970, 55980, 54960, 53950,
	        53930 };

	private static final int[] Ycoords = { 219860, 219820, 218790, 217770,
	        217760, 217770, 219920, 218790, 219860, 217760, 217770, 219920,
	        218790, 219860, 217760 };

	private static final int[] Zcoords = { -3488, -3488, -3488, -3488, -3488,
	        -3216, -3216, -3216, -3216, -3216, -2944, -2944, -2944, -2944,
	        -2944 };
	...
	public Zaken(final int questId, final String name, final String descr)
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new Runnable() {
			...
			public void run()
			{
				try
				{
					if (getTimeHour() == Config.HOUR_ENABLE_DOOR_ZAKEN)
					{
						...
						ThreadPoolManager.getInstance().scheduleGeneral(new Runnable() {
							...
							public void run()
							{
								...
								catch (final Throwable e)
								{
									...
									LOGGER.warning("Cannot close door ID: 21240006 "
									        + e);
								}
							}
						}, 300000L);
		...
		if (status == DEAD)
		{
			...
			// if zaken is locked until a certain time, mark it so and start the
			// unlock timer
	        ...
	        final L2PcInstance player)
	{
		...
		if (status == DEAD && event == null
		        || !event.equalsIgnoreCase("zaken_unlock"))
		{
		...
		if (event.equalsIgnoreCase("1001"))
		{
			...
			if (getTimeHour() < 5)
			{
				        ...
				        && _ai0 == 0)
				{
					...
					{
						if ((((((L2Attackable) npc).getMostHated().getX()
						        - _ai1)
						        * (((L2Attackable) npc).getMostHated().getX()
						                - _ai1))
						        + ((((L2Attackable) npc).getMostHated().getY()
						                - _ai2)
						                * (((L2Attackable) npc).getMostHated().getY()
						                        - _ai2))) > (1500 * 1500))
						{
						...
						if (_quest0 > 0)
						{
							...
							else if ((((c_quest0.getX() - _ai1)
							        * (c_quest0.getX() - _ai1))
							        + ((c_quest0.getY() - _ai2)
							                * (c_quest0.getY() - _ai2))) > (1500
							                        * 1500))
							{
								i0 = 1;
							}
						...
						if (_quest0 > 1)
						{
							...
							else if ((((c_quest1.getX() - _ai1)
							        * (c_quest1.getX() - _ai1))
							        + ((c_quest1.getY() - _ai2)
							                * (c_quest1.getY() - _ai2))) > (1500
							                        * 1500))
							{
								i0 = 1;
							}
						...
						if (_quest0 > 2)
						{
							...
							else if ((((c_quest2.getX() - _ai1)
							        * (c_quest2.getX() - _ai1))
							        + ((c_quest2.getY() - _ai2)
							                * (c_quest2.getY() - _ai2))) > (1500
							                        * 1500))
							{
								i0 = 1;
							}
						...
						if (_quest0 > 3)
						{
							...
							else if ((((c_quest3.getX() - _ai1)
							        * (c_quest3.getX() - _ai1))
							        + ((c_quest3.getY() - _ai2)
							                * (c_quest3.getY() - _ai2))) > (1500
							                        * 1500))
							{
								i0 = 1;
							}
						...
						if (_quest0 > 4)
						{
							...
							else if ((((c_quest4.getX() - _ai1)
							        * (c_quest4.getX() - _ai1))
							        + ((c_quest4.getY() - _ai2)
							                * (c_quest4.getY() - _ai2))) > (1500
							                        * 1500))
							{
								i0 = 1;
							}
			...
			if (sk_4227 == 1) // when switching to day time, cancel zaken night
			                  // regen
			{
				npc.setTarget(npc);
		...
		if (event.equalsIgnoreCase("1003"))
		{
			if (_ai4 == 1)
			{
				...
				addSpawn(pirates_zombie_captain_b, Xcoords[rr]
				        + Rnd.get(650), Ycoords[rr]
				                + Rnd.get(650), Zcoords[rr], Rnd.get(65536), false, 0)/*
				                                                                       * .setIsRaidMinion
				                                                                       * (
				                                                                       * true)
				                                                                       */;
			...
			{
				final int rr = Rnd.get(15);
				addSpawn(doll_blader_b, Xcoords[rr] + Rnd.get(650), Ycoords[rr]
				        + Rnd.get(650), Zcoords[rr], Rnd.get(65536), false, 0)/*
				                                                               * .setIsRaidMinion
				                                                               * (
				                                                               * true)
			...
			{
				addSpawn(vale_master_b, Xcoords[Rnd.get(15)]
				        + Rnd.get(650), Ycoords[Rnd.get(15)]
				                + Rnd.get(650), Zcoords[Rnd.get(15)], Rnd.get(65536), false, 0)/*
				                                                                                * .setIsRaidMinion
				                                                                                * (
				                                                                                * true)
				                                                                                */;
			...
			{
				addSpawn(pirates_zombie_b, Xcoords[Rnd.get(15)]
				        + Rnd.get(650), Ycoords[Rnd.get(15)]
				                + Rnd.get(650), Zcoords[Rnd.get(15)], Rnd.get(65536), false, 0)/*
				                                                                                * .setIsRaidMinion
				                                                                                * (
	...
	@Override
	public String onFactionCall(final L2NpcInstance npc,
	        final L2NpcInstance caller, final L2PcInstance attacker,
	        final boolean isPet)
	{
		...
		if (getTimeHour() < 5 && callerId != ZAKEN && npcId == ZAKEN)
		{
			...
			if (npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_IDLE
			        && _ai0 == 0 && damage < 10 && Rnd.get((30 * 15)) < 1)// todo
			                                                              // -
			                                                              // damage
			                                                              // missing
			{
	        ...
	        final L2PcInstance player, final L2Skill skill)
	{
		if (npc.getNpcId() == ZAKEN)
		{
			...
			else if (skillId == 4216)
			{
				...
				player.teleToLocation(Xcoords[i1] + Rnd.get(650), Ycoords[i1]
				        + Rnd.get(650), Zcoords[i1]);
			...
			else if (skillId == 4217)
			{
				...
				if (c_quest0 != null && _quest0 > 0 && c_quest0 != player
				        && c_quest0.getZ() > (player.getZ() - 100)
				        && c_quest0.getZ() < (player.getZ() + 100))
				{
					if ((((c_quest0.getX() - player.getX())
					        * (c_quest0.getX() - player.getX()))
					        + ((c_quest0.getY() - player.getY())
					                * (c_quest0.getY() - player.getY()))) > (250
					                        * 250))
					{
						i0 = 1;
					}
					...
					{
						i1 = Rnd.get(15);
						c_quest0.teleToLocation(Xcoords[i1]
						        + Rnd.get(650), Ycoords[i1]
						                + Rnd.get(650), Zcoords[i1]);
						((L2Attackable) npc).stopHating(c_quest0);
					}
				}
				if (c_quest1 != null && _quest0 > 1 && c_quest1 != player
				        && c_quest1.getZ() > (player.getZ() - 100)
				        && c_quest1.getZ() < (player.getZ() + 100))
				{
					if ((((c_quest1.getX() - player.getX())
					        * (c_quest1.getX() - player.getX()))
					        + ((c_quest1.getY() - player.getY())
					                * (c_quest1.getY() - player.getY()))) > (250
					                        * 250))
					{
						i0 = 1;
					}
					...
					{
						i1 = Rnd.get(15);
						c_quest1.teleToLocation(Xcoords[i1]
						        + Rnd.get(650), Ycoords[i1]
						                + Rnd.get(650), Zcoords[i1]);
						((L2Attackable) npc).stopHating(c_quest1);
					}
				}
				if (c_quest2 != null && _quest0 > 2 && c_quest2 != player
				        && c_quest2.getZ() > (player.getZ() - 100)
				        && c_quest2.getZ() < (player.getZ() + 100))
				{
					if ((((c_quest2.getX() - player.getX())
					        * (c_quest2.getX() - player.getX()))
					        + ((c_quest2.getY() - player.getY())
					                * (c_quest2.getY() - player.getY()))) > (250
					                        * 250))
					{
						i0 = 1;
					}
					...
					{
						i1 = Rnd.get(15);
						c_quest2.teleToLocation(Xcoords[i1]
						        + Rnd.get(650), Ycoords[i1]
						                + Rnd.get(650), Zcoords[i1]);
						((L2Attackable) npc).stopHating(c_quest2);
					}
				}
				if (c_quest3 != null && _quest0 > 3 && c_quest3 != player
				        && c_quest3.getZ() > (player.getZ() - 100)
				        && c_quest3.getZ() < (player.getZ() + 100))
				{
					if ((((c_quest3.getX() - player.getX())
					        * (c_quest3.getX() - player.getX()))
					        + ((c_quest3.getY() - player.getY())
					                * (c_quest3.getY() - player.getY()))) > (250
					                        * 250))
					{
						i0 = 1;
					}
					...
					{
						i1 = Rnd.get(15);
						c_quest3.teleToLocation(Xcoords[i1]
						        + Rnd.get(650), Ycoords[i1]
						                + Rnd.get(650), Zcoords[i1]);
						((L2Attackable) npc).stopHating(c_quest3);
					}
				}
				if (c_quest4 != null && _quest0 > 4 && c_quest4 != player
				        && c_quest4.getZ() > (player.getZ() - 100)
				        && c_quest4.getZ() < (player.getZ() + 100))
				{
					if ((((c_quest4.getX() - player.getX())
					        * (c_quest4.getX() - player.getX()))
					        + ((c_quest4.getY() - player.getY())
					                * (c_quest4.getY() - player.getY()))) > (250
					                        * 250))
					{
						i0 = 1;
					}
					...
					{
						i1 = Rnd.get(15);
						c_quest4.teleToLocation(Xcoords[i1]
						        + Rnd.get(650), Ycoords[i1]
						                + Rnd.get(650), Zcoords[i1]);
	        ...
	        final boolean isPet)
	{
		...
		if (npcId == ZAKEN)
		{
			...
			if (!npc.getSpawn().is_customBossInstance())
			{
				...
				final long respawnTime = (long) (Config.ZAKEN_RESP_FIRST
				        + Rnd.get(Config.ZAKEN_RESP_SECOND)) * 3600000;
		...
		else if (status == ALIVE)
		{
			if (npcId != ZAKEN)
			{
				startQuestTimer("CreateOnePrivateEx", ((30 + Rnd.get(60))
				        * 1000), npc, null);
			}
		}
	...
	/*
	 * public String onSkillSee(L2NpcInstance npc, L2PcInstance caster, L2Skill
	 * skill, L2Object[] targets, boolean isPet) { int npcId = npc.getNpcId();
	 * if (npcId == ZAKEN) { if (skill.getAggroPoints() > 0) { ((L2Attackable)
	 * npc).addDamageHate(caster, 0, (((skill.getAggroPoints() / npc.getMaxHp())
	 * * 10) * 150)); } if (Rnd.get(12) < 1) { int i0 = Rnd.get((15 * 15)); if
	 * (i0 < 1) { npc.setTarget(caster);
	 * npc.doCast(SkillTable.getInstance().getInfo(4216, 1)); } else if (i0 < 2)
	 * { npc.setTarget(caster);
	 * npc.doCast(SkillTable.getInstance().getInfo(4217, 1)); } else if (i0 < 4)
	 * { npc.setTarget(caster);
	 * npc.doCast(SkillTable.getInstance().getInfo(4219, 1)); } else if (i0 < 8)
	 * { npc.setTarget(caster);
	 * npc.doCast(SkillTable.getInstance().getInfo(4218, 1)); } else if (i0 <
	 * 15) { for (L2Character character :
	 * npc.getKnownList().getKnownCharactersInRadius(100)) { if (character !=
	 * caster) continue; if (caster != ((L2Attackable) npc).getMostHated()) {
	 * npc.setTarget(caster); npc.doCast(SkillTable.getInstance().getInfo(4221,
	 * 1)); } } } if (Rnd.get(2) < 1) { if (caster == ((L2Attackable)
	 * npc).getMostHated()) { npc.setTarget(caster);
	 * npc.doCast(SkillTable.getInstance().getInfo(4220, 1)); } } } } return
	        ...
	        final L2PcInstance player, final boolean isPet)
	{
		...
		if (npcId == ZAKEN)
		{
			...
			if (player.getZ() > (npc.getZ() - 100)
			        && player.getZ() < (npc.getZ() + 100))
			{
