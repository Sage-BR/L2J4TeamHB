...
public class FastMRUCache<K, V>
{
	...
	private ConcurrentHashMap<K, CacheNode> _cache = new ConcurrentHashMap<>();

	private ConcurrentHashMap<K, V> _map;

	private ArrayList<K> _mruList = new ArrayList<>();
	...
	public FastMRUCache()
	{
		this(new ConcurrentHashMap<>(), DEFAULT_CAPACITY, DEFAULT_FORGET_TIME);
	}
	...
	public synchronized V get(K key)
	{
		...
		else
		{
			...
			if ((current._lastModified
			        + _forgetTime) <= System.currentTimeMillis())
			{
