...
public class AugmentationData
{
	...
	public AugmentationData()
	{
		...
		_log.info("AugmentationData: Loaded: "
		        + (_augmentationStats[0].size() * 4) + " augmentation stats.");
		_log.info("AugmentationData: Loaded: " + _activeSkills.size()
		        + " active, " + _passiveSkills.size() + " passive and "
		        + _chanceSkills.size() + " chance skills");
	}
	...
	public class augmentationSkill
	{
		...
		public augmentationSkill(int skillId, int maxSkillLevel,
		        int augmentationSkillId)
		{
			_skillId = skillId;
	...
	private final void load()
	{
		...
		try
		{
			...
			File file = new File(Config.DATAPACK_ROOT
		...
		for (int i = 1; i < 5; i++)
		{
			try
			{
				...
				File file = new File(Config.DATAPACK_ROOT
				        + "/data/stats/augmentation/augmentation_stats" + i
				        + ".xml");
				if (!file.exists())
				{
					if (Config.DEBUG)
					{
						_log.info("The augmentation stat data file " + i
						        + " is missing.");
					}
					return;
				}
					...
					if ("list".equalsIgnoreCase(n.getNodeName()))
					{
						for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
						{
							if ("stat".equalsIgnoreCase(d.getNodeName()))
							{
								...
								// store this stat
								((ArrayList<augmentationStat>) _augmentationStats[(i
								        - 1)]).add(new augmentationStat(Stats.valueOfXml(statName), soloValues, combinedValues));
							}
						}
					}
				}
			...
			{
				_log.log(Level.SEVERE, "Error parsing augmentation_stats" + i
				        + ".xml.", e);
				return;
			}
		}
	 ...
	 */
	public L2Augmentation generateRandomAugmentation(L2ItemInstance item,
	        int lifeStoneLevel, int lifeStoneGrade)
	{
		// Note that stat12 stands for stat 1 AND 2 (same for stat34 ;p )
		// this is because a value can contain up to 2 stat modifications
		// (there are two short values packed in one integer value, meaning 4
		// stat modifications at max)
		...
		// whats known is: u can also get a red result from a normal grade
		// lifeStone
		// however I will make it so that a higher grade lifeStone will more
		// likely result in a
		// higher grade augmentation... and the augmentation result will at
		// least have the grade
		// of the life stone
		...
		else if (lifeStoneGrade == 2
		        || resultColor <= (15 * lifeStoneGrade) + 20)
		{
			resultColor = 2;
		}
		else if (lifeStoneGrade == 1
		        || resultColor <= (15 * lifeStoneGrade) + 30)
		{
			resultColor = 1;
		}
		else
	...
	// normal stat
		...
		if (generateSkill)
		{
			...
			{
				case 1: // chance skill
					temp = _chanceSkills.get(Rnd.get(0, _chanceSkills.size()
					        - 1));
					skill = temp.getSkill(lifeStoneLevel);
					stat34 = temp.getAugmentationSkillId();
					break;
				case 2: // active skill
					temp = _activeSkills.get(Rnd.get(0, _activeSkills.size()
					        - 1));
					skill = temp.getSkill(lifeStoneLevel);
					stat34 = temp.getAugmentationSkillId();
					break;
				case 3: // passive skill
					temp = _passiveSkills.get(Rnd.get(0, _passiveSkills.size()
					        - 1));
					skill = temp.getSkill(lifeStoneLevel);
					stat34 = temp.getAugmentationSkillId();
					break;
	...
	{
		ArrayList<AugStat> temp = new ArrayList<>();
		// An augmentation id contains 2 short vaues so we gotta seperate them
		// here
		// both values contain a number from 1-16380, the first 14560 values are
		// stats
		...
		// the first 13 values are so called Solo-stats and they have the
		// highest stat increase possible
		// after the 13 Solo-stats come 78 combined stats (thats every possible
		// combination of the 13 solo stats)
		// the first 12 combined stats (14-26) is the stat 1 combined with stat
		// 2-13
		// the next 11 combined stats then are stat 2 combined with stat 3-13
		// and so on...
		// to get the idea have a look @ optiondata_client-e.dat - thats where
		// the data came from :)
		int stats[] = new int[2];
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (stats[i] >= STAT_START && stats[i] <= STAT_END)
			{
				...
				{
					augmentationStat as = ((augmentationStat) _augmentationStats[block].get((stats[i]
					        - 1)));
					temp.add(new AugStat(as.getStat(), as.getSingleStatValue(subblock)));
				}
				else // twin stat
				{
					stats[i] -= 13; // rescale to 0 (if first of first combined
					                // block)
					...
					// get 2nd stat
					as = ((augmentationStat) _augmentationStats[block].get(rescales
					        + stats[i]));
