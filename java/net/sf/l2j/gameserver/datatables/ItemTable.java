...
public class ItemTable
{
	...
	private static final Map<String, Integer> _materials = new ConcurrentHashMap<>();

	private static final Map<String, Integer> _crystalTypes = new ConcurrentHashMap<>();

	private static final Map<String, L2WeaponType> _weaponTypes = new ConcurrentHashMap<>();

	private static final Map<String, L2ArmorType> _armorTypes = new ConcurrentHashMap<>();

	private static final Map<String, Integer> _slots = new ConcurrentHashMap<>();
	...
	static
	{
		...
		_slots.put("rfinger,lfinger", L2Item.SLOT_R_FINGER
		        | L2Item.SLOT_L_FINGER);
	...
	/**
	 * All item templates (etcitem, armor, weapon) are now loaded exclusively
	 * from XML.
	 */
	...
	/** List of etcItem */
	private static final Map<Integer, Item> itemData = new ConcurrentHashMap<>();

	/** List of weapons */
	private static final Map<Integer, Item> weaponData = new ConcurrentHashMap<>();

	/** List of armor */
	private static final Map<Integer, Item> armorData = new ConcurrentHashMap<>();
	...
	private void buildFastLookupTable()
	{
		...
		// Get highest ID of item in armor ConcurrentHashMap, then in weapon
		// ConcurrentHashMap, and finally in etcitem ConcurrentHashMap
	        ...
	        L2PcInstance actor, L2Object reference)
	{
		...
		if (process.equalsIgnoreCase("loot") && !Config.AUTO_LOOT)
		{
			...
			if (reference instanceof L2GrandBossInstance
		...
		if (actor != null)
		{
			if (actor.isGM())
			{
				...
				GMAudit.auditGMAction(actor.getName(), process + "(id: "
				        + itemId + " count: " + count + " name: "
				        + item.getItemName() + " objId: " + item.getObjectId()
	        ...
	        L2PcInstance actor, L2Object reference)
	{
		synchronized (item)
		{
			...
			if (Config.LOG_ITEMS)
			{
				LogRecord record = new LogRecord(Level.INFO, "DELETE:"
				        + process);
				record.setLoggerName("item");
				record.setParameters(new Object[] { item, actor, reference });
				_logItems.log(record);
			...
			if (actor != null)
			{
				if (actor.isGM())
				{
					...
					String targetName = (actor.getTarget() != null ? actor.getTarget().getName() : "no-target");
					GMAudit.auditGMAction(actor.getName(), process + "(id: "
					        + item.getItemId() + " count: " + item.getCount()
