	...
	private void fillSpawnTable()
	{
		...
		if (Config.CUSTOM_SPAWNLIST_TABLE)
		{
			try
			{
				...
				while (rset.next())
				{
					...
					{
						_log.warning("CustomSpawnTable: Spawn could not be initialized: "
						        + e);
					}
				}
				rset.close();
			...
			{
				_log.warning("CustomSpawnTable: Error processing custom spawnlist query: "
				        + e);
			}
			finally
			{
			...
			_log.config("CustomSpawnTable: Loaded " + _customSpawnCount
			        + " Npc Spawn Locations.");
		...
		if (Config.DEBUG)
		{
			_log.fine("SpawnTable: Spawning completed, total number of NPCs in the world: "
			        + (_npcSpawnCount + _customSpawnCount));
		}
