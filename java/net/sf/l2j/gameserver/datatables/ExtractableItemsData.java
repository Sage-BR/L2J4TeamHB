...
public class ExtractableItemsData
{
	...
	public ExtractableItemsData()
	{
		...
		try
		{
			s = new Scanner(new File(Config.DATAPACK_ROOT
			        + "/data/extractable_items.csv"));
		}
		catch (Exception e)
		{
			_log.warning("Extractable items data: Can not find '"
			        + Config.DATAPACK_ROOT + "/data/extractable_items.csv'");
			return;
		}
		...
		while (s.hasNextLine())
		{
			...
			{
				_log.warning("Extractable items data: Error in line "
				        + lineCount
			...
			ArrayList<L2ExtractableProductItem> product_temp = new ArrayList<>();
		...
		s.close();
		_log.info("Extractable items data: Loaded " + _items.size()
		        + " extractable items!");
	}
