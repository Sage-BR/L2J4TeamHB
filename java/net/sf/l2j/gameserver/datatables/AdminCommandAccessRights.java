...
public class AdminCommandAccessRights
{
	 ...
	 */
	private Map<String, AdminCommandAccessRight> _adminCommandAccessRights = new ConcurrentHashMap<>();
	...
	private AdminCommandAccessRights()
	{
		...
		{
			_log.warning("AdminCommandAccessRights: Error loading from database:"
			        + e);
		}
		finally
		{
		...
		_log.info("AdminCommandAccessRights: Loaded "
		        + _adminCommandAccessRights.size() + " from database.");
	}
	 ...
	 * <br>
	 *
	 * @return AdminCommandAccessRights: the one and only instance of this
	 *         class<br>
	 */
	...
	public boolean hasAccess(String adminCommand, AccessLevel accessLevel)
	{
		...
		if (acar == null)
		{
			_log.info("AdminCommandAccessRights: No rights defined for admin command "
			        + adminCommand + ".");
			return false;
		}
