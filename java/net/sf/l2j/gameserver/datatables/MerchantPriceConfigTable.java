...
public class MerchantPriceConfigTable
{
	...
	private Map<Integer, MerchantPriceConfig> _mpcs = new ConcurrentHashMap<>();
	...
	private MerchantPriceConfigTable()
	{
		...
		{
			this.loadXML();
			_log.info("MerchantPriceConfigTable: Loaded " + _mpcs.size()
			        + " merchant price configs.");
		}
		catch (Exception e)
		{
			_log.log(Level.SEVERE, "Failed loading MerchantPriceConfigTable. Reason: "
			        + e.getMessage(), e);
		}
	}
	...
	public static final class MerchantPriceConfig
	{
		...
		public MerchantPriceConfig(int id, String name, int baseTax,
		        Castle castle, int zoneId)
		{
			_id = id;
			_name = name;
			_baseTax = baseTax;
		...
		{
			return this.hasCastle() ? (getCastle().getTaxPercent()
			        + getBaseTax()) : getBaseTax();
		}
