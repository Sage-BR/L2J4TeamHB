...
public class SummonItemsData
{
	...
	{
		_summonitems = new ConcurrentHashMap<>();
		...
		try
		{
			s = new Scanner(new File(Config.DATAPACK_ROOT
			        + "/data/summon_items.csv"));
		}
		catch (Exception e)
		{
			_log.warning("Summon items data: Can not find '"
			        + Config.DATAPACK_ROOT + "/data/summon_items.csv'");
			return;
		}
		...
		while (s.hasNextLine())
		{
			...
			{
				_log.warning("Summon items data: Error in line " + lineCount
		...
		_log.info("Summon items data: Loaded " + _summonitems.size()
		        + " summon items.");
	}
