...
public class SevenSigns
{
	...
	protected void restoreSevenSignsData()
	{
		...
		{
			_log.severe("SevenSigns: Unable to load Seven Signs data from database: "
			        + e);
		}
		finally
		{
	...
	public void saveSevenSignsData(L2PcInstance player, boolean updateSettings)
	{
		...
		try
		{
			...
			for (StatsSet sevenDat : _signsPlayerData.values())
			{
				...
				if (Config.DEBUG)
				{
					_log.info("SevenSigns: Updated data in database for char ID "
					        + sevenDat.getInteger("charId") + " ("
			...
			if (updateSettings)
			{
				...
				{
					statement.setInt(18
					        + i, SevenSignsFestival.getInstance().getAccumulatedBonus(i));
				}

				statement.setInt(18
				        + SevenSignsFestival.FESTIVAL_COUNT, Calendar.getInstance().get(Calendar.DAY_OF_WEEK));
				statement.execute();
	        ...
	        int chosenSeal)
	{
			...
			try
			{
				...
				if (Config.DEBUG)
				{
					_log.info("SevenSigns: Inserted data in DB for char ID "
					        + currPlayerData.getInteger("charId") + " ("
		...
		{
			_signsDuskSealTotals.put(chosenSeal, _signsDuskSealTotals.get(chosenSeal)
			        + 1);
		}
	        ...
	        int greenCount, int redCount)
	{
		...
		currPlayer.set("red_stones", currPlayer.getInteger("red_stones")
		        + redCount);
		currPlayer.set("green_stones", currPlayer.getInteger("green_stones")
		        + greenCount);
		currPlayer.set("blue_stones", currPlayer.getInteger("blue_stones")
		        + blueCount);
	...
	protected void calcNewSealOwners()
	{
		...
		for (Integer currSeal : _signsDawnSealTotals.keySet())
		{
			...
			int dawnPercent = Math.round(((float) dawnProportion
			...
			int duskPercent = Math.round(((float) duskProportion
