...
public class AiInstance
{
	...
	/**
	 * <p>
	 * This methode add the handler to the {@link EventHandlerSet} associated
	 * with the specified{@link AiEventType}
	 * </p>
	 *
	 * @param handler
	...
	public class QueueEventRunner implements Runnable
	{
		...
		public QueueEventRunner(EventHandlerSet set, AiParameters ai,
		        AiEvent event)
		{
			_set = set;
			_ai = ai;
	...
	public void triggerEvent(AiEvent event, AiParameters aiParams)
	{
		...
		{
			// if there was no events in the queue start processing events after
			// we add them.
			if (!aiParams.hasEvents())
			{
				restart = true;
