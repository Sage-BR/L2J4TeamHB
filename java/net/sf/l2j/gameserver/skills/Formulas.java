...
public final class Formulas
{
	...
	static
	{
		...
		{
			STRbonus[i] = Math.floor(Math.pow(STRCompute[0], i - STRCompute[1])
			        * 100 + .5d) / 100;
		}
		for (int i = 0; i < INTbonus.length; i++)
		{
			INTbonus[i] = Math.floor(Math.pow(INTCompute[0], i - INTCompute[1])
			        * 100 + .5d) / 100;
		}
		for (int i = 0; i < DEXbonus.length; i++)
		{
			DEXbonus[i] = Math.floor(Math.pow(DEXCompute[0], i - DEXCompute[1])
			        * 100 + .5d) / 100;
		}
		for (int i = 0; i < WITbonus.length; i++)
		{
			WITbonus[i] = Math.floor(Math.pow(WITCompute[0], i - WITCompute[1])
			        * 100 + .5d) / 100;
		}
		for (int i = 0; i < CONbonus.length; i++)
		{
			CONbonus[i] = Math.floor(Math.pow(CONCompute[0], i - CONCompute[1])
			        * 100 + .5d) / 100;
		}
		for (int i = 0; i < MENbonus.length; i++)
		{
			MENbonus[i] = Math.floor(Math.pow(MENCompute[0], i - MENCompute[1])
			        * 100 + .5d) / 100;
		}
	}
	...
	static class FuncMDefMod extends Func
	{
		...
		public void calc(Env env)
		{
			...
			env.value *= MENbonus[env.player.getMEN()]
			        * env.player.getLevelMod();
		}
	}
		...
		public void calc(Env env)
		{
			if (env.player instanceof L2PcInstance)
			{
				...
				boolean hasMagePDef = (p.getClassId().isMage()
				        || p.getClassId().getId() == 0x31); // orc mystics are a
				                                            // special case
	...
	public final double calcHpRegen(L2Character cha)
	{
		...
		if (cha instanceof L2PcInstance)
		{
			...
			init += (player.getLevel() > 10) ? ((player.getLevel() - 1)
			        / 10.0) : 0.5;
			        ...
			        && player.getClan() != null)
			{
				...
				if (clanHallIndex > 0)
				{
					...
					if (clansHall != null)
					{
						...
						{
							hpRegenMultiplier *= 1
							        + clansHall.getFunction(ClanHall.FUNC_RESTORE_HP).getLvl()
							                / 100;
						}
					}
				}
			        ...
			        && player.getClan() != null)
			{
				...
				if (castleIndex > 0)
				{
					...
					if (castle != null)
					{
						...
						{
							hpRegenMultiplier *= 1
							        + castle.getFunction(Castle.FUNC_RESTORE_HP).getLvl()
							                / 100;
						}
					}
				}
			        ...
			        && player.getClan() != null)
			{
				...
				if (clanHallIndex > 0)
				{
					...
					if (clansHall != null)
					{
						...
						{
							mpRegenMultiplier *= 1
							        + clansHall.getFunction(ClanHall.FUNC_RESTORE_MP).getLvl()
							                / 100;
						}
					}
				}
			        ...
			        && player.getClan() != null)
			{
				...
				if (castleIndex > 0)
				{
					...
					if (castle != null)
					{
						...
						{
							mpRegenMultiplier *= 1
							        + castle.getFunction(Castle.FUNC_RESTORE_MP).getLvl()
							                / 100;
						}
					}
				}
	...
	public double calcBlowDamage(L2Character attacker, L2Character target,
	        L2Skill skill, boolean shld, boolean ss)
	{
		if (target.isInvul())
		{
			return 0;
		...
		damage += 1.5 * (attacker.calcStat(Stats.CRITICAL_DAMAGE, damage
		        + power, target, skill)
	        ...
	        L2Skill skill, boolean shld, boolean crit, boolean dual, boolean ss)
	{
		...
		if (stat != null)
		{
			// get the vulnerability due to skills (buffs, passives, toggles,
			// etc)
	        ...
	        L2Skill skill, boolean ss, boolean bss, boolean mcrit)
	{
		...
		double damage = 91 * Math.sqrt(mAtk) / mDef * skill.getPower(attacker)
		        * calcSkillVulnerability(target, skill);
	...
	public final double calcMagicDam(L2CubicInstance attacker,
	        L2Character target, L2Skill skill, boolean mcrit)
	{
		if (target.isInvul())
		{
			return 0;
		...
		// Failure calculation
		if (Config.ALT_GAME_MAGICFAILURES
		        && !calcMagicSuccess(owner, target, skill))
		{
			if (calcMagicSuccess(owner, target, skill)
			        && (target.getLevel() - skill.getMagicLevel()) <= 9)
			{
				if (skill.getSkillType() == SkillType.DRAIN)
				{
	...
	public final boolean calcBlow(L2Character activeChar, L2Character target,
	        int chance)
	{
		return activeChar.calcStat(Stats.BLOW_RATE, chance
		        * (1.0 + (activeChar.getDEX() - 20)
		                / 100), target, null) > Rnd.get(100);
	}

	/** Calcul value of lethal chance */
	public final double calcLethal(L2Character activeChar, L2Character target,
	        int baseLethal)
	{
		return activeChar.calcStat(Stats.LETHAL_RATE, (baseLethal
		        * ((double) activeChar.getLevel()
		                / target.getLevel())), target, null);
	}

	public final boolean calcLethalHit(L2Character activeChar,
	        L2Character target, L2Skill skill)
	{
		if (!target.isRaid() && !(target instanceof L2DoorInstance)
		        && !(target instanceof L2NpcInstance
		                && ((L2NpcInstance) target).getNpcId() == 35062))
		{
			int chance = Rnd.get(100);
			// 2nd lethal effect activate (cp,hp to 1 or if target is npc then
			// hp to 1)
			if (skill.getLethalChance2() > 0
			        && chance < calcLethal(activeChar, target, skill.getLethalChance2()))
			{
				if (target instanceof L2NpcInstance)
				{
					target.reduceCurrentHp(target.getCurrentHp()
					        - 1, activeChar);
				}
				...
				activeChar.sendPacket(new SystemMessage(SystemMessageId.LETHAL_STRIKE));
			        ...
			        && chance < calcLethal(activeChar, target, skill.getLethalChance1()))
			{
				if (target instanceof L2PcInstance)
				{
					L2PcInstance player = (L2PcInstance) target;
	...
	public boolean calcShldUse(L2Character attacker, L2Character target)
	{
		...
		int degreeside = (int) target.calcStat(Stats.SHIELD_DEFENCE_ANGLE, 0, null, null)
		        + 120;
	...
	public double calcSkillVulnerability(L2Character target, L2Skill skill)
	{
		...
		if (skill != null)
		{
			...
			// For additional effects on PDAM and MDAM skills (like STUN, SHOCK,
			// PARALYZE...)
			if (type != null
			        && (type == SkillType.PDAM || type == SkillType.MDAM))
			{
	        ...
	        L2Skill skill, boolean ss, boolean sps, boolean bss)
	{
		...
		if (type == SkillType.PDAM || type == SkillType.MDAM
		        || type == SkillType.WEAPON_SA) // For additional effects on
		                                        // PDAM skills (like STUN,
		                                        // SHOCK,...)
		{
		...
		if (lvlDepend == 0)
		{
			lvlDepend = (type == SkillType.PARALYZE
			        || type == SkillType.FEAR) ? 1 : 2;
		}

		// TODO: Temporary fix for NPC skills with MagicLevel not set
		// int lvlmodifier = (skill.getMagicLevel() - target.getLevel()) *
		// lvlDepend;
		int lvlmodifier = ((skill.getMagicLevel() > 0 ? skill.getMagicLevel() : attacker.getLevel())
		        - target.getLevel()) * lvlDepend;
		...
		if (Config.DEVELOPER)
		{
			_log.info(skill.getName() + ": " + value + ", " + statmodifier
			        + ", " + lvlmodifier + ", " + resmodifier + ", "
			        + ((int) (Math.pow((double) attacker.getMAtk(target, skill)
			                / target.getMDef(attacker, skill), 0.2) * 100)
			                - 100)
			        + ", " + ssmodifier + " ==> " + rate);
		}
		return (Rnd.get(100) < rate);
	...
	public boolean calcCubicSkillSuccess(L2CubicInstance attacker,
	        L2Character target, L2Skill skill)
	{
		SkillType type = skill.getSkillType();
		...
		// int lvlmodifier = (skill.getMagicLevel() - target.getLevel()) *
		// lvlDepend;
		int lvlmodifier = ((skill.getMagicLevel() > 0 ? skill.getMagicLevel() : attacker.getOwner().getLevel())
		        - target.getLevel()) * lvlDepend;
		double statmodifier = calcSkillStatModifier(type, target);
		double resmodifier = calcSkillVulnerability(target, skill);
		...
		if (skill.isMagic())
		{
			rate += (int) (Math.pow((double) attacker.getMAtk()
			        / target.getMDef(attacker.getOwner(), skill), 0.2) * 100)
			        - 100;
		}
		...
		if (Config.DEVELOPER)
		{
			_log.info(skill.getName() + ": " + value + ", " + statmodifier
			        ...
			        + ((int) (Math.pow((double) attacker.getMAtk()
			                / target.getMDef(attacker.getOwner(), skill), 0.2)
			                * 100) - 100)
			        + " ==> " + rate);
		}
		return (Rnd.get(100) < rate);
	        ...
	        L2Skill skill)
	{
		double lvlDifference = (target.getLevel()
		        - (skill.getMagicLevel() > 0 ? skill.getMagicLevel() : attacker.getLevel()));
