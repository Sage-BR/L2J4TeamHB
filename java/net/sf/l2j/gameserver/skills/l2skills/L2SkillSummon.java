...
public class L2SkillSummon extends L2Skill
{
	...
	public boolean checkCondition(L2Character activeChar)
	{
		if (activeChar instanceof L2PcInstance)
		{
			...
			if (isCubic())
			{
				...
				if (count > mastery)
				{
					activeChar.sendMessage("You already have " + count
					        + " cubic(s).");
					return false;
				}
			}
	...
	public void useSkill(L2Character caster, L2Object[] targets)
	{
		...
		if (_npcId == 0)
		{
			activeChar.sendMessage("Summon skill " + getId()
			        + " not described yet");
			return;
		}
		...
		{
			_log.warning("Summon attempt for nonexisting NPC ID:" + _npcId
			        + ", skill ID:" + this.getId());
		...
		{
			summon.getStat().setExp(Experience.LEVEL[Experience.LEVEL.length
			        - 1]);
		...
		summon.spawnMe(activeChar.getX() + 50, activeChar.getY()
		        + 100, activeChar.getZ());
