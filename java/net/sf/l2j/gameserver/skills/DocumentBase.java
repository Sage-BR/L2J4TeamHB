		...
		if (attrs.getNamedItem("time") != null)
		{
			...
			if (Config.ENABLE_MODIFY_SKILL_DURATION)
			{
				if (Config.SKILL_DURATION_LIST.containsKey(((L2Skill) template).getId()))
				{
					...
					else if ((((L2Skill) template).getLevel() >= 100)
					        && (((L2Skill) template).getLevel() < 140))
					{
						time += Config.SKILL_DURATION_LIST.get(((L2Skill) template).getId());
					}
					...
					if (Config.DEBUG)
					{
						_log.info("*** Skill " + ((L2Skill) template).getName()
						        + " (" + ((L2Skill) template).getLevel()
						        + ") changed duration to " + time
						        + " seconds.");
					}
				}
			}
		...
		if (template instanceof L2Weapon)
		{
			                ...
			                && attrs.getNamedItem("onCast") == null))
			{
				((L2Weapon) template).attach(skill); // Attach as skill
				                                     // triggered on use
			// if (attrs.getNamedItem("onCrit") != null) ((L2Weapon)
			// template).attachOnCrit(skill); // Attach as skill triggered on
			// critical hit
			// if (attrs.getNamedItem("onCast") != null) ((L2Weapon)
			// template).attachOnCast(skill); // Attach as skill triggered on
			// cast
			}
		}
