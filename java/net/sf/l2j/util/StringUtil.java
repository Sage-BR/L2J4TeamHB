 ...
 * <h1>How to Use It</h1>
 * <h2>concat() or append()</h2> If concatenating strings in single call, use
 * StringUtil.concat(), otherwise use StringUtil.append() and its variants.
 ...
 *
 * Why?</br>
 * In first case, new memory has to be allocated for the second string. In
 * second case already allocated memory is reused, but only in case the new
 * string is not longer than the previously allocated string. Anyway, the second
 * way is better because the string either fits in the memory and some memory is
 * saved, or it does not fit in the memory, and in that case it works as in the
 * first case.
 * <h2>Primitives to Strings</h2> To convert primitives to string, use
 * String.valueOf().
 * <h2>How much faster is it?</h2> Here are some results of my tests. Count is
 * number of strings concatenated. Don't take the numbers as 100% true as the
 * numbers are affected by other programs running on my computer at the same
 * time. Anyway, from the results it is obvious that using StringBuilder with
 * predefined size is the fastest (and also most memory efficient) solution. It
 * is about 5 times faster when concatenating 7 strings, compared to
 * StringBuilder. Also, with more strings concatenated, the difference between
 * StringBuilder and StringBuilder gets larger. In code, there are many cases,
