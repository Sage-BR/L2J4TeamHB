...
public final class Config
{
	...
	public static final String BOSS_SETTINGS = "./config/Mods/BossSettings.ini";

	public static final String SCHEME_BUFF = "./config/Mods/SchemeBuff.ini";

	public static final String L2JMOD_CONFIG_FILE = "./config/Mods/L2J4Team.properties";
	...
	public static List<Integer> TVT_DOORS_IDS_TO_OPEN = new ArrayList<>();

	public static List<Integer> TVT_DOORS_IDS_TO_CLOSE = new ArrayList<>();
	...
	public static List<int[]> CUSTOM_STARTER_ITEMS_FIGHTER = new ArrayList<>();

	public static List<int[]> CUSTOM_STARTER_ITEMS_MAGE = new ArrayList<>();
	...
	/**
	 * Number to divide the xp received by, to calculate karma lost on xp
	 * gain/lost
	 */
	...
	public static int HOUR_ENABLE_DOOR_ZAKEN;
	...
	public static void load()
	{
		if (Server.serverMode == Server.MODE_GAMESERVER)
		{
			...
			try
			{
				...
				is.close();
				ALT_DEV_NO_AI = Boolean.parseBoolean(serverSettings.getProperty("AltDevNoAI", "False"));
				ALT_DEV_NO_SCRIPT = Boolean.parseBoolean(serverSettings.getProperty("AltDevNoScript", "False"));
				ENABLE_ALL_EXCEPTIONS = Boolean.parseBoolean(serverSettings.getProperty("EnableAllExceptionsLog", "false"));
				SCRIPT_DEBUG = Boolean.valueOf(serverSettings.getProperty("EnableScriptDebug", "false"));
				SCRIPT_ALLOW_COMPILATION = Boolean.valueOf(serverSettings.getProperty("AllowCompilation", "true"));
				SCRIPT_CACHE = Boolean.valueOf(serverSettings.getProperty("UseCache", "true"));
				SCRIPT_ERROR_LOG = Boolean.valueOf(serverSettings.getProperty("EnableScriptErrorLog", "true"));
			...
			{
				e.printStackTrace();
				throw new Error("Failed to Load " + FEATURE_CONFIG_FILE
				        + " File.");
			}

			// BossSettings Config File
			try
			{
				Properties bossSettings = new Properties();
				InputStream is = new FileInputStream(new File(BOSS_SETTINGS));
				...
				HOUR_ENABLE_DOOR_ZAKEN = Integer.parseInt(bossSettings.getProperty("HourEnableDoorZaken", "60"));
			...
			{
				e.printStackTrace();
				throw new Error("Failed to Load " + BOSS_SETTINGS + " File.");
			}

			// SchemeBuff Config File
			try
			{
				Properties buffer = new Properties();
				InputStream is = new FileInputStream(new File(SCHEME_BUFF));
				buffer.load(is);
				is.close();
			...
			{
				e.printStackTrace();
				throw new Error("Failed to Load " + SCHEME_BUFF + " File.");
			}

			// SpecialMods Config File
			try
			{
				Properties specialmods = new Properties();
				InputStream is = new FileInputStream(new File(SPECIAL_MODS));
				specialmods.load(is);
				is.close();

				ANNOUNCE_RAIDBOS_KILL = Boolean.parseBoolean(specialmods.getProperty("AnnounceRaidBossKill", "false"));
				ANNOUNCE_GRANDBOS_KILL = Boolean.parseBoolean(specialmods.getProperty("AnnounceGranBossKill", "false"));
				ANNOUNCE_BOSS_ALIVE = Boolean.parseBoolean(specialmods.getProperty("AnnounceSpawnAllBoss", "false"));
			}
			catch (Exception e)
			{
				e.printStackTrace();
			...
			try
			{
				...
				MASTERACCESS_NAME_COLOR = Integer.decode("0x"
				        + Character.getProperty("MasterNameColor", "00FF00"));
				MASTERACCESS_TITLE_COLOR = Integer.decode("0x"
				        + Character.getProperty("MasterTitleColor", "00FF00"));
				...
				RESPAWN_RESTORE_CP = Double.parseDouble(Character.getProperty("RespawnRestoreCP", "0"))
				        / 100;
				RESPAWN_RESTORE_HP = Double.parseDouble(Character.getProperty("RespawnRestoreHP", "70"))
				        / 100;
				RESPAWN_RESTORE_MP = Double.parseDouble(Character.getProperty("RespawnRestoreMP", "70"))
				        / 100;
				HP_REGEN_MULTIPLIER = Double.parseDouble(Character.getProperty("HpRegenMultiplier", "100"))
				        / 100;
				MP_REGEN_MULTIPLIER = Double.parseDouble(Character.getProperty("MpRegenMultiplier", "100"))
				        / 100;
				CP_REGEN_MULTIPLIER = Double.parseDouble(Character.getProperty("CpRegenMultiplier", "100"))
				        / 100;
				...
				{
					SKILL_DURATION_LIST = new ConcurrentHashMap<>();
					...
					for (String skill : propertySplit)
					{
						...
						{
							System.out.println("[SkillDurationList]: invalid config property -> SkillDurationList \""
							        + skill + "\"");
						}
						else
						{
				...
				{
					SKILL_REUSE_LIST = new ConcurrentHashMap<>();
					String[] propertySplit;
					...
					for (String skill : propertySplit)
					{
						...
						if (skillSplit.length != 2)
						{
							System.out.println("[SkillReuseList]: invalid config property -> SkillReuseList \""
							        + skill + "\"");
						}
						else
						{
							...
							catch (NumberFormatException nfe)
							{
								if (!skill.equals(""))
								{
									System.out.println("[SkillReuseList]: invalid config property -> SkillList \""
									        + skillSplit[0] + "\""
									        + skillSplit[1]);
								}
							}
				...
				ALT_GAME_CANCEL_BOW = Character.getProperty("AltGameCancelByHit", "Cast").equalsIgnoreCase("bow")
				        || Character.getProperty("AltGameCancelByHit", "Cast").equalsIgnoreCase("all");
				ALT_GAME_CANCEL_CAST = Character.getProperty("AltGameCancelByHit", "Cast").equalsIgnoreCase("cast")
				        || Character.getProperty("AltGameCancelByHit", "Cast").equalsIgnoreCase("all");
				...
				// ALT_GAME_NEW_CHAR_ALWAYS_IS_NEWBIE =
				// Boolean.parseBoolean(Character.getProperty("AltNewCharAlwaysIsNewbie",
				// "False"));
			...
			{
				e.printStackTrace();
				throw new Error("Failed to Load " + CHARACTER_CONFIG_FILE
				        + " File.");
			}
			...
			try
			{
				...
				TCP_NODELAY = Boolean.parseBoolean(General.getProperty("TCP_NODELAY", "True"));
				...
				HERB_AUTO_DESTROY_TIME = Integer.parseInt(General.getProperty("AutoDestroyHerbTime", "15"))
				        * 1000;
				...
				SAVE_DROPPED_ITEM_INTERVAL = Integer.parseInt(General.getProperty("SaveDroppedItemInterval", "0"))
				        * 60000;
				...
				// CoordSynchronize removed — unified mode (VERGE + Revellion)
				// always active.
			...
			{
				e.printStackTrace();
				throw new Error("Failed to Load " + GENERAL_CONFIG_FILE
				        + " File.");
			}
			...
			try
			{
				...
				RAID_HP_REGEN_MULTIPLIER = Double.parseDouble(NPC.getProperty("RaidHpRegenMultiplier", "100"))
				        / 100;
				RAID_MP_REGEN_MULTIPLIER = Double.parseDouble(NPC.getProperty("RaidMpRegenMultiplier", "100"))
				        / 100;
				RAID_PDEFENCE_MULTIPLIER = Double.parseDouble(NPC.getProperty("RaidPDefenceMultiplier", "100"))
				        / 100;
				RAID_MDEFENCE_MULTIPLIER = Double.parseDouble(NPC.getProperty("RaidMDefenceMultiplier", "100"))
				        / 100;
			...
			try
			{
				...
				RATE_DROP_SUPERIOR_HERBS = Float.parseFloat(ratesSettings.getProperty("RateSuperiorHerbs", "0.8"))
				        * 10;
				RATE_DROP_SPECIAL_HERBS = Float.parseFloat(ratesSettings.getProperty("RateSpecialHerbs", "0.2"))
				        * 10;
			...
			try
			{
						...
						else
						{
							...
							else
							{
								...
								for (String reward : propertySplit)
								{
									...
									{
										_log.warning("TvTEventEngine[Config.load()]: invalid config property -> TvTEventReward \""
										        + reward + "\"");
									}
									else
									{
								...
								for (String door : propertySplit)
								{
									...
									catch (NumberFormatException nfe)
									{
										if (!door.equals(""))
										{
											_log.warning("TvTEventEngine[Config.load()]: invalid config property -> TvTDoorsToOpen \""
											        + door + "\"");
										}
									}
								}
								...
								for (String door : propertySplit)
								{
									...
									catch (NumberFormatException nfe)
									{
										if (!door.equals(""))
										{
											_log.warning("TvTEventEngine[Config.load()]: invalid config property -> TvTDoorsToClose \""
											        + door + "\"");
										}
									}
								}
				...
				if (ALLOW_CUSTOM_STARTER_ITEMS)
				{
					...
					for (String item : fighterSplit)
					{
						...
						if (split.length != 2)
						{
							System.out.println("StarterItems[Config.load()]: invalid fighter item -> "
							        + item);
						}
						else
						{
							try
							{
								CUSTOM_STARTER_ITEMS_FIGHTER.add(new int[] {
					...
					for (String item : mageSplit)
					{
						...
						if (split.length != 2)
						{
							System.out.println("StarterItems[Config.load()]: invalid mage item -> "
							        + item);
						}
						else
						{
							try
							{
								CUSTOM_STARTER_ITEMS_MAGE.add(new int[] {
								        Integer.parseInt(split[0]),
								        Integer.parseInt(split[1]) });
							}
							catch (NumberFormatException nfe)
			...
			{
				e.printStackTrace();
				throw new Error("Failed to Load " + L2JMOD_CONFIG_FILE
				        + " File.");
			}
			// pvp config
			try
	...
	public static boolean setParameterValue(String pName, String pValue)
	{
		...
		else if (pName.equalsIgnoreCase("AllowCustomStartLvl"))
		{
			ALLOW_CUSTOM_CHAR_LVL = Boolean.parseBoolean(pValue);
		}
		else if (pName.equalsIgnoreCase("CustomStartLvl"))
		{
			CUSTOM_CHAR_LVL = Integer.parseInt(pValue);
		}
		else if (pName.equalsIgnoreCase("AllowCustomStarterItems"))
		{
			ALLOW_CUSTOM_STARTER_ITEMS = Boolean.parseBoolean(pValue);
		}
		...
		else if (pName.equalsIgnoreCase("MaxPAtkSpeedArchers"))
		{
			MAX_PATK_SPEED_ARCHERS = Integer.parseInt(pValue);
		}
