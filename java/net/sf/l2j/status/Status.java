...
public class Status extends Thread
{
	...
	public Status(int mode) throws IOException
	{
		...
		if (_mode == Server.MODE_GAMESERVER || _mode == Server.MODE_LOGINSERVER)
		{
			...
			_log.info("Telnet StatusServer started successfully, listening on Port: "
			        + _statusPort);
		}
	...
	private String rndPW(int length)
	{
		...
		for (int i = 0; i < length; i++)
		{
			...
			{
				case 0:
					password.append(lowerChar.charAt(Rnd.nextInt(lowerChar.length()
					        - 1)));
					break;
				case 1:
					password.append(upperChar.charAt(Rnd.nextInt(upperChar.length()
					        - 1)));
					break;
				case 2:
