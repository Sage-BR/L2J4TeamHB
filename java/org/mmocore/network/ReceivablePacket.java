	...
	extends AbstractPacket<T> implements Runnable
{
	 ...
	 *
	 * @param dst
	 *            : the byte array which will be filled with the data.
	 */
	...
	/**
	 * Reads <B>byte[]</B> from the buffer. <BR>
	 * Reads as many bytes as the given length (len). Starts to fill the byte
	 * array from the given offset to <B>offset</B> + <B>len</B>.
	 *
	 * @param dst
	 *            : the byte array which will be filled with the data.
	 * @param offset
	 *            : starts to fill the byte array from the given offset.
	 * @param len
