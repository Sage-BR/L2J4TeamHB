 ...
 * 
 * Usage: java Dev.SpecialMods.GeoDataPatcher [input.l2j] [-o output.l2j] [-t threshold]
 * 
 * Default input: ./data/geodata/16_19.l2j
 * Default output: input_patched.l2j
...
public class GeoDataPatcher
{
    ...
    /**
     * Finds the height of the layer at (nx, ny) closest to targetZ.
     * For Flat/Complex cells there is only one layer; for Multilayer cells
     * all layers are scanned and the closest height (by absolute diff) is returned.
     */
    private static int findNearestHeight(int nx, int ny, int targetZ)
    {
        ...
        if (type == TYPE_FLAT)
        {
            if (blockStart + 3 > _data.length) return Integer.MIN_VALUE;
        ...
        else if (type == TYPE_COMPLEX)
        {
            ...
            if (cellPos + 2 > _data.length) return Integer.MIN_VALUE;
        ...
        else if (type == TYPE_MULTILAYER)
        {
            ...
            {
                if (cellDataPos >= _data.length) return Integer.MIN_VALUE;
                int layers = _data[cellDataPos] & 0xFF;
                cellDataPos += 1 + layers * 2;
            }
            if (cellDataPos + 1 > _data.length) return Integer.MIN_VALUE;
            int layers = _data[cellDataPos] & 0xFF;
            cellDataPos++;
            
            if (layers <= 0) return Integer.MIN_VALUE;
            ...
            for (int l = 0; l < layers; l++)
            {
                ...
                if (layerPos + 2 > _data.length) break;
    ...
    /**
     * Check if cell at (gx, gy) should have its NSWE fixed.
     * Uses findNearestHeight() for neighbor comparisons so that multilayer
     * cells find the closest matching layer instead of only the first layer.
     */
    private static boolean shouldFixCell(int gx, int gy)
    {
        ...
        if ((nswe & NSWE_S) == 0)
        {
            if (gy >= REGION_CELLS_Y - 1) return false;
            int nh = findNearestHeight(gx, gy + 1, h);
            if (nh == Integer.MIN_VALUE || Math.abs(nh - h) > MAX_RAMP_DIFF)
                return false;
        }
        ...
        if ((nswe & NSWE_E) == 0)
        {
            if (gx >= REGION_CELLS_X - 1) return false;
            int nh = findNearestHeight(gx + 1, gy, h);
            if (nh == Integer.MIN_VALUE || Math.abs(nh - h) > MAX_RAMP_DIFF)
                return false;
        }
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        for (int blockIdx = 0; blockIdx < REGION_BLOCKS; blockIdx++)
        {
            ...
            {
                System.err.println("WARNING: Unknown block type " + type + " at block " + blockIdx);
                break;
            }
        }
        ...
        for (int blockIdx = 0; blockIdx < REGION_BLOCKS; blockIdx++)
        {
                ...
                for (int cy = 0; cy < BLOCK_CELLS_Y; cy++)
                    for (int cx = 0; cx < BLOCK_CELLS_X; cx++)
                    {
                        ...
                        if (nswe != NSWE_ALL && shouldFixCell(gx, gy))
                        {
                            ...
                            _cellNSWE[gx][gy] = NSWE_ALL;
                            _fixedComplex++;
                            System.out.println("  FIXED complex (" + gx + "," + gy + ") h=" + height +
                                " NSWE=" + nswe + " (" + describeBlockedNSWE(nswe) + ") -> NSWE=15 (ALL)");
                        }
                    }
                pos += 128;
            }
            else if (type == TYPE_MULTILAYER)
            {
                ...
                for (int cy = 0; cy < BLOCK_CELLS_Y; cy++)
                    for (int cx = 0; cx < BLOCK_CELLS_X; cx++)
                    {
                        if (cellDataPos >= _data.length) break;
                        int layers = _data[cellDataPos] & 0xFF;
                        cellDataPos++;
                        ...
                        // Check each layer individually using nearest-height neighbor lookups
                        for (int l = 0; l < layers; l++)
                        {
                            int layerPos = cellDataPos + l * 2;
                            if (layerPos + 2 > _data.length) break;
                            ...
                            if (shouldFixCell(gx, gy))
                            {
                                ...
                                if (!cellFixed)
                                {
                                    _fixedMultilayer++;
                                    cellFixed = true;
                                    System.out.println("  FIXED multi  (" + gx + "," + gy + ") h=" + height +
                                        " NSWE=" + nswe + " (" + describeBlockedNSWE(nswe) + ") layers=" + layers + " l=" + l + " -> NSWE=15 (ALL)");
                                }
                            }
                        }
                        
                        // Restore original first-layer data for subsequent neighbor checks
        ...
        if (totalFixed > 0)
        {
            ...
            System.out.println("\nPatched file written to: " + new File(outputPath).getAbsolutePath());
            System.out.println("\nTo apply: copy \"" + outputPath + "\" \"" + inputPath + "\"");
    ...
    private static String describeBlockedNSWE(int nswe)
    {
        ...
        if ((nswe & NSWE_N) == 0) sb.append("N");
        if ((nswe & NSWE_S) == 0) sb.append("S");
        if ((nswe & NSWE_W) == 0) sb.append("W");
        if ((nswe & NSWE_E) == 0) sb.append("E");
        return sb.toString();
    }
}
